...
struct RaysSdfSoa {
  ...
  void copy_from_other_async(uint32_t n_elements, const RaysSdfSoa &other,
                             cudaStream_t stream) {
    CUDA_CHECK_THROW(cudaMemcpyAsync(pos.data(), other.pos.data(),
                                     n_elements * sizeof(Eigen::Vector3f),
                                     cudaMemcpyDeviceToDevice, stream));
    CUDA_CHECK_THROW(cudaMemcpyAsync(normal.data(), other.normal.data(),
                                     n_elements * sizeof(Eigen::Vector3f),
                                     cudaMemcpyDeviceToDevice, stream));
    CUDA_CHECK_THROW(cudaMemcpyAsync(distance.data(), other.distance.data(),
                                     n_elements * sizeof(float),
                                     cudaMemcpyDeviceToDevice, stream));
    CUDA_CHECK_THROW(cudaMemcpyAsync(
        prev_distance.data(), other.prev_distance.data(),
        n_elements * sizeof(float), cudaMemcpyDeviceToDevice, stream));
    CUDA_CHECK_THROW(cudaMemcpyAsync(
        total_distance.data(), other.total_distance.data(),
        n_elements * sizeof(float), cudaMemcpyDeviceToDevice, stream));
    CUDA_CHECK_THROW(cudaMemcpyAsync(
        min_visibility.data(), other.min_visibility.data(),
        n_elements * sizeof(float), cudaMemcpyDeviceToDevice, stream));
    CUDA_CHECK_THROW(cudaMemcpyAsync(payload.data(), other.payload.data(),
                                     n_elements * sizeof(SdfPayload),
                                     cudaMemcpyDeviceToDevice, stream));
  }
#endif
