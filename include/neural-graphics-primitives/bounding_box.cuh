...
NGP_HOST_DEVICE inline void project(Eigen::Vector3f points[N_POINTS],
                                    const Eigen::Vector3f &axis, float &min,
                                    float &max) {
...
struct BoundingBox {
  ...
  NGP_HOST_DEVICE BoundingBox(const Eigen::Vector3f &a,
  ...
  BoundingBox(std::vector<Triangle>::iterator begin,
  ...
  NGP_HOST_DEVICE Eigen::Vector3f diag() const { return max - min; }

  NGP_HOST_DEVICE Eigen::Vector3f relative_pos(
      const Eigen::Vector3f &pos) const {
    return (pos - min).cwiseQuotient(diag());
  }

  NGP_HOST_DEVICE Eigen::Vector3f center() const { return 0.5f * (max + min); }
  ...
  NGP_HOST_DEVICE Eigen::Vector2f ray_intersect(
      const Eigen::Vector3f &pos, const Eigen::Vector3f &dir) const {
