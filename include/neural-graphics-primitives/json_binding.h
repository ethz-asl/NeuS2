  ...
  for (std::size_t row = 0; row < j.size(); ++row) {
    ...
    if (jrow.is_array()) {
      for (std::size_t col = 0; col < jrow.size(); ++col) {
        ...
        mat(row, col) =
            value.get<typename Eigen::MatrixBase<Derived>::Scalar>();
      }
    } else {
...
inline void to_json(nlohmann::json &j, const NerfDataset &dataset) {
  ...
  for (size_t i = 0; i < dataset.n_images; ++i) {
    ...
    to_json(j["metadata"].at(i)["rolling_shutter"],
...
inline void from_json(const nlohmann::json &j, NerfDataset &dataset) {
  ...
  for (size_t i = 0; i < dataset.n_images; ++i) {
    ...
    if (j.contains("camera_distortion"))
      from_json(j.at("camera_distortion"),
                dataset.metadata[i].camera_distortion);
    if (j.contains("principal_point"))
      from_json(j.at("principal_point"), dataset.metadata[i].principal_point);
    if (j.contains("rolling_shutter"))
      from_json(j.at("rolling_shutter"), dataset.metadata[i].rolling_shutter);
    if (j.contains("focal_length"))
      from_json(j.at("focal_length"), dataset.metadata[i].focal_length);
    if (j.contains("image_resolution"))
      from_json(j.at("image_resolution"), dataset.metadata[i].resolution);
