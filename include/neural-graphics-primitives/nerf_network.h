...
class NerfNetwork : public tcnn::Network<float, T> {
 public:
  ...
  NerfNetwork(uint32_t n_pos_dims, uint32_t n_dir_dims, uint32_t n_extra_dims,
    ...
    m_pos_encoding.reset(tcnn::create_encoding<T>(
        ...
        density_network.contains("otype") &&
            ...
            ? 2u
            : 2u));
    m_density_network_input_width = tcnn::next_multiple(
        m_n_pos_dims + m_pos_encoding->output_width(), rgb_alignment);  // 40
#else
    m_pos_encoding.reset(tcnn::create_encoding<T>(
        n_pos_dims, pos_encoding,
        density_network.contains("otype") &&
    ...
    m_rgb_network_input_width = tcnn::next_multiple(
        m_n_pos_dims + m_n_pos_dims + m_dir_encoding->padded_output_width() +
    ...
    m_delta_network = std::make_shared<DeltaNetwork<T>>(
        m_pos_encoding->input_width() + m_dir_encoding->input_width());

    m_variance_network = std::make_shared<TrainableBuffer<1, 1, T>>(
        Eigen::Matrix<int, 1, 1>{(int)4});

    m_variance = 0.3f;
    m_training_step = 0;
    m_sdf_bias = (T)(local_density_network_config.value("sdf_bias", -0.1f));

    accumulated_transition = std::make_shared<TrainableBuffer<1, 1, T>>(
        Eigen::Matrix<int, 1, 1>{(int)4});
#if rotation_reprensentation
    accumulated_rotation = std::make_shared<TrainableBuffer<1, 1, T>>(
        Eigen::Matrix<int, 1, 1>{(int)12});
#else
    accumulated_rotation = std::make_shared<TrainableBuffer<1, 1, T>>(
        Eigen::Matrix<int, 1, 1>{(int)4});
#endif
  ...
  void inference_mixed_precision_impl(
    ...
    return;  // Since we need gradient in inference. Should it be replaced by
             // forward_impl?
  ...
  std::unique_ptr<tcnn::Context> forward_impl(
    ...
    forward->density_network_input = tcnn::GPUMatrixDynamic<T>{
        m_density_network_input_width, batch_size, stream,
        m_pos_encoding->preferred_output_layout()};
    forward->density_network_input.memset_async(stream, 0);
    forward->rgb_network_input =
        tcnn::GPUMatrixDynamic<T>{m_rgb_network_input_width, batch_size, stream,
                                  m_dir_encoding->preferred_output_layout()};
    ...
    tcnn::GPUMatrixDynamic<float> deformed_xyz{
        m_pos_encoding->input_width(), batch_size, stream, input.layout()};
    tcnn::GPUMatrixDynamic<float> deformed_viewdir{
        m_dir_encoding->input_width(), batch_size, stream, input.layout()};

    // xyz + delta
    forward->delta_network_output =
        tcnn::GPUMatrixDynamic<float>{m_delta_network->padded_output_width(),
                                      batch_size, stream, input.layout()};

    if (m_use_delta) {
      forward->delta_network_input = tcnn::GPUMatrixDynamic<float>{
          m_delta_network->input_width(), batch_size, stream, input.layout()};
      tcnn::linear_kernel(fill_positions_view<float, float>, 0, stream,
                          ...
                          forward->delta_network_input.view());

      tcnn::linear_kernel(fill_positions_view<float, float>, 0, stream,
                          batch_size * m_dir_encoding->input_width(),
                          m_dir_encoding->input_width(),
                          get_advance(input.view(), m_dir_offset, 0),
                          get_advance(forward->delta_network_input.view(),
                                      m_pos_encoding->input_width(), 0));

      forward->delta_network_ctx = m_delta_network->forward(
          stream, forward->delta_network_input, &forward->delta_network_output,
          use_inference_params, prepare_input_gradients);

      tcnn::linear_kernel(fill_positions_view<float, float>, 0, stream,
                          batch_size * m_pos_encoding->input_width(),
                          m_pos_encoding->input_width(),
                          forward->delta_network_output.view(),
                          deformed_xyz.view());
      tcnn::linear_kernel(fill_positions_view<float, float>, 0, stream,
                          batch_size * m_dir_encoding->input_width(),
                          m_dir_encoding->input_width(),
                          get_advance(forward->delta_network_output.view(),
                                      m_pos_encoding->input_width(), 0),
      ...
      tcnn::linear_kernel(fill_positions_view<float, float>, 0, stream,
                          batch_size * m_pos_encoding->input_width(),
                          m_pos_encoding->input_width(), deformed_xyz.view(),
                          forward->delta_network_output.view());
      tcnn::linear_kernel(fill_positions_view<float, float>, 0, stream,
                          batch_size * m_dir_encoding->input_width(),
                          m_dir_encoding->input_width(),
...
#if GEOMETRY_INIT
    tcnn::GPUMatrixDynamic<T> encoded_xyz{
        m_pos_encoding->padded_output_width(), batch_size, stream,
        forward->density_network_input.layout()};
    forward->pos_encoding_ctx = m_pos_encoding->forward(
        stream, deformed_xyz.slice_rows(0, m_pos_encoding->input_width()),
        &encoded_xyz, use_inference_params, true);

    // xyz + encoding
    tcnn::linear_kernel(
        fill_positions_view_with_fixed_offset<T, float>, 0, stream,
        batch_size * m_pos_encoding->input_width(),
        m_pos_encoding->input_width(),
        deformed_xyz.slice_rows(0, m_pos_encoding->input_width()).view(),
        forward->density_network_input.view());
    ...
    tcnn::linear_kernel(fill_positions_view<T, T>, 0, stream,
                        ...
                        encoded_xyz.view(),
                        get_advance(forward->density_network_input.view(),
                                    m_pos_encoding->input_width(), 0));

#else
    forward->pos_encoding_ctx = m_pos_encoding->forward(
        stream, deformed_xyz.slice_rows(0, m_pos_encoding->input_width()),
        &forward->density_network_input, use_inference_params, true);
#endif
    ...
    forward->density_network_ctx = m_density_network->forward(
        ...
        &forward->density_network_output, use_inference_params, true);
    // end density network forward

    tcnn::GPUMatrixDynamic<T> dSDF_dSDF{
        m_density_network->padded_output_width(), batch_size, stream,
        forward->density_network_output.layout()};
    tcnn::GPUMatrixDynamic<float> dSDF_dDeformedPos{
        m_pos_encoding->input_width(), batch_size, stream,
        forward->density_network_output.layout()};
    dSDF_dSDF.memset_async(stream, 0);
    dSDF_dDeformedPos.memset_async(stream, 0);

    tcnn::linear_kernel(set_constant_value_view<T>, 0, stream, batch_size, 1.0f,
                        dSDF_dSDF.view());

    tcnn::GPUMatrixDynamic<T> dSDF_dPosEncoding{
        m_pos_encoding->padded_output_width(), batch_size, stream,
        m_pos_encoding->preferred_output_layout()};

    forward->dSDF_dPos =
        tcnn::GPUMatrixDynamic<float>{m_pos_encoding->input_width(), batch_size,
                                      stream, input.layout()};  // = gradient.

#if GEOMETRY_INIT
    tcnn::GPUMatrixDynamic<T> dSDF_dSDFInput{
        m_density_network_input_width, batch_size, stream,
        m_pos_encoding->preferred_output_layout()};
    m_density_network->backward(
        stream, *forward->density_network_ctx, forward->density_network_input,
        forward->density_network_output, dSDF_dSDF, &dSDF_dSDFInput,
        use_inference_params, tcnn::EGradientMode::Ignore);
    tcnn::linear_kernel(
        fill_positions_view<T, T>, 0, stream,
        batch_size * m_pos_encoding->padded_output_width(),
        m_pos_encoding->padded_output_width(),
        get_advance(dSDF_dSDFInput.view(), m_pos_encoding->input_width(), 0),
        dSDF_dPosEncoding.view());

    m_pos_encoding->backward(
        stream, *forward->pos_encoding_ctx,
        deformed_xyz.slice_rows(0, m_pos_encoding->input_width()), encoded_xyz,
        dSDF_dPosEncoding, &dSDF_dDeformedPos, use_inference_params,
        tcnn::EGradientMode::Ignore);

    tcnn::linear_kernel(add_positions_view<float, T>, 0, stream,
                        batch_size * m_pos_encoding->input_width(),
                        m_pos_encoding->input_width(), dSDF_dSDFInput.view(),
                        dSDF_dDeformedPos.view());

    forward->dSDF_dPos =
        dSDF_dDeformedPos.slice_rows(0, m_pos_encoding->input_width());
#else
    m_density_network->backward(
        stream, *forward->density_network_ctx, forward->density_network_input,
        forward->density_network_output, dSDF_dSDF, &dSDF_dPosEncoding,
        use_inference_params, tcnn::EGradientMode::Ignore);
    m_pos_encoding->backward(
        stream, *forward->pos_encoding_ctx,
        deformed_xyz.slice_rows(0, m_pos_encoding->input_width()),
        forward->density_network_input, dSDF_dPosEncoding, &dSDF_dDeformedPos,
        use_inference_params, tcnn::EGradientMode::Ignore);
    ...
    forward->dir_encoding_ctx =
        m_dir_encoding->forward(stream, deformed_viewdir, &dir_out,
                                use_inference_params, prepare_input_gradients);
    ...
    tcnn::linear_kernel(fill_positions_view<T, float>, 0, stream,
                        batch_size * m_pos_encoding->input_width(),
                        m_pos_encoding->input_width(), deformed_xyz.view(),
                        get_advance(forward->rgb_network_input.view(),
                                    m_density_network->padded_output_width() +
                                        m_dir_encoding->padded_output_width(),
                                    0));
    ...
    tcnn::GPUMatrixDynamic<T> rgb_network_output = tcnn::GPUMatrixDynamic<T>{
        output->data(), m_rgb_network->padded_output_width(), batch_size,
        output->layout()};
    forward->rgb_network_ctx =
        m_rgb_network->forward(stream, forward->rgb_network_input,
                               output ? &rgb_network_output : nullptr,
                               use_inference_params, prepare_input_gradients);
...
// geo init with sdf bias
#if GEOMETRY_INIT
      tcnn::linear_kernel(
          extract_sdf_value_view_with_bias<T>, 0, stream, batch_size,
          forward->density_network_output.view(), m_sdf_bias,
          get_advance(output->view(), m_pos_encoding->input_width(), 0));
#else
      tcnn::linear_kernel(
          extract_sdf_value_view<T>, 0, stream, batch_size,
          forward->density_network_output.view(),
          get_advance(output->view(), m_pos_encoding->input_width(), 0));
#endif

      tcnn::linear_kernel(
          extract_dSDF_dPos_view<T, float>, 0, stream, batch_size * 3,
          forward->dSDF_dPos.view(),
          get_advance(output->view(), 1 + m_pos_encoding->input_width(), 0));

      tcnn::linear_kernel(extract_single_variance_view<T, T>, 0, stream,
                          batch_size, m_variance_network->params(),
                          output->view());
  ...
  void backward_impl(cudaStream_t stream, const tcnn::Context &ctx,
    ...
    // Make sure our teporary buffers have the correct size for the given batch
    // size
    uint32_t batch_size = input.n();

    tcnn::GPUMatrix<T> dL_drgb{m_rgb_network->padded_output_width(), batch_size,
                               stream};
    CUDA_CHECK_THROW(
        cudaMemsetAsync(dL_drgb.data(), 0, dL_drgb.n_bytes(), stream));
    tcnn::linear_kernel(extract_rgb<T>, 0, stream, batch_size * 3, dL_drgb.m(),
                        dL_doutput.m(), dL_doutput.data(), dL_drgb.data());

    const tcnn::GPUMatrixDynamic<T> rgb_network_output{
        (T *)output.data(), m_rgb_network->padded_output_width(), batch_size,
        output.layout()};
    tcnn::GPUMatrixDynamic<T> dL_drgb_network_input{
        m_rgb_network_input_width, batch_size, stream,
        m_dir_encoding->preferred_output_layout()};
    m_rgb_network->backward(stream, *forward.rgb_network_ctx,
                            forward.rgb_network_input, rgb_network_output,
                            dL_drgb, &dL_drgb_network_input,
                            use_inference_params, param_gradients_mode);

    // Backprop through dir encoding if it is trainable or if we need input
    // gradients

#if viewdir_backward
    tcnn::GPUMatrixDynamic<float> dL_ddir_encoding_input;
    if (m_dir_encoding->n_params() > 0 || dL_dinput ||
        m_use_delta && m_train_delta) {
      printf("some not implemented happend!\n");
      tcnn::GPUMatrixDynamic<T> dL_ddir_encoding_output =
          dL_drgb_network_input.slice_rows(
              m_density_network->padded_output_width(),
              m_dir_encoding->padded_output_width());
      dL_ddir_encoding_input = tcnn::GPUMatrixDynamic<float>{
          m_dir_encoding->input_width(), batch_size, stream, input.layout()};

      m_dir_encoding->backward(
          stream, *forward.dir_encoding_ctx,
          forward.delta_network_output.slice_rows(
              m_pos_encoding->input_width(), m_dir_encoding->input_width()),
          forward.rgb_network_input.slice_rows(
              m_density_network->padded_output_width(),
              m_dir_encoding->padded_output_width()),
          dL_ddir_encoding_output, dL_ddir_encoding_input, use_inference_params,
    ...
    if (m_dir_encoding->n_params() > 0 || dL_dinput) {
      ...
      m_dir_encoding->backward(
          ...
          dL_dinput ? &dL_ddir_encoding_input : nullptr, use_inference_params,
          param_gradients_mode);
    }
    ...
    tcnn::GPUMatrixDynamic<T> dL_ddensity_network_output =
        dL_drgb_network_input.slice_rows(
            0, m_density_network->padded_output_width());
    tcnn::linear_kernel(add_density_gradient<T>, 0, stream, batch_size,
                        dL_doutput.m(), dL_doutput.data(),
                        dL_ddensity_network_output.layout() == tcnn::RM
                            ? 1
...
#if GEOMETRY_INIT
    tcnn::GPUMatrixDynamic<T> dL_dposencoding_output{
        m_pos_encoding->padded_output_width(), batch_size, stream,
        m_pos_encoding->preferred_output_layout()};
    ...
    tcnn::linear_kernel(fill_positions_view<T, T>, 0, stream,
                        batch_size * m_pos_encoding->padded_output_width(),
                        m_pos_encoding->padded_output_width(),
                        get_advance(dL_ddensity_network_input.view(),
                                    m_pos_encoding->input_width(), 0),  //
                        dL_dposencoding_output.view());

    m_pos_encoding->backward(
        stream, *forward.pos_encoding_ctx,
        forward.delta_network_output.slice_rows(0,
                                                m_pos_encoding->input_width()),
        forward.density_network_input.slice_rows(
            m_pos_encoding->input_width(),
            m_pos_encoding->padded_output_width()),  // To do:: need check
        dL_dposencoding_output,
        // dL_dinput ? &dL_dpos_encoding_input : nullptr,
        &dL_dpos_encoding_input, use_inference_params, param_gradients_mode);
    ...
    if (m_train_canonical) {
      // update variance
      {
        ...
        tcnn::linear_kernel(add_variance_view_to_loss<T>, 0, stream, batch_size,
                            dL_doutput.view(), dL_dvariance.view());
        gradients = tcnn::reduce_sum(dL_dvariance.data(), batch_size, stream);
        cudaMemcpyAsync(m_variance_network->gradients(), &gradients, sizeof(T),
                        cudaMemcpyHostToDevice, stream);
      }

      // fill in dloss_d(normal)
      {
        tcnn::GPUMatrixDynamic<float> dL_dsdf_dinput{
            m_pos_encoding->input_width(), batch_size};
        dL_dsdf_dinput.memset_async(stream, 0);

        // d(rgb_output)_d(normal)
        tcnn::linear_kernel(
            fill_positions_view<float, T>, 0, stream,
            batch_size * m_pos_encoding->input_width(),
            m_pos_encoding->input_width(),
            get_advance(dL_drgb_network_input.view(),
                        m_density_network->padded_output_width() +
                            m_dir_encoding->padded_output_width() +
                            m_pos_encoding->input_width(),
                        0),  //
        ...
        tcnn::linear_kernel(add_positions_view_ekloss<float, T>, 0, stream,
                            batch_size * m_pos_encoding->input_width(),
                            m_pos_encoding->input_width(),
                            indeed_batch_size,  // ek_loss's backsize (N) is
                                                // #samples, instead of #rays
        ...
        tcnn::GPUMatrixDynamic<T> dL_denc_output{
            m_pos_encoding->padded_output_width(), batch_size, stream,
            m_pos_encoding->preferred_output_layout()};
        // d(mlp_output)_d(mlp_output)
        tcnn::GPUMatrixDynamic<T> dL_dmlp_output{
            m_density_network->padded_output_width(), batch_size, stream};

        dL_denc_output.memset_async(stream, 0);
        dL_dmlp_output.memset_async(stream, 0);
        tcnn::linear_kernel(set_constant_value_view<T>, 0, stream, batch_size,
                            1.0f, dL_dmlp_output.view());
        ...
        m_density_network->backward(
            stream, *forward.density_network_ctx, forward.density_network_input,
            forward.density_network_output, dL_dmlp_output, &dL_ddensity_input,
            use_inference_params, tcnn::EGradientMode::Ignore);

        tcnn::linear_kernel(fill_positions_view<T, T>, 0, stream,
                            batch_size * m_pos_encoding->padded_output_width(),
                            m_pos_encoding->padded_output_width(),
                            get_advance(dL_ddensity_input.view(),
                                        m_pos_encoding->input_width(), 0),  //
                            dL_denc_output.view());
#else
        m_density_network->backward(
            stream, *forward.density_network_ctx, forward.density_network_input,
            forward.density_network_output, dL_dmlp_output, &dL_denc_output,
            use_inference_params, tcnn::EGradientMode::Ignore);

#endif

        tcnn::GPUMatrixDynamic<float> dL_dsdf_dinput_d_input{
            m_pos_encoding->input_width(), batch_size, stream};
        dL_dsdf_dinput_d_input.memset_async(stream, 0);

        tcnn::GPUMatrixDynamic<T> pos_encoding_dy{
            m_pos_encoding->padded_output_width(), batch_size, stream};
        pos_encoding_dy.memset_async(stream, 0);
        m_pos_encoding->backward_backward_input(
            stream, *forward.pos_encoding_ctx,
            forward.delta_network_output.slice_rows(
                0, m_pos_encoding->input_width()),
            dL_dsdf_dinput,    // dL_d(d(mlp_output)_dx)
            dL_denc_output,    // d(mlp_output)_d(encoding)
            &pos_encoding_dy,  // dl_d(dy'_dy)
            &dL_dsdf_dinput_d_input, use_inference_params,
            tcnn::EGradientMode::Accumulate);

#if GEOMETRY_INIT
        tcnn::GPUMatrixDynamic<T> d_density_input{m_density_network_input_width,
                                                  batch_size, stream};
        ...
        tcnn::linear_kernel(fill_positions_view<T, T>, 0, stream,
                            batch_size * m_pos_encoding->padded_output_width(),
                            m_pos_encoding->padded_output_width(),
                            pos_encoding_dy.view(),
                            get_advance(d_density_input.view(),
                                        m_pos_encoding->input_width(), 0));
        ...
        m_density_network->backward_backward_input(
            ...
            nullptr, nullptr, use_inference_params,
            //  param_gradients_mode
            tcnn::EGradientMode::Accumulate);
        ...
        m_density_network->backward_backward_input(
            stream, *forward.density_network_ctx, forward.density_network_input,
            pos_encoding_dy, dL_dmlp_output,
            nullptr,  // assume dl_ddensity_dx_dx === 0
            nullptr, use_inference_params, tcnn::EGradientMode::Accumulate);

#endif
      }
    }  // backward
    // chain rule loss contain two loss (1. first order loss:
    // dL_ddelta_network_output 2. second order loss: dl_(dsdf_dx))

    if (m_train_delta && m_use_delta) {
      tcnn::GPUMatrixDynamic<float> dL_ddelta_network_output{
          m_delta_network->padded_output_width(), batch_size, stream,
          dL_dpos_encoding_input.layout()};
      dL_ddelta_network_output.memset_async(stream, 0);

      // add dl_dxyz in pos_encoding_input
      tcnn::linear_kernel(fill_positions_view<float, float>, 0, stream,
                          batch_size * 3u, 3u, dL_dpos_encoding_input.view(),
                          dL_ddelta_network_output.view());

      // add dl_dxyz in rgb_network_input [xyz] in [feature_vector +
      // encoded_viewdir + xyz + normals]
      tcnn::linear_kernel(add_positions_view<float, T>, 0, stream,
                          batch_size * 3u, 3u,
                          get_advance(dL_drgb_network_input.view(),
                                      m_density_network->padded_output_width() +
      ...
      tcnn::linear_kernel(fill_positions_view<float, float>, 0, stream,
                          ...
                          get_advance(dL_ddelta_network_output.view(),
                                      m_pos_encoding->input_width(), 0));
#endif
      ...
      tcnn::linear_kernel(add_positions_view<float, T>, 0, stream,
                          batch_size * 3u, 3u, dL_ddensity_network_input.view(),
                          dL_ddelta_network_output.view());
      ...
      if (dL_dinput) {
        ...
        exit(1);
        // dL_ddelta_network_input = dL_dinput->slice_rows(0,
        // m_pos_encoding->input_width());
      }

      m_delta_network->backward(
          stream, *forward.delta_network_ctx,
          // input.slice_rows(0, m_pos_encoding->input_width()),
          forward.delta_network_input, forward.delta_network_output,
          dL_ddelta_network_output,
          dL_dinput ? &dL_ddelta_network_input : nullptr, use_inference_params,
          param_gradients_mode);
    }
  }

  void sdf(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float> &input,
           tcnn::GPUMatrixDynamic<T> &output,
    ...
    if (m_use_delta) {
      ...
      tcnn::linear_kernel(fill_positions_view<float, float>, 0, stream,
                          batch_size * 3u, 3u, input.view(),
                          delta_network_input.view());

      m_delta_network->inference_mixed_precision(
          stream,  // TODO:: accelerate inference w/o cal dy_dx
          delta_network_input, deformed_xyz, use_inference_params);
    } else {
      deformed_xyz = tcnn::GPUMatrixDynamic<float>{
          m_pos_encoding->input_width(), batch_size, stream, input.layout()};
      deformed_xyz = input.slice_rows(0, m_pos_encoding->input_width());
    }
    ...
    tcnn::linear_kernel(
        ...
        m_pos_encoding->input_width(),
        deformed_xyz.slice_rows(0, m_pos_encoding->input_width()).view(),
        density_network_input.view());

    tcnn::linear_kernel(fill_positions_view<T, T>, 0, stream,
                        batch_size * m_pos_encoding->padded_output_width(),
                        m_pos_encoding->padded_output_width(),
                        encoded_xyz.view(),
                        get_advance(density_network_input.view(),
                                    m_pos_encoding->input_width(), 0));
...
#if GEOMETRY_INIT
    tcnn::linear_kernel(sdf_add_bias<T>, 0, stream, batch_size, m_sdf_bias,
                        output.view());
#endif
  ...
  void set_params(T *params, T *inference_params, T *backward_params,
    ...
    m_variance_network->set_params(params + offset, inference_params + offset,
  ...
  std::vector<float> load_sdf_mlp_weight(uint32_t n_elements) {
    ...
    if (!fp) {
      ...
      printf(
          "[ERROR] Please run in the base directory of NeuS2 so that the "
          "`utils/mlp_weights.txt` can be found!\n");
      exit(1);
    } else {
      uint32_t i;
...
// geometry initialization
#if GEOMETRY_INIT
    std::vector<float> sdf_mlp_weight =
        load_sdf_mlp_weight(m_density_network->n_params());
    CUDA_CHECK_THROW(cudaMemcpy(
        params_full_precision + offset, sdf_mlp_weight.data(),
        m_density_network->n_params() * sizeof(float), cudaMemcpyHostToDevice));
#endif
    ...
    tcnn::generate_random_uniform<float>(m_rng, variance_n_params,
                                         params_full_precision + offset, 0.300f,
                                         0.300f);
  ...
  void initialize_sdf_mlp_params(tcnn::pcg32 &rnd, float *params_full_precision,
                                 T *params, T *inference_params,
                                 T *backward_params, T *gradients,
                                 float scale = 1) {
    size_t offset = 0;
...
// geometry initialization
  ...
  size_t n_params_delta() const override { return 0; }
  ...
  uint32_t output_width() const override { return 7; }
  ...
  std::vector<std::pair<uint32_t, uint32_t>> layer_sizes_canonical()
      const override {
    auto layers = m_density_network->layer_sizes();
    auto rgb_layers = m_rgb_network->layer_sizes();
    layers.insert(layers.end(), rgb_layers.begin(), rgb_layers.end());
  ...
  void reset_delta_network() {
    ...
    params_buffer.resize(sizeof(T) * n_params * 3 +
    ...
    T *new_m_params_backward =
        (T *)(params_buffer.data() + sizeof(float) * n_params +
              sizeof(T) * n_params);
    T *new_m_param_gradients =
        (T *)(params_buffer.data() + sizeof(float) * n_params +
              sizeof(T) * n_params * 2);
    ...
    m_delta_network->initialize_params(
        rnd, new_m_params_full_precision + offset, new_m_params + offset,
        new_m_params + offset, new_m_params_backward + offset,
        new_m_param_gradients + offset);
  }

  void init_accumulation_movement() {
    uint32_t n_params =
        accumulated_transition->n_params() + accumulated_rotation->n_params();

    m_accumulation_params_buffer.resize(sizeof(T) * n_params * 3 +
                                        sizeof(float) * n_params * 1);
    m_accumulation_params_buffer.memset(0);

    float *params_full_precision =
        (float *)(m_accumulation_params_buffer.data());
    T *params =
        (T *)(m_accumulation_params_buffer.data() + sizeof(float) * n_params);
    T *backward_params = (T *)(m_accumulation_params_buffer.data() +
                               sizeof(float) * n_params + sizeof(T) * n_params);
    T *gradients = (T *)(m_accumulation_params_buffer.data() +
                         sizeof(float) * n_params + sizeof(T) * n_params * 2);
    ...
    accumulated_transition->initialize_params(
        rnd, params_full_precision + offset, params + offset, params + offset,
        backward_params + offset, gradients + offset, scale);

    tcnn::generate_random_uniform<float>(
        rnd, 4u, params_full_precision + offset, 0.000f, 0.000f);
    ...
    tcnn::generate_random_uniform<float>(
        rnd, 1u, params_full_precision + offset, 1.000f, 1.000f);
    tcnn::generate_random_uniform<float>(
        rnd, 3u, params_full_precision + offset + 1u, 0.000f, 0.000f);
    tcnn::generate_random_uniform<float>(
        rnd, 1u, params_full_precision + offset + 4u, 1.000f, 1.000f);
    tcnn::generate_random_uniform<float>(
        rnd, 3u, params_full_precision + offset + 5u, 0.000f, 0.000f);
    tcnn::generate_random_uniform<float>(
        rnd, 1u, params_full_precision + offset + 8u, 1.000f, 1.000f);
#else
    tcnn::generate_random_uniformc<float>(
        rnd, 3u, params_full_precision + offset, 0.000f, 0.000f);
    tcnn::generate_random_uniform<float>(
        rnd, 1u, params_full_precision + offset + 3u, 1.000f, 1.000f);
#endif

    // initialize_params is only expected to initialize m_params_full_precision.
    // Cast and copy these over!
    parallel_for_gpu(n_params, [params_fp = params_full_precision,
                                params = params] __device__(size_t i) {
  ...
  const float &variance() const { return m_variance; }

  float cos_anneal_ratio() const {
    if (m_anneal_end == 0) {
      return 1.0;
    } else {
      // printf("m_training_step:%d,
      // m_anneal_end:%d",m_training_step,m_anneal_end); printf("m_nerf_network
      // cos_anneal_ratio:%f", min(1.0, (float)m_training_step / m_anneal_end));
      return min(1.0, (float)m_training_step / m_anneal_end);
    }
  }

  void set_anneal_end(const int &anneal_end) { m_anneal_end = anneal_end; }
  ...
  const uint32_t &training_step() const { return m_training_step; }
...
#if rotation_reprensentation
    tcnn::linear_kernel(accumulate_global_movement_rotation_6d_kernel<T>, 0,
                        stream, 1u, m_delta_network->rotation()->params(),
                        m_delta_network->transition()->params(),
                        accumulated_rotation->params(),
                        accumulated_transition->params());

    CUDA_CHECK_THROW(cudaMemcpy(accumulated_rotation->params_inference(),
                                accumulated_rotation->params(),
                                sizeof(T) * accumulated_rotation->n_params(),
                                cudaMemcpyDeviceToDevice));
    CUDA_CHECK_THROW(cudaMemcpy(accumulated_transition->params_inference(),
                                accumulated_transition->params(),
                                sizeof(T) * accumulated_transition->n_params(),
                                cudaMemcpyDeviceToDevice));
#else
    tcnn::linear_kernel(
        accumulate_global_movement_rotation_quaternion_kernel<T>, 0, stream, 1u,
        m_delta_network->rotation()->params(),
        m_delta_network->transition()->params(), accumulated_rotation->params(),
        accumulated_transition->params());
#endif
  }

  void save_global_movement(cudaStream_t stream, tcnn::json &network_config) {
    // when save_global_movement, we have not accumulated the global movement,
    // so we need to accumulate it first

    tcnn::GPUMemory<T> save_accumulated_rotation;
    tcnn::GPUMemory<T> save_accumulated_transition;

    save_accumulated_rotation.resize(sizeof(T) *
                                     accumulated_rotation->n_params());
    save_accumulated_transition.resize(sizeof(T) *
                                       accumulated_transition->n_params());

#if rotation_reprensentation
    tcnn::linear_kernel(
        save_global_movement_rotation_6d_kernel<T>, 0, stream, 1u,
        m_delta_network->rotation()->params(),
        m_delta_network->transition()->params(), accumulated_rotation->params(),
        accumulated_transition->params(), save_accumulated_rotation.data(),
    ...
    network_config["snapshot"]["rotation"] = tcnn::gpu_memory_to_json_binary(
        accumulated_rotation->params(),
        sizeof(T) * accumulated_rotation->n_params());
    network_config["snapshot"]["transition"] = tcnn::gpu_memory_to_json_binary(
        accumulated_transition->params(),
        sizeof(T) * accumulated_transition->n_params());
  }
  ...
  void load_global_movement(const tcnn::json network_config) {
    ...
    parallel_for_gpu(n_params,
                     [params = accumulated_rotation->params(),
                      params_hp = params_hp.data()] __device__(size_t i) {
    ...
    parallel_for_gpu(n_params,
                     [params = accumulated_transition->params(),
                      params_hp = params_hp.data()] __device__(size_t i) {
                       params[i] = (T)params_hp[i];
                     });
    ...
    CUDA_CHECK_THROW(cudaMemcpy(rotation_quat.data(), rotation_quat_gpu,
                                (9) * sizeof(precision_t),
                                cudaMemcpyDeviceToHost));
    CUDA_CHECK_THROW(cudaMemcpy(transition.data(), transition_gpu,
                                (3) * sizeof(precision_t),
                                cudaMemcpyDeviceToHost));
    printf("rotation: %f, %f, %f\n", (float)rotation_quat[0],
           (float)rotation_quat[1], (float)rotation_quat[2]);
    printf("rotation: %f, %f, %f\n", (float)rotation_quat[3],
           (float)rotation_quat[4], (float)rotation_quat[5]);
    printf("rotation: %f, %f, %f\n", (float)rotation_quat[6],
           (float)rotation_quat[7], (float)rotation_quat[8]);
    printf("transition: %f, %f, %f\n\n", (float)transition[0],
           (float)transition[1], (float)transition[2]);
  }

  void save_local_movement(cudaStream_t stream, tcnn::json &network_config) {
    network_config["snapshot"]["local_rotation"] =
        tcnn::gpu_memory_to_json_binary(
            m_delta_network->rotation()->params(),
            sizeof(T) * m_delta_network->rotation()->n_params());
    network_config["snapshot"]["local_transition"] =
        tcnn::gpu_memory_to_json_binary(
            m_delta_network->transition()->params(),
            sizeof(T) * m_delta_network->transition()->n_params());
  }
  ...
  void load_local_movement(const tcnn::json network_config) {
    ...
    parallel_for_gpu(n_params,
                     [params = m_delta_network->rotation()->params(),
                      params_hp = params_hp.data()] __device__(size_t i) {
                       params[i] = (T)params_hp[i];
                     });
    ...
    parallel_for_gpu(n_params,
                     [params = m_delta_network->transition()->params(),
                      params_hp = params_hp.data()] __device__(size_t i) {
                       params[i] = (T)params_hp[i];
                     });
  }
