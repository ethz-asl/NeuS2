...
struct Triangle {
  NGP_HOST_DEVICE Eigen::Vector3f sample_uniform_position(
  ...
  NGP_HOST_DEVICE float ray_intersect(const Eigen::Vector3f &ro,
  ...
  NGP_HOST_DEVICE float distance_sq(const Eigen::Vector3f &pos) const {
    ...
    return
        // inside/outside test
        (sign(v21.cross(nor).dot(p1)) + sign(v32.cross(nor).dot(p2)) +
         ...
         2.0f)
            ...
            std::min({
                (v21 *
                     tcnn::clamp(v21.dot(p1) / v21.squaredNorm(), 0.0f, 1.0f) -
                 p1)
                    .squaredNorm(),
                (v32 *
                     tcnn::clamp(v32.dot(p2) / v32.squaredNorm(), 0.0f, 1.0f) -
                 p2)
                    .squaredNorm(),
                (v13 *
                     tcnn::clamp(v13.dot(p3) / v13.squaredNorm(), 0.0f, 1.0f) -
                 p3)
                    .squaredNorm(),
            })
            :
  ...
  NGP_HOST_DEVICE Eigen::Vector3f closest_point_to_line(
