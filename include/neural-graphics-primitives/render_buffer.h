...
class CudaSurface2D : public SurfaceProvider {
 public:
  ...
  cudaSurfaceObject_t surface() override { return m_surface; }

  cudaArray_t array() override { return m_array; }

  Eigen::Vector2i resolution() const override { return m_size; }
...
class GLTexture : public SurfaceProvider {
 public:
  ...
  void resize(const Eigen::Vector2i &new_size, int n_channels,
              bool is_8bit = false);

  void resize(const Eigen::Vector2i &new_size) override { resize(new_size, 4); }
  ...
  class CUDAMapping {
   public:
    ...
    cudaSurfaceObject_t surface() const {
      return m_cuda_surface ? m_cuda_surface->surface() : m_surface;
    }

    cudaArray_t array() const {
      return m_cuda_surface ? m_cuda_surface->array() : m_mapped_array;
    }
...
class CudaRenderBuffer {
 public:
  CudaRenderBuffer(const std::shared_ptr<SurfaceProvider> &surf)
      : m_surface_provider{surf} {}

  CudaRenderBuffer(const CudaRenderBuffer &other) = delete;
  CudaRenderBuffer(CudaRenderBuffer &&other) = default;

  cudaSurfaceObject_t surface() { return m_surface_provider->surface(); }

  Eigen::Vector2i in_resolution() const { return m_in_resolution; }
  ...
  void reset_accumulation() { m_spp = 0; }

  uint32_t spp() const { return m_spp; }

  Eigen::Array4f *frame_buffer() const { return m_frame_buffer.data(); }

  float *depth_buffer() const { return m_depth_buffer.data(); }
  ...
  void tonemap(float exposure, const Eigen::Array4f &background_color,
               EColorSpace output_color_space, cudaStream_t stream);

  void overlay_image(float alpha, const Eigen::Array3f &exposure,
                     ...
                     const Eigen::Vector2i &resolution, int fov_axis,
                     float zoom, const Eigen::Vector2f &screen_center,
                     cudaStream_t stream);

  void overlay_false_color(Eigen::Vector2i training_resolution, bool to_srgb,
                           int fov_axis, cudaStream_t stream,
                           const float *error_map,
                           Eigen::Vector2i error_map_resolution,
                           const float *average, float brightness,
                           bool viridis);

  SurfaceProvider &surface_provider() { return *m_surface_provider; }
  ...
  void disable_dlss();
  void set_dlss_sharpening(float value) { m_dlss_sharpening = value; }

  const std::shared_ptr<IDlss> &dlss() const { return m_dlss; }
