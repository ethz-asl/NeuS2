...
__global__ void kernel_takikawa(
    ...
    const TriangleOctreeDualNode *octree_dual_nodes, const T *__restrict__ grid,
    const tcnn::MatrixView<const float> data_in, tcnn::MatrixView<T> data_out,
  ...
  int level = TriangleOctree::traverse(
      octree_nodes, octree_dual_nodes, n_levels + starting_level,
      {
          data_in(0, i),
          data_in(1, i),
          data_in(2, i),
      },
      [&](const TriangleOctreeDualNode &node, uint32_t level,
          Eigen::Vector3f pos) {
...
#pragma unroll
          for (uint32_t feature = 0; feature < N_FEATURES_PER_LEVEL;
               ++feature) {
            data_out(level * N_FEATURES_PER_LEVEL + feature, i) =
                result[feature];
          }
        }
...
#pragma unroll
            ...
            *(tcnn::vector_fullp_t<N_FEATURES_PER_LEVEL>
                  *)&dy_dx[i * fan_out_grad + level * N_FEATURES_PER_LEVEL +
...
__global__ void kernel_takikawa_backward_input(
    ...
    const tcnn::MatrixView<const T> dL_dy, const float *__restrict__ dy_dx,
...
class TakikawaEncoding : public tcnn::Encoding<T> {
 public:
  ...
  std::unique_ptr<tcnn::Context> forward_impl(
    ...
    tcnn::linear_kernel(
        kernel_takikawa<T, N_FEATURES_PER_LEVEL>, 0, stream, input.n(),
        n_levels(), m_starting_level, m_interpolation_type,
        m_octree->nodes_gpu(), m_octree->dual_nodes_gpu(),
        use_inference_params ? m_params_inference : m_params, input.view(),
        output ? output->view() : tcnn::MatrixView<T>{}, forward->dy_dx.data());
  ...
  // tcnn::EGradientMode::Overwrite ) override { 	return;
  // }
  ...
  void backward_impl(cudaStream_t stream, const tcnn::Context &ctx,
    ...
    if (param_gradients_mode != tcnn::EGradientMode::Ignore) {
      ...
      if (!std::is_same<grad_t, T>::value) {
        params_gradient_tmp =
            tcnn::allocate_workspace(stream, n_params() * sizeof(grad_t));
      ...
      if (param_gradients_mode == tcnn::EGradientMode::Overwrite) {
        CUDA_CHECK_THROW(cudaMemsetAsync(params_gradient, 0,
                                         n_params() * sizeof(grad_t), stream));
      }

      tcnn::linear_kernel(
          kernel_takikawa_backward<T, grad_t, N_FEATURES_PER_LEVEL>, 0, stream,
          num_elements, n_levels(), m_starting_level, m_interpolation_type,
          m_octree->nodes_gpu(), m_octree->dual_nodes_gpu(), params_gradient,
          input.view(), dL_doutput.view());
    ...
    if (dL_dinput) {
      tcnn::linear_kernel(kernel_takikawa_backward_input<T>, 0, stream,
                          ...
                          N_FEATURES_PER_LEVEL * n_levels(), dL_doutput.view(),
                          forward.dy_dx.data(), dL_dinput->view());
    }
  }

  uint32_t input_width() const override { return 3; }

  uint32_t padded_output_width() const override {
    return m_n_padded_output_dims;
  }

  uint32_t output_width() const override { return m_n_padded_output_dims; }
  ...
  void set_alignment(uint32_t alignment) override {
    if (m_n_output_dims != tcnn::next_multiple(m_n_output_dims, alignment)) {
      throw std::runtime_error{
          std::string{"TakikawaEncoding only supports number of output dims "
                      "that divide into "} +
  ...
  uint32_t min_alignment() const override { return N_FEATURES_PER_LEVEL; }
  ...
  void initialize_params(tcnn::pcg32 &rnd, float *params_full_precision,
    ...
    // Initialize the encoding from the GPU, because the number of parameters
    // can be quite large.
    tcnn::generate_random_uniform<float>(rnd, n_params(), params_full_precision,
                                         -1e-4f, 1e-4f);
  }
  ...
  uint32_t n_levels() const { return m_octree->depth() - m_starting_level; }
