 ...
 *  @brief  Shared functionality among multiple neural-graphics-primitives
 * components.
 */
...
inline __host__ __device__ Eigen::Array3f srgb_to_linear(
    const Eigen::Array3f &x) {
  return {srgb_to_linear(x.x()), srgb_to_linear(x.y()),
          (srgb_to_linear(x.z()))};
}
...
inline __host__ __device__ Eigen::Array3f srgb_to_linear_derivative(
    const Eigen::Array3f &x) {
  return {srgb_to_linear_derivative(x.x()), srgb_to_linear_derivative(x.y()),
          (srgb_to_linear_derivative(x.z()))};
}
...
inline __host__ __device__ Eigen::Array3f linear_to_srgb(
    const Eigen::Array3f &x) {
...
inline __host__ __device__ Eigen::Array3f linear_to_srgb_derivative(
    const Eigen::Array3f &x) {
  return {linear_to_srgb_derivative(x.x()), linear_to_srgb_derivative(x.y()),
          (linear_to_srgb_derivative(x.z()))};
}

template <uint32_t N_DIMS, typename T>
__host__ __device__ Eigen::Matrix<float, N_DIMS, 1> read_image(
    const T *__restrict__ data, const Eigen::Vector2i &resolution,
  ...
  auto read_val = [&](Eigen::Vector2i pos) {
    ...
    if (std::is_same<T, float>::value) {
      result = *(Eigen::Matrix<T, N_DIMS, 1>
                     *)&data[(pos.x() + pos.y() * resolution.x()) * N_DIMS];
    } else {
      auto val = *(tcnn::vector_t<T, N_DIMS>
                       *)&data[(pos.x() + pos.y() * resolution.x()) * N_DIMS];
...
__device__ void deposit_image_gradient(
    const Eigen::Matrix<float, N_DIMS, 1> &value, T *__restrict__ gradient,
    T *__restrict__ gradient_weight, const Eigen::Vector2i &resolution,
    const Eigen::Vector2f &pos) {
  ...
  auto deposit_val = [&](const Eigen::Matrix<float, N_DIMS, 1> &value, T weight,
                         Eigen::Vector2i pos) {
    pos.x() = std::max(std::min(pos.x(), resolution.x() - 1), 0);
    pos.y() = std::max(std::min(pos.y(), resolution.y() - 1), 0);
    ...
    if (std::is_same<T, __half>::value) {
      for (uint32_t c = 0; c < N_DIMS; c += 2) {
        atomicAdd(
            (__half2
                 *)&gradient[(pos.x() + pos.y() * resolution.x()) * N_DIMS + c],
  ...
  deposit_val(value, (1 - weight.x()) * (1 - weight.y()),
              {texel.x(), texel.y()});
  deposit_val(value, (weight.x()) * (1 - weight.y()),
              {texel.x() + 1, texel.y()});
  deposit_val(value, (1 - weight.x()) * (weight.y()),
              {texel.x(), texel.y() + 1});
  deposit_val(value, (weight.x()) * (weight.y()),
              {texel.x() + 1, texel.y() + 1});
}

template <typename T>
__device__ __host__ inline void apply_camera_distortion(const T *extra_params,
                                                        const T u, const T v,
                                                        T *du, T *dv) {
...
template <typename T>
__device__ __host__ inline void iterative_camera_undistortion(const T *params,
                                                              T *u, T *v) {
...
inline __host__ __device__ Ray pixel_to_ray_pinhole(
    uint32_t spp, const Eigen::Vector2i &pixel,
    const Eigen::Vector2i &resolution, const Eigen::Vector2f &focal_length,
    const Eigen::Matrix<float, 3, 4> &camera_matrix,
    const Eigen::Vector2f &screen_center, float focus_z = 1.0f,
    float dof = 0.0f) {
  const Eigen::Vector2f uv =
      pixel.cast<float>().cwiseQuotient(resolution.cast<float>());
...
inline __host__ __device__ Eigen::Matrix<float, 3, 4>
get_xform_given_rolling_shutter(const TrainingXForm &training_xform,
...
inline __host__ __device__ Eigen::Vector3f f_theta_undistortion(
    const Eigen::Vector2f &uv, const float *params,
    const Eigen::Vector3f &error_direction) {
  // we take f_theta intrinsics to be: r0, r1, r2, r3, resx, resy; we rescale to
  ...
  float alpha =
      params[0] +
      norm * (params[1] +
              norm * (params[2] + norm * (params[3] + norm * params[4])));
...
inline __host__ __device__ Ray pixel_to_ray(
    ...
    const Eigen::Vector2i &resolution, const Eigen::Vector2f &focal_length,
    const Eigen::Matrix<float, 3, 4> &camera_matrix,
    const Eigen::Vector2f &screen_center, const Eigen::Vector3f &parallax_shift,
    bool snap_to_pixel_centers = false, float focus_z = 1.0f, float dof = 0.0f,
  ...
  Eigen::Vector2f offset =
      ld_random_pixel_offset(snap_to_pixel_centers ? 0 : spp);
  Eigen::Vector2f uv =
      (pixel.cast<float>() + offset).cwiseQuotient(resolution.cast<float>());

  Eigen::Vector3f dir;
  if (camera_distortion.mode == ECameraDistortionMode::FTheta) {
    dir = f_theta_undistortion(uv - screen_center, camera_distortion.params,
                               {1000.f, 0.f, 0.f});
    if (dir.x() == 1000.f) {
      return {{1000.f, 0.f, 0.f},
              {0.f, 0.f, 1.f}};  // return a point outside the aabb so the pixel
                                 // is not rendered
    }
  } else {
    dir = {
    ...
    if (camera_distortion.mode == ECameraDistortionMode::Iterative) {
      iterative_camera_undistortion(camera_distortion.params, &dir.x(),
                                    &dir.y());
    }
  }
  if (distortion_data) {
  ...
  dir -= head_pos /
         parallax_shift.z();  // we could use focus_z here in the denominator.
                              // for now, we pack m_scale in here.
  dir = camera_matrix.block<3, 3>(0, 0) * dir;

  Eigen::Vector3f origin =
      camera_matrix.block<3, 3>(0, 0) * head_pos + camera_matrix.col(3);
  if (dof == 0.0f) {
    return {origin, dir};
  }
  ...
  Eigen::Vector2f blur =
      dof * square2disk_shirley(
                ld_random_val_2d(spp, (uint32_t)pixel.x() * 19349663 +
...
inline __host__ __device__ Eigen::Vector2f pos_to_pixel(
    const Eigen::Vector3f &pos, const Eigen::Vector2i &resolution,
  ...
  if (camera_distortion.mode == ECameraDistortionMode::Iterative) {
    float du, dv;
    apply_camera_distortion(camera_distortion.params, dir.x(), dir.y(), &du,
                            &dv);
    dir.x() += du;
    dir.y() += dv;
...
inline __host__ __device__ Eigen::Vector2f motion_vector_3d(
    const uint32_t sample_index, const Eigen::Vector2i &pixel,
    const Eigen::Vector2i &resolution, const Eigen::Vector2f &focal_length,
    ...
    const CameraDistortion &camera_distortion = {}) {
  Ray ray =
      pixel_to_ray(sample_index, pixel, resolution, focal_length, camera,
                   screen_center, parallax_shift, snap_to_pixel_centers, 1.0f,
                   0.0f, camera_distortion, nullptr, Eigen::Vector2i::Zero());

  Eigen::Vector2f prev_pixel =
      pos_to_pixel(ray.o + ray.d * depth, resolution, focal_length, prev_camera,
                   screen_center, parallax_shift, camera_distortion);

  return prev_pixel -
         (pixel.cast<float>() + ld_random_pixel_offset(sample_index));
}

inline __host__ __device__ Eigen::Vector2f pixel_to_image_uv(
    const uint32_t sample_index, const Eigen::Vector2i &pixel,
    const Eigen::Vector2i &resolution, const Eigen::Vector2i &image_resolution,
    const Eigen::Vector2f &screen_center, const float view_dist,
    const Eigen::Vector2f &image_pos, const bool snap_to_pixel_centers) {
  Eigen::Vector2f jit =
...
inline __host__ __device__ Eigen::Vector2f image_uv_to_pixel(
  ...
  return {((uv.x() / image_resolution.y() * image_resolution.x()) +
           view_dist * image_pos.x()) *
                  resolution.x() / x_scale -
              offset.x(),
          (uv.y() + view_dist * image_pos.y()) * resolution.y() / y_scale -
              offset.y()};
}
...
inline __host__ __device__ Eigen::Vector2f motion_vector_2d(
    ...
    const Eigen::Vector2i &resolution, const Eigen::Vector2i &image_resolution,
    const Eigen::Vector2f &screen_center, const float view_dist,
    const float prev_view_dist, const Eigen::Vector2f &image_pos,
    const Eigen::Vector2f &prev_image_pos, const bool snap_to_pixel_centers) {
  Eigen::Vector2f uv = pixel_to_image_uv(
      sample_index, pixel, resolution, image_resolution, screen_center,
      view_dist, image_pos, snap_to_pixel_centers);

  Eigen::Vector2f prev_pixel =
      image_uv_to_pixel(uv, resolution, image_resolution, screen_center,
                        prev_view_dist, prev_image_pos);

  return prev_pixel -
         (pixel.cast<float>() + ld_random_pixel_offset(sample_index));
}

inline __host__ __device__ float fov_to_focal_length(int resolution,
                                                     float degrees) {
  return 0.5f * (float)resolution / tanf(0.5f * degrees * (float)PI() / 180);
}

inline __host__ __device__ Eigen::Vector2f fov_to_focal_length(
    const Eigen::Vector2i &resolution, const Eigen::Vector2f &degrees) {
  return 0.5f *
         resolution.cast<float>().cwiseQuotient(
             (0.5f * degrees * (float)PI() / 180).array().tan().matrix());
}

inline __host__ __device__ float focal_length_to_fov(int resolution,
                                                     float focal_length) {
  return 2.f * 180.f / PI() * atanf(float(resolution) / (focal_length * 2.f));
}

inline __host__ __device__ Eigen::Vector2f focal_length_to_fov(
    const Eigen::Vector2i &resolution, const Eigen::Vector2f &focal_length) {
  return 2.f * 180.f / PI() *
...
inline __host__ __device__ Eigen::Vector3f faceforward(
    const Eigen::Vector3f &n, const Eigen::Vector3f &i,
    const Eigen::Vector3f &nref) {
  return n * copysignf(1.0f, i.dot(nref));
}

template <typename T>
__global__ void from_rgba32(const uint64_t num_pixels,
...
inline __host__ __device__ Eigen::Array3f hsv_to_rgb(
    const Eigen::Array3f &hsv) {
...
inline __host__ __device__ Eigen::Array3f to_rgb(const Eigen::Vector2f &dir) {
  return hsv_to_rgb(
      {atan2f(dir.y(), dir.x()) / (2.0f * PI()) + 0.5f, 1.0f, dir.norm()});
}
...
inline NGP_HOST_DEVICE Eigen::Vector2i image_pos(
    const Eigen::Vector2f &pos, const Eigen::Vector2i &resolution) {
  return pos.cwiseProduct(resolution.cast<float>())
      .cast<int>()
      .cwiseMin(resolution - Eigen::Vector2i::Constant(1))
      .cwiseMax(0);
}

inline NGP_HOST_DEVICE uint64_t pixel_idx(const Eigen::Vector2i &pos,
...
inline NGP_HOST_DEVICE uint64_t pixel_idx(const Eigen::Vector2f &xy,
                                          const Eigen::Vector2i &resolution,
                                          uint32_t img) {
  return pixel_idx(image_pos(xy, resolution), resolution, img);
}

// inline NGP_HOST_DEVICE Array3f composit_and_lerp(Vector2f pos, const
// Vector2i& resolution, uint32_t img, const __half* training_images, const
// Array3f& background_color, const Array3f& exposure_scale = Array3f::Ones()) {
// 	pos = (pos.cwiseProduct(resolution.cast<float>()) -
// Vector2f::Constant(0.5f)).cwiseMax(0.0f).cwiseMin(resolution.cast<float>() -
// Vector2f::Constant(1.0f + 1e-4f));

// 	const Vector2i pos_int = pos.cast<int>();
// 	const Vector2f weight = pos - pos_int.cast<float>();

// 	const Vector2i idx = pos_int.cwiseMin(resolution -
// Vector2i::Constant(2)).cwiseMax(0);

// 	auto read_val = [&](const Vector2i& p) {
// 		__half val[4];
// 		*(uint64_t*)&val[0] = ((uint64_t*)training_images)[pixel_idx(p,
// resolution, img)]; 		return Array3f{val[0], val[1], val[2]} * exposure_scale +
// background_color * (1.0f - (float)val[3]);
// 	};

// 	return (
// 		(1 - weight.x()) * (1 - weight.y()) * read_val({idx.x(),
// idx.y()}) + 		(weight.x()) * (1 - weight.y()) * read_val({idx.x()+1, idx.y()})
// + 		(1 - weight.x()) * (weight.y()) * read_val({idx.x(), idx.y()+1}) +
// 		(weight.x()) * (weight.y()) * read_val({idx.x()+1, idx.y()+1})
// 	);
// }

// inline NGP_HOST_DEVICE Array3f composit(Vector2f pos, const Vector2i&
// resolution, uint32_t img, const __half* training_images, const Array3f&
// background_color, const Array3f& exposure_scale = Array3f::Ones()) { 	auto
// read_val = [&](const Vector2i& p) {
// 		__half val[4];
// 		*(uint64_t*)&val[0] = ((uint64_t*)training_images)[pixel_idx(p,
// resolution, img)]; 		return Array3f{val[0], val[1], val[2]} * exposure_scale +
// background_color * (1.0f - (float)val[3]);
...
inline NGP_HOST_DEVICE Eigen::Array4f read_rgba(
    Eigen::Vector2i px, const Eigen::Vector2i &resolution, const void *pixels,
  ...
  switch (image_data_type) {
    ...
    case EImageDataType::Byte: {
      uint8_t val[4];
      *(uint32_t *)&val[0] =
          ((uint32_t *)pixels)[pixel_idx(px, resolution, img)];
    ...
    case EImageDataType::Half: {
      __half val[4];
      *(uint64_t *)&val[0] =
          ((uint64_t *)pixels)[pixel_idx(px, resolution, img)];
...
inline NGP_HOST_DEVICE Eigen::Array4f read_rgba(
    Eigen::Vector2f pos, const Eigen::Vector2i &resolution, const void *pixels,
    EImageDataType image_data_type, uint32_t img = 0) {
  return read_rgba(image_pos(pos, resolution), resolution, pixels,
                   image_data_type, img);
}

inline NGP_HOST_DEVICE float read_depth(Eigen::Vector2f pos,
                                        const Eigen::Vector2i &resolution,
                                        const float *depth, uint32_t img = 0) {
...
Eigen::Matrix<float, 3, 4> log_space_lerp(
    const Eigen::Matrix<float, 3, 4> &begin,
    const Eigen::Matrix<float, 3, 4> &end, float t);

tcnn::GPUMemory<float> load_exr(const std::string &filename, int &width,
                                int &height);
tcnn::GPUMemory<float> load_stbi(const std::string &filename, int &width,
                                 int &height);
