...
struct TrainingImageMetadata {
  ...
  // the memory to back the pixels and rays is held by GPUMemory objects in the
  // NerfDataset and copied here.
  ...
  Eigen::Vector3f light_dir =
      Eigen::Vector3f::Constant(0.f);  // TODO: replace this with more generic
                                       // float[] of task-specific metadata.
};

inline size_t image_type_size(EImageDataType type) {
  switch (type) {
    case EImageDataType::None:
      return 0;
...
struct NerfDataset {
  ...
  void set_training_image(int frame_idx,
                          ...
                          const void *pixels, const void *depth_pixels,
                          float depth_scale, bool image_data_on_gpu,
                          EImageDataType image_type, EDepthDataType depth_type,
                          float sharpen_amount = 0.f,
                          bool white_transparent = false,
                          bool black_transparent = false,
                          uint32_t mask_color = 0, const Ray *rays = nullptr);
  ...
  Eigen::Matrix<float, 3, 4> nerf_matrix_to_ngp(
      const Eigen::Matrix<float, 3, 4> &nerf_matrix) {
  ...
  Eigen::Matrix<float, 3, 4> ngp_matrix_to_nerf(
      const Eigen::Matrix<float, 3, 4> &ngp_matrix) {
  ...
  void nerf_ray_to_ngp(Ray &ray, bool scale_direction = false) {
    ...
    if (scale_direction) ray.d *= scale;
...
NerfDataset load_nerf(const std::vector<filesystem::path> &jsonpaths,
                      float sharpen_amount = 0.f);
NerfDataset create_empty_nerf_dataset(size_t n_images, int aabb_scale = 1,
                                      bool is_hdr = false);
