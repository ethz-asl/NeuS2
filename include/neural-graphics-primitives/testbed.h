...
class Testbed {
 public:
  ...
  ~Testbed();
  Testbed(ETestbedMode mode, const std::string &data_path) : Testbed(mode) {
    load_training_data(data_path);
  }
  Testbed(ETestbedMode mode, const std::string &data_path,
          const std::string &network_config_path)
      : Testbed(mode, data_path) {
  ...
  using distance_fun_t =
      std::function<void(uint32_t, const tcnn::GPUMemory<Eigen::Vector3f> &,
                         tcnn::GPUMemory<float> &, cudaStream_t)>;
  using normals_fun_t =
      std::function<void(uint32_t, const tcnn::GPUMemory<Eigen::Vector3f> &,
                         tcnn::GPUMemory<Eigen::Vector3f> &, cudaStream_t)>;
  ...
  class SphereTracer {
   public:
    ...
    void init_rays_from_camera(
        uint32_t spp, const Eigen::Vector2i &resolution,
        const Eigen::Vector2f &focal_length,
        const Eigen::Matrix<float, 3, 4> &camera_matrix,
        const Eigen::Vector2f &screen_center,
        const Eigen::Vector3f &parallax_shift, bool snap_to_pixel_centers,
        const BoundingBox &aabb, float floor_y, float plane_z, float dof,
        const float *envmap_data, const Eigen::Vector2i &envmap_resolution,
        Eigen::Array4f *frame_buffer, float *depth_buffer,
        const TriangleOctree *octree, uint32_t n_octree_levels,
        cudaStream_t stream);

    void init_rays_from_data(uint32_t n_elements, const RaysSdfSoa &data,
                             cudaStream_t stream);
    uint32_t trace_bvh(TriangleBvh *bvh, const Triangle *triangles,
                       cudaStream_t stream);
    uint32_t trace(const distance_fun_t &distance_function, float zero_offset,
                   float distance_scale, float maximum_distance,
                   const BoundingBox &aabb, const float floor_y,
                   const TriangleOctree *octree, uint32_t n_octree_levels,
                   cudaStream_t stream);
  ...
  class NerfTracer {
   public:
    ...
    void init_rays_from_camera(
        uint32_t spp, uint32_t padded_output_width, uint32_t n_extra_dims,
        const Eigen::Vector2i &resolution, const Eigen::Vector2f &focal_length,
        const Eigen::Matrix<float, 3, 4> &camera_matrix0,
        const Eigen::Matrix<float, 3, 4> &camera_matrix1,
        const Eigen::Vector4f &rolling_shutter, Eigen::Vector2f screen_center,
        Eigen::Vector3f parallax_shift, bool snap_to_pixel_centers,
        const BoundingBox &render_aabb, float plane_z, float dof,
        const CameraDistortion &camera_distortion, const float *envmap_data,
        const Eigen::Vector2i &envmap_resolution, const float *distortion_data,
        const Eigen::Vector2i &distortion_resolution,
        Eigen::Array4f *frame_buffer, float *depth_buffer, uint8_t *grid,
        int show_accel, float cone_angle_constant, ERenderMode render_mode,
        cudaStream_t stream, const tcnn::network_precision_t *rotation,
        const tcnn::network_precision_t *transition,
        const Eigen::Vector3f first_frame_offset);

    uint32_t trace(
        NerfNetwork<precision_t> &network, const BoundingBox &render_aabb,
        ...
        const uint8_t *grid, ERenderMode render_mode,
        const Eigen::Matrix<float, 3, 4> &camera_matrix, float depth_scale,
        int visualized_layer, int visualized_dim,
        ENerfActivation rgb_activation, ENerfActivation density_activation,
        int show_accel, float min_transmittance, float glow_y_cutoff,
        int glow_mode, const float *extra_dims_gpu, cudaStream_t stream);
  ...
  class FiniteDifferenceNormalsApproximator {
   public:
    ...
    void normal(uint32_t n_elements, const distance_fun_t &distance_function,
                tcnn::GPUMemory<Eigen::Vector3f> &pos,
                tcnn::GPUMemory<Eigen::Vector3f> &normal, float epsilon,
                cudaStream_t stream);
  ...
  // static constexpr float LOSS_SCALE = 1.0f;
  // static constexpr float LOSS_SCALE = 0.01f;
  // static constexpr float LOSS_SCALE = 0.001f;
  ...
  void render_volume(CudaRenderBuffer &render_buffer,
                     const Eigen::Vector2f &focal_length,
                     const Eigen::Matrix<float, 3, 4> &camera_matrix,
                     const Eigen::Vector2f &screen_center, cudaStream_t stream);
  void train_volume(size_t target_batch_size, bool get_loss_scalar,
                    cudaStream_t stream);
  void training_prep_volume(uint32_t batch_size, cudaStream_t stream) {}
  void load_volume();

  void render_sdf(const distance_fun_t &distance_function,
  ...
  void render_nerf(CudaRenderBuffer &render_buffer,
                   const Eigen::Vector2i &max_res,
                   const Eigen::Vector2f &focal_length,
                   const Eigen::Matrix<float, 3, 4> &camera_matrix0,
                   const Eigen::Matrix<float, 3, 4> &camera_matrix1,
  ...
  void render_frame(const Eigen::Matrix<float, 3, 4> &camera_matrix0,
                    const Eigen::Matrix<float, 3, 4> &camera_matrix1,
                    const Eigen::Vector4f &nerf_rolling_shutter,
                    CudaRenderBuffer &render_buffer, bool to_srgb = true);
  void visualize_nerf_cameras(ImDrawList *list,
  ...
  void reload_network_from_file(
      const std::string &network_config_path,
      const std::string &network_output_path = "output");
  void reload_network_from_json(
      const nlohmann::json &json,
      const std::string &config_base_path =
          "");  // config_base_path is needed so that if the passed in json uses
                // the 'parent' feature, we know where to look... be sure to use
                // a filename, or if a directory, end with a trailing slash
  void reset_accumulation(bool due_to_camera_movement = false,
                          bool immediate_redraw = true);
  ...
  void create_empty_nerf_dataset(size_t n_images, int aabb_scale = 1,
                                 bool is_hdr = false);
  void load_nerf();
  void load_nerf(uint32_t frame_time_idx, bool is_downsample = false);
  void load_mesh();
  ...
  void reset_camera();
  bool keyboard_event();
  void generate_training_samples_sdf(Eigen::Vector3f *positions,
                                     float *distances, uint32_t n_to_generate,
                                     cudaStream_t stream, bool uniform_only);
  void update_density_grid_nerf(float decay,
  ...
  void train_nerf(uint32_t target_batch_size, bool get_loss_scalar,
                  cudaStream_t stream);
  void train_nerf_step(uint32_t target_batch_size, uint32_t n_rays_per_batch,
                       uint32_t *counter, uint32_t *compacted_counter,
                       float *loss, float *ek_loss, float *mask_loss,
                       cudaStream_t stream);
  void train_sdf(size_t target_batch_size, bool get_loss_scalar,
                 cudaStream_t stream);
  void train_image(size_t target_batch_size, bool get_loss_scalar,
                   cudaStream_t stream);
  ...
  Eigen::Vector2f calc_focal_length(const Eigen::Vector2i &resolution,
                                    int fov_axis, float zoom) const;
  Eigen::Vector2f render_screen_center() const;
  void optimise_mesh_step(uint32_t N_STEPS);
  void compute_mesh_vertex_colors();
  tcnn::GPUMemory<float> get_density_on_grid(
      Eigen::Vector3i res3d,
      const BoundingBox &aabb);  // network version (nerf or sdf)
  tcnn::GPUMemory<float> get_sdf_gt_on_grid(
      Eigen::Vector3i res3d,
      const BoundingBox &aabb);  // sdf gt version (sdf only)
  tcnn::GPUMemory<Eigen::Array4f> get_rgba_on_grid(Eigen::Vector3i res3d,
  ...
  void determine_autofocus_target_from_pixel(
      const Eigen::Vector2i &focus_pixel);
  void autofocus();
  size_t n_params();
  ...
  pybind11::dict compute_marching_cubes_pc(
      ...
      BoundingBox aabb = BoundingBox{Eigen::Vector3f::Zero(),
                                     Eigen::Vector3f::Ones()},
      float thresh = 2.5f);
  pybind11::array_t<float> render_to_cpu(int width, int height, int spp,
                                         bool linear, float start_t,
                                         float end_t, float fps,
                                         float shutter_fraction);
  pybind11::array_t<float> my_render_to_cpu(int width, int height, int spp,
                                            bool linear, float start_t,
                                            float end_t, float fps,
                                            float shutter_fraction);
  pybind11::array_t<float> render_with_rolling_shutter_to_cpu(
      const Eigen::Matrix<float, 3, 4> &camera_transform_start,
      const Eigen::Matrix<float, 3, 4> &camera_transform_end,
      const Eigen::Vector4f &rolling_shutter, int width, int height, int spp,
      bool linear);
  pybind11::array_t<float> screenshot(bool linear) const;
  void override_sdf_training_data(pybind11::array_t<float> points,
                                  pybind11::array_t<float> distances);
#endif

  double calculate_iou(uint32_t n_samples = 128 * 1024 * 1024,
                       float scale_existing_results_factor = 0.0,
                       bool blocking = true, bool force_use_octree = true);
  void draw_visualizations(ImDrawList *list,
                           const Eigen::Matrix<float, 3, 4> &camera_matrix);
  ...
  void save_snapshot_incremental(const std::string &filepath_string,
                                 bool include_optimizer_state);
  void load_snapshot(const std::string &filepath_string);
  void load_snapshot_incremental(const std::string &filepath_string);
  ...
  void compute_and_save_marching_cubes_mesh(
      const char *filename,
      Eigen::Vector3i res3d = Eigen::Vector3i::Constant(128),
      BoundingBox aabb = {}, float thresh = 2.5f, bool unwrap_it = false);
  Eigen::Vector3i compute_and_save_png_slices(const char *filename, int res,
                                              BoundingBox aabb = {},
                                              float thresh = 2.5f,
                                              float density_range = 4.f,
                                              bool flip_y_and_z_axes = false);
  ...
  // dynamic scene
  bool m_training_to_next_frame = false;
  int current_training_time_frame = 0;
  uint32_t all_training_time_frame = 1u;

  bool m_incremental_reinit_sdf_mlp = false;
  int m_incremental_reinit_sdf_mlp_iters = 10;
  ...
  // loss weight
  float m_mask_loss_weight = 0.f;
  float m_ek_loss_weight = 0.01f;

  std::vector<filesystem::path> all_json_paths;
  std::FILE *log_fp;
  ...
  Eigen::Vector2f m_focal_length = Eigen::Vector2f::Ones();
  uint32_t m_fov_axis = 1;
  float m_zoom = 1.f;  // 2d zoom factor (for insets?)
  Eigen::Vector2f m_screen_center =
  ...
  struct Nerf {
    ...
    struct Training {
      ...
      int n_images_for_training =
          0;  // how many images to train from, as a high watermark compared to
              // the dataset size
      ...
      AdamOptimizer<Eigen::Vector2f> cam_focal_length_offset =
          AdamOptimizer<Eigen::Vector2f>(0.f);

      tcnn::GPUMemory<float>
          extra_dims_gpu;  // if the model demands a latent code per training
                           // image, we put them in here.
      ...
      struct Counters {
        ...
        float update_after_training(uint32_t target_batch_size,
                                    bool get_loss_scalar, cudaStream_t stream);
      };
      ...
      void set_camera_intrinsics(int frame_idx, float fx, float fy = 0.0f,
      ...
      void set_image(int frame_idx, pybind11::array_t<float> img,
                     pybind11::array_t<float> depth_img, float depth_scale);
#endif

      void reset_camera_extrinsics();
      void export_camera_extrinsics(
          const std::string &filename,
          bool export_extrinsics_in_quat_format = true);

    } training = {};

    tcnn::GPUMemory<float> density_grid;  // NERF_GRIDSIZE()^3 grid of EMA
                                          // smoothed densities from the network
    ...
    uint32_t extra_dim_idx_for_inference =
        0;  // which training image's latent code should be presented at
            // inference time
  ...
  struct Sdf {
    ...
    int octree_depth_target =
        0;  // we duplicate this state so that you can waggle the slider without
    ...
    bool brick_smooth_normals = false;  // if true, then we space the central
                                        // difference taps by one voxel
  ...
  inline float get_floor_y() const {
    return m_floor_enable ? m_aabb.min.y() + 0.001f : -10000.f;
  }
  ...
  // The frame contains everything, i.e. training + rendering + GUI and buffer
  // swapping
  ...
  bool m_single_view =
      true;  // Whether a single neuron is visualized, or all in a tiled grid
  float m_picture_in_picture_res =
      0.f;  // if non zero, requests a small second picture :)

  bool m_imgui_enabled = true;  // tab to toggle
  bool m_visualize_unit_cube = false;
  bool m_snap_to_pixel_centers = false;

  Eigen::Vector2f m_parallax_shift = {
      0.f, 0.f};  // to shift the viewer's head position by some amount parallel
                  // to the screen
  Eigen::Vector3f get_scaled_parallax_shift() const {
    return {m_parallax_shift.x(), m_parallax_shift.y(), m_scale};
  }  // pack m_scale into the parallax parameter so we know where the screen
     // plane is.
  ...
  uint32_t m_top_grid_resolution;
  ...
  Ema m_mask_loss_scalar = {EEmaType::Time, 100};
  ...
  struct TrainableGlobalMove {
    std::shared_ptr<tcnn::Optimizer<precision_t>> optimizer;
    std::shared_ptr<DeltaNetwork<precision_t>> delta_network;
    // std::shared_ptr<tcnn::DifferentiableObject<float, precision_t, float>>
    // delta_network;
    std::shared_ptr<tcnn::Trainer<float, precision_t, float>> trainer;
