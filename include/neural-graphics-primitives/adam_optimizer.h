...
class AdamOptimizer {
 public:
  AdamOptimizer(float learning_rate, const T &zero = T::Zero(),
                float epsilon = 1e-08f, float beta1 = 0.9f, float beta2 = 0.99f)
      : m_state(zero) {
  ...
  void step(const T &gradient) {
    ...
    m_state.variable -=
        ...
        m_state.first_moment.cwiseQuotient(
            ...
            T::Constant(m_state.variable.size(), m_hparams.epsilon));
  }

  uint32_t step() const { return m_state.iter; }

  void set_learning_rate(float lr) { m_hparams.learning_rate = lr; }

  T &variable() { return m_state.variable; }

  const T &variable() const { return m_state.variable; }

  void reset_state(const T &zero = T::Zero()) { m_state = State(zero); }
...
class RotationAdamOptimizer {
 public:
  RotationAdamOptimizer(float learning_rate, float epsilon = 1e-08f,
                        float beta1 = 0.9f, float beta2 = 0.99f) {
    m_hparams = {learning_rate, epsilon, beta1, beta2};
  }
  ...
  RotationAdamOptimizer(const RotationAdamOptimizer &arg) { *this = arg; }
  ...
  void step(const Eigen::Vector3f &gradient) {
    ...
    Eigen::Matrix3f mat =
        Eigen::AngleAxisf(-rot_len, rot_len > 0 ? rot / rot_len : Z)
            .toRotationMatrix() *
        Eigen::AngleAxisf(var_len, var_len > 0 ? variable() / var_len : Z)
            .toRotationMatrix();
  ...
  const Eigen::Vector3f &variable() const { return m_state.variable; }

  void reset_state() { m_state = {}; }
