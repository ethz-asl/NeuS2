...
struct hash<ngp::Vector4i16> {
  size_t operator()(const ngp::Vector4i16 &x) const {
    return (size_t)x.x() * 73856093 + (size_t)x.y() * 19349663 +
           (size_t)x.z() * 83492791 + (size_t)x.w() * 25165843;
  }
};
}  // namespace std
...
inline void write_brick_voxel_positions(Eigen::Vector3f *dst, uint32_t B,
                                        float size,
...
class TriangleOctree {
 public:
  std::vector<Eigen::Vector3f> build_brick_voxel_position_list(
      uint32_t B) const {  // brick size B^3
    std::vector<Eigen::Vector3f> brick_pos;
    brick_pos.resize(m_dual_nodes.size() * B * B * B);
    float rstep = 1.f / (B - 1);
    write_brick_voxel_positions(brick_pos.data(), B, 1.f,
                                Eigen::Vector3f::Zero());
    ...
    for (ni = 0; ni < (uint32_t)m_nodes.size(); ++ni) {
      ...
      if (n.depth > last_level) {
        tlog::info() << (ni - prev_ni) << " bricks at level " << last_level
                     << " with " << (ni - prev_ni) * B * B * B << " values.";
        last_level = n.depth;
        prev_ni = ni;
      }
      ...
      for (int i = 0; i < 8; ++i) {
        ...
        write_brick_voxel_positions(brick_pos.data() + (child_idx * B * B * B),
                                    B, child_size, base_pos);
      }
    }
    tlog::info() << (ni - prev_ni) << " bricks at level " << last_level
                 << " with " << (ni - prev_ni) * B * B * B << " values.";
    tlog::info() << (m_dual_nodes.size() - ni) << " bricks at level "
                 << last_level + 1 << " with "
                 << (m_dual_nodes.size() - ni) * B * B * B << " values.";
    return brick_pos;
  }
  ...
  void build(const TriangleBvh &bvh, const std::vector<Triangle> &triangles,
    ...
    // Only generate nodes up to max_depth-1! The dual nodes will truly reach to
    // the max depth
    for (uint8_t depth = 0; depth < max_depth - 1; ++depth) {
      int n_pending_nodes_to_build = node_counter - n_nodes;

      // Reserve enough memory as if the next layer was dense. This allows
      // parallelizing the following code
    ...
    auto generate_dual_coords = [&](TriangleOctreeDualNode &dual_node,
                                    int depth, const Vector3i16 pos) {
      for (uint32_t i = 0; i < 8; ++i) {
        Vector4i16 coord = {(uint16_t)pos.x(), (uint16_t)pos.y(),
  ...
  __device__ static uint8_t traverse(const TriangleOctreeNode *nodes,
                                     ...
                                     int max_depth, Eigen::Vector3f pos,
                                     F fun) {
    int node_idx = 0;
...
#pragma unroll
  ...
  __device__ static float ray_intersect(const TriangleOctreeNode *nodes,
                                        int max_depth,
                                        const Eigen::Vector3f &ro,
    ...
    // Ensure that closer children are checked last such that they rise to the
    // top of the stack
