...
class DeltaNetwork : public tcnn::DifferentiableObject<float, T, float> {
 public:
  DeltaNetwork(uint32_t n_pos_dims = 6u) : m_n_pos_dims{n_pos_dims} {
    m_transition = std::make_shared<TrainableBuffer<1, 1, T>>(
        Eigen::Matrix<int, 1, 1>{(int)4});

#if rotation_reprensentation
    m_rotation = std::make_shared<TrainableBuffer<1, 1, T>>(
        Eigen::Matrix<int, 1, 1>{(int)8});
#else
    m_rotation = std::make_shared<TrainableBuffer<1, 1, T>>(
        Eigen::Matrix<int, 1, 1>{(int)4});
#endif
  ...
  void inference_mixed_precision_impl(
      ...
      tcnn::GPUMatrixDynamic<float> &output, bool use_inference_params = true) {
  ...
  std::unique_ptr<tcnn::Context> forward_impl(
      cudaStream_t stream, const tcnn::GPUMatrixDynamic<float> &input,
      tcnn::GPUMatrixDynamic<float> *output = nullptr,
      bool use_inference_params = false, bool prepare_input_gradients = false) {
    uint32_t batch_size = input.n();

    auto forward = std::make_unique<ForwardContext>();
#if rotation_reprensentation
    tcnn::linear_kernel(add_global_movement_with_rotation_6d<float, T>, 0,
                        stream, batch_size, m_rotation->params(),
                        m_transition->params(), first_frame_offset,
                        input.view(), output->view());
#else
    printf("some error not implemented! 1\n");
    tcnn::linear_kernel(add_global_movement_with_rotation_quaternion<float, T>,
                        0, stream, batch_size, m_rotation->params(),
  ...
  void backward_impl(cudaStream_t stream, const tcnn::Context &ctx,
...
#if rotation_reprensentation
    rotation_dof = 6;
    dL_drotation_quat_each =
        tcnn::GPUMatrixDynamic<T>{rotation_dof, batch_size, stream};

    tcnn::linear_kernel(add_loss_to_rotation_6d_each<T, float>, 0, stream,
                        batch_size, dL_doutput.view(), input.view(),
                        m_rotation->params(), m_transition->params(),
                        first_frame_offset, dL_drotation_quat_each.view(),
                        dL_dtransition_each.view(), dL_dinput_each.view());
    ...
    tcnn::linear_kernel(add_loss_to_rotation_quaternion_each<T, float>, 0,
                        stream, batch_size, dL_doutput.view(), input.view(),
                        m_rotation->params(), dL_drotation_quat_each.view(),
                        dL_dinput_each.view());
    ...
    if (param_gradients_mode != tcnn::EGradientMode::Ignore) {
      ...
      CUDA_CHECK_THROW(cudaMemsetAsync(m_rotation->gradients(), 0,
                                       sizeof(T) * m_rotation->n_params(),
                                       stream));
      ...
      for (uint32_t i = 0; i < rotation_dof; i++) {
        ...
        tcnn::linear_kernel(add_rotation_view_to_loss<T, T>, 0, stream,
                            batch_size, dL_drotation_quat_each.view(), i,
                            dL_drotation.view());
        gradients = tcnn::reduce_sum(dL_drotation.data(), batch_size, stream);

        cudaMemcpy(m_rotation->gradients() + i, &gradients, sizeof(T),
                   cudaMemcpyHostToDevice);  // 将数据从CPU传递到GPU
      }

      CUDA_CHECK_THROW(cudaMemsetAsync(m_transition->gradients(), 0,
                                       sizeof(T) * m_transition->n_params(),
                                       stream));
      ...
      for (uint32_t i = 0; i < 3; i++) {
        ...
        tcnn::linear_kernel(add_transition_view_to_loss<T, T>, 0, stream,
                            batch_size, dL_dtransition_each.view(), i,
                            dL_dtransition.view());
        // printf("dL_dvariance
        ...
        cudaMemcpy(m_transition->gradients() + i, &gradients, sizeof(T),
                   cudaMemcpyHostToDevice);  // 将数据从CPU传递到GPU
      }
    }
    ...
    if (dL_dinput != nullptr) {
      ...
      tcnn::linear_kernel(fill_positions_view<float, T>, 0, stream,
                          batch_size * 3u, 3u, dL_dinput_each.view(),
                          dL_dinput->view());
      // tcnn::linear_kernel(fill_positions_view<float, float>, 0, stream,
      // 			batch_size * 3u, 3u, dL_doutput.view(),
      // dL_dinput->view());
    }
  }
  ...
  void set_params(T *params, T *inference_params, T *backward_params,
    ...
    m_transition->set_params(params + offset, inference_params + offset,
                             backward_params + offset, gradients + offset);
    offset += m_transition->n_params();

    m_rotation->set_params(params + offset, inference_params + offset,
                           backward_params + offset, gradients + offset);
  ...
  void initialize_params(tcnn::pcg32 &rnd, float *params_full_precision,
                         ...
                         T *gradients, float scale = 1) {
    size_t offset = 0;

    m_transition->initialize_params(rnd, params_full_precision + offset,
    ...
    tcnn::generate_random_uniform<float>(m_rng, transition_n_params,
                                         params_full_precision + offset, 0.000f,
                                         0.000f);

    offset += transition_n_params;
    printf("transition_n_params:%d\n", transition_n_params);  // 1

    m_rotation->initialize_params(rnd, params_full_precision + offset,
                                  params + offset, inference_params + offset,
                                  backward_params + offset, gradients + offset,
                                  scale);
    ...
    tcnn::generate_random_uniform<float>(
        m_rng, 1u, params_full_precision + offset, 1.000f, 1.000f);
    tcnn::generate_random_uniform<float>(
        m_rng, 3u, params_full_precision + offset + 1u, 0.000f, 0.000f);
    tcnn::generate_random_uniform<float>(
        m_rng, 1u, params_full_precision + offset + 4u, 1.000f, 1.000f);
    tcnn::generate_random_uniform<float>(
        m_rng, 1u, params_full_precision + offset + 5u, 0.000f, 0.000f);

#else
    // unit quat (0,0,0,1)
    tcnn::generate_random_uniform<float>(
        m_rng, 3u, params_full_precision + offset, 0.000f, 0.000f);
    tcnn::generate_random_uniform<float>(
        m_rng, 1u, params_full_precision + offset + 3u, 1.000f, 1.000f);
#endif
  ...
  uint32_t input_width() const { return m_n_pos_dims; }

  uint32_t output_width() const { return 6; }

  uint32_t n_extra_dims() const { return 0u; }
  ...
  std::pair<const T *, tcnn::MatrixLayout> forward_activations(
      const tcnn::Context &ctx, uint32_t layer) const {
