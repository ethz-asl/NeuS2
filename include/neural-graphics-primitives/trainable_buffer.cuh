 ...
 *  @brief  An implementation of a trainable N-channel buffer within the tcnn
 * API.
 */
...
class TrainableBuffer : public tcnn::DifferentiableObject<float, T, T> {
 ...
 public:
  ...
  // tcnn::EGradientMode::Overwrite ) override { 	printf("cal_dy override
  // succeed!\n"); 	return;
  // 	// kernel_cal_dy()
  // }
  ...
  std::unique_ptr<tcnn::Context> forward_impl(
    ...
    throw std::runtime_error{
        "The trainable buffer does not support forward(). Its content is meant "
  ...
  void initialize_params(tcnn::pcg32 &rnd, float *params_full_precision,
    ...
    CUDA_CHECK_THROW(
        cudaMemset(params_full_precision, 0, n_params() * sizeof(float)));
  }

  size_t n_params() const override { return m_resolution.prod() * N_DIMS; }

  uint32_t input_width() const override { return RANK; }

  uint32_t padded_output_width() const override { return N_DIMS; }

  uint32_t output_width() const override { return N_DIMS; }
  ...
  T *gradients() const { return m_params_gradient; }

  T *gradient_weights() const { return m_params_gradient_weight.data(); }

  T *params() const { return m_params; }

  T *params_inference() const { return m_params_inference; }
