 ...
 *          real time. Such primitives include images, signed distance
 * functions, and volumetric density/radiance representations (like NeRF).
 */
...
inline __host__ __device__ Eigen::Vector3f cylindrical_to_dir(
...
inline __host__ __device__ Eigen::Vector2f dir_to_cylindrical(
...
inline __host__ __device__ Eigen::Vector2f dir_to_spherical_unorm(
    const Eigen::Vector3f &d) {
  const float cos_theta = fminf(fmaxf(d.z(), -1.0f), 1.0f);
...
__device__ __host__ Eigen::Vector3f fibonacci_dir(
    uint32_t i, const Eigen::Vector2f &offset) {
...
inline __host__ __device__ Eigen::Vector2f square2disk_shirley(
    const Eigen::Vector2f &square) {
  float phi, r;
  float a = square.x();
  float b = square.y();
...
inline __host__ __device__ __device__ Eigen::Vector3f cosine_hemisphere(
...
// https://www.jcgt.org/published/0009/04/01/paper.pdf
...
inline __host__ __device__ uint32_t laine_karras_permutation(uint32_t x,
                                                             uint32_t seed) {
  x += seed;
  x ^= x * 0x6c50b47cu;
  x ^= x * 0xb82f1e52u;
...
nested_uniform_scramble_base2(uint32_t x, uint32_t seed) {
  x = reverse_bits(x);
...
inline __host__ __device__ Vector4i32
shuffled_scrambled_sobol4d(uint32_t index, uint32_t seed) {
...
inline __host__ __device__ Vector2i32
shuffled_scrambled_sobol2d(uint32_t index, uint32_t seed) {
  index = nested_uniform_scramble_base2(index, seed);
...
inline __host__ __device__ Eigen::Vector4f ld_random_val_4d(uint32_t index,
                                                            uint32_t seed) {
  constexpr float S = float(1.0 / (1ull << 32));
  Vector4i32 x = shuffled_scrambled_sobol4d(index, seed);
  return {(float)x.x() * S, (float)x.y() * S, (float)x.z() * S,
          (float)x.w() * S};
}

inline __host__ __device__ Eigen::Vector2f ld_random_val_2d(uint32_t index,
                                                            uint32_t seed) {
  constexpr float S = float(1.0 / (1ull << 32));
...
inline __host__ __device__ float ld_random_val(uint32_t index, uint32_t seed,
                                               uint32_t dim = 0) {
  constexpr float S = float(1.0 / (1ull << 32));
  index = nested_uniform_scramble_base2(index, seed);
  return (float)nested_uniform_scramble_base2(sobol(index, dim),
...
// Halton
// inline __host__ __device__ Eigen::Vector2f ld_random_pixel_offset(const
// uint32_t spp) { 	Eigen::Vector2f offset = Eigen::Vector2f::Constant(0.5f) -
// halton23(0) + halton23(spp); 	offset.x() = fractf(offset.x()); 	offset.y() =
// fractf(offset.y()); 	return offset;
// }

// Scrambled Sobol
inline __host__ __device__ Eigen::Vector2f ld_random_pixel_offset(
    const uint32_t spp) {
