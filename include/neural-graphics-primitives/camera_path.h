...
struct CameraKeyframe {
  ...
  CameraKeyframe(const Eigen::Vector4f &r, const Eigen::Vector3f &t, float sl,
                 float sc, float fv, float df)
      : R(r), T(t), slice(sl), scale(sc), fov(fv), dof(df) {}
  CameraKeyframe(Eigen::Matrix<float, 3, 4> m, float sl, float sc, float fv,
                 float df)
      : slice(sl), scale(sc), fov(fv), dof(df) {
    T = m.col(3);
    R = Eigen::Quaternionf(m.block<3, 3>(0, 0)).coeffs();
  }
  CameraKeyframe operator*(float f) const {
  ...
  CameraKeyframe operator+(const CameraKeyframe &rhs) const {
    ...
    return {R + Rr,        T + rhs.T,    slice + rhs.slice, scale + rhs.scale,
            fov + rhs.fov, dof + rhs.dof};
  }
...
struct CameraPath {
  ...
  CameraKeyframe eval_camera_path(float t) {
    ...
    int t1 = (int)floorf(t);
    return spline(t - floorf(t), get_keyframe(t1 - 1), get_keyframe(t1),
                  get_keyframe(t1 + 1), get_keyframe(t1 + 2));
  }

  void save(const std::string &filepath_string);
  void load(const std::string &filepath_string,
  ...
  int imgui(char path_filename_buf[128], float frame_milliseconds,
            Eigen::Matrix<float, 3, 4> &camera, float slice_plane_z,
            float scale, float fov, float dof, float bounding_radius,
            const Eigen::Matrix<float, 3, 4> &first_xform);
  bool imgui_viz(ImDrawList *list, Eigen::Matrix<float, 4, 4> &view2proj,
                 Eigen::Matrix<float, 4, 4> &world2proj,
                 Eigen::Matrix<float, 4, 4> &world2view, Eigen::Vector2f focal,
                 float aspect);
#endif
};

#ifdef NGP_GUI
void add_debug_line(ImDrawList *list, const Eigen::Matrix<float, 4, 4> &proj,
                    Eigen::Vector3f a, Eigen::Vector3f b,
                    uint32_t col = 0xffffffff, float thickness = 1.f);
...
void visualize_nerf_camera(ImDrawList *list,
                           ...
                           float aspect, uint32_t col = 0x80ffffff);
#endif
