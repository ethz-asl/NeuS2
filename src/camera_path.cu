...
CameraKeyframe lerp(const CameraKeyframe &p0, const CameraKeyframe &p1, float t,
                    float t0, float t1) {
  t = (t - t0) / (t1 - t0);
...
CameraKeyframe spline(float t, const CameraKeyframe &p0,
                      const CameraKeyframe &p1, const CameraKeyframe &p2,
...
void to_json(json &j, const CameraKeyframe &p) {
  j = json{{"R", p.R},         {"T", p.T},     {"slice", p.slice},
           {"scale", p.scale}, {"fov", p.fov}, {"dof", p.dof}};
}
...
void from_json(bool is_first, const json &j, CameraKeyframe &p,
               const CameraKeyframe &first,
               const Eigen::Matrix<float, 3, 4> &ref) {
...
void CameraPath::save(const std::string &filepath_string) {
  json j = {{"time", m_playtime}, {"path", m_keyframes}};
  std::ofstream f(filepath_string);
  f << j;
}

void CameraPath::load(const std::string &filepath_string,
...
#ifdef NGP_GUI
int CameraPath::imgui(char path_filename_buf[128], float frame_milliseconds,
                      Matrix<float, 3, 4> &camera, float slice_plane_z,
                      float scale, float fov, float dof, float bounding_radius,
                      const Eigen::Matrix<float, 3, 4> &first_xform) {
  ...
  if (!m_keyframes.empty()) {
    ...
    ImGui::SameLine();
    if (ImGui::Button("<")) {
      m_playtime = n ? std::max(0.f, floorf((m_playtime - 0.0001f) * (float)n) /
                                         (float)n)
                     : 0.f;
      read = 2;
    }
    ImGui::SameLine();
    if (ImGui::Button(m_update_cam_from_path ? "STOP" : "READ")) {
    ...
    if (ImGui::Button("Dup")) {
      m_update_cam_from_path = false;
      m_keyframes.insert(m_keyframes.begin() + i, m_keyframes[i]);
      m_playtime = i / float(n + 1);
      read = 2;
    }
    ImGui::SameLine();
    if (ImGui::Button("Del")) {
  ...
  if (ImGui::BeginPopupModal("Camera path load error", NULL,
  ...
  if (!m_keyframes.empty()) {
    ImGui::SameLine();
    if (ImGui::Button("Save")) save(path_filename_buf);
  }
  if (!m_keyframes.empty()) {
    int i = (int)round(m_playtime * (float)n);
    ...
    if (ImGui::SliderFloat("slice Z", &m_keyframes[i].slice, -bounding_radius,
                           bounding_radius))
      read = 2;
...
void add_debug_line(ImDrawList *list, const Matrix<float, 4, 4> &proj,
                    Vector3f a, Vector3f b, uint32_t col, float thickness) {
  ImVec2 aa, bb;
...
void visualize_unit_cube(ImDrawList *list,
                         const Matrix<float, 4, 4> &world2proj) {
  add_debug_line(list, world2proj, Vector3f{0.f, 0.f, 0.f},
                 Vector3f{0.f, 0.f, 1.f}, 0xffff4040);  // Z
  add_debug_line(list, world2proj, Vector3f{1.f, 0.f, 0.f},
                 Vector3f{1.f, 0.f, 1.f}, 0xffffffff);
  add_debug_line(list, world2proj, Vector3f{0.f, 1.f, 0.f},
                 Vector3f{0.f, 1.f, 1.f}, 0xffffffff);
  add_debug_line(list, world2proj, Vector3f{1.f, 1.f, 0.f},
                 Vector3f{1.f, 1.f, 1.f}, 0xffffffff);

  add_debug_line(list, world2proj, Vector3f{0.f, 0.f, 0.f},
                 Vector3f{1.f, 0.f, 0.f}, 0xff4040ff);  // X
  add_debug_line(list, world2proj, Vector3f{0.f, 1.f, 0.f},
                 Vector3f{1.f, 1.f, 0.f}, 0xffffffff);
  add_debug_line(list, world2proj, Vector3f{0.f, 0.f, 1.f},
                 Vector3f{1.f, 0.f, 1.f}, 0xffffffff);
  add_debug_line(list, world2proj, Vector3f{0.f, 1.f, 1.f},
                 Vector3f{1.f, 1.f, 1.f}, 0xffffffff);

  add_debug_line(list, world2proj, Vector3f{0.f, 0.f, 0.f},
                 Vector3f{0.f, 1.f, 0.f}, 0xff40ff40);  // Y
  add_debug_line(list, world2proj, Vector3f{1.f, 0.f, 0.f},
                 Vector3f{1.f, 1.f, 0.f}, 0xffffffff);
  add_debug_line(list, world2proj, Vector3f{0.f, 0.f, 1.f},
                 Vector3f{0.f, 1.f, 1.f}, 0xffffffff);
  add_debug_line(list, world2proj, Vector3f{1.f, 0.f, 1.f},
                 Vector3f{1.f, 1.f, 1.f}, 0xffffffff);
}

void visualize_nerf_camera(ImDrawList *list,
                           const Matrix<float, 4, 4> &world2proj,
                           const Eigen::Matrix<float, 3, 4> &xform,
                           float aspect, uint32_t col) {
  const float axis_size = 0.025f;
  const Vector3f *xforms = (const Vector3f *)&xform;
  Vector3f pos = xforms[3];
  add_debug_line(list, world2proj, pos, pos + axis_size * xforms[0],
                 0xff4040ff);
  add_debug_line(list, world2proj, pos, pos + axis_size * xforms[1],
                 0xff40ff40);
  add_debug_line(list, world2proj, pos, pos + axis_size * xforms[2],
                 0xffff4040);
...
bool CameraPath::imgui_viz(ImDrawList *list, Matrix<float, 4, 4> &view2proj,
                           Matrix<float, 4, 4> &world2proj,
                           Matrix<float, 4, 4> &world2view, Vector2f focal,
                           float aspect) {
  bool changed = false;
  float flx = focal.x();
  float fly = focal.y();
  ...
  if (!m_update_cam_from_path) {
    ...
    for (int i = 0; i < m_keyframes.size(); ++i) {
      visualize_nerf_camera(list, world2proj, m_keyframes[i].m(), aspect,
                            (i == cur_cam_i) ? 0xff80c0ff : 0x8080c0ff);
    ...
    if (!m_keyframes.empty()) {
      ...
      if (ImGuizmo::Manipulate(
              (const float *)&world2view, (const float *)&view2proj_guizmo,
              (ImGuizmo::OPERATION)m_gizmo_op, (ImGuizmo::MODE)m_gizmo_mode,
              (float *)&matrix, NULL, NULL)) {
        int i0 = cur_cam_i;
        while (i0 > 0 && m_keyframes[cur_cam_i].SamePosAs(m_keyframes[i0 - 1]))
          i0--;
        int i1 = cur_cam_i;
        while (i1 < m_keyframes.size() - 1 &&
               m_keyframes[cur_cam_i].SamePosAs(m_keyframes[i1 + 1]))
          i1++;
        for (int i = i0; i <= i1; ++i) {
          m_keyframes[i].T = matrix.block<3, 4>(0, 0).col(3);
          m_keyframes[i].R =
              Eigen::Quaternionf(matrix.block<3, 3>(0, 0)).coeffs();
        }
        changed = true;
      }

      visualize_nerf_camera(list, world2proj, eval_camera_path(m_playtime).m(),
                            aspect, 0xff80ff80);
