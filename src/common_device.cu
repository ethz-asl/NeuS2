...
Matrix<float, 3, 4> log_space_lerp(const Matrix<float, 3, 4> &begin,
                                   const Matrix<float, 3, 4> &end, float t) {
...
GPUMemory<float> load_exr(const std::string &filename, int &width,
                          int &height) {
...
GPUMemory<float> load_stbi(const std::string &filename, int &width,
                           int &height) {
  ...
  if (is_hdr) {
    ...
    linear_kernel(from_rgba32<float>, 0, nullptr, width * height, bytes.data(),
                  result.data(), false, false, 0);
  }
