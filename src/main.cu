...
int main(int argc, char **argv) {
  ...
  ValueFlag<string> mode_flag{
      parser,
      "MODE",
      "Mode can be 'nerf', 'sdf', 'image', or 'volume'. Inferred from the "
  ...
  try {
    ...
    if (!mode_flag) {
      ...
      if (scene_path.is_directory() ||
          equals_case_insensitive(scene_path.extension(), "json")) {
        mode = ETestbedMode::Nerf;
      } else if (equals_case_insensitive(scene_path.extension(), "obj") ||
                 equals_case_insensitive(scene_path.extension(), "stl")) {
    ...
    if (snapshot_flag) {
      ...
      if (!network_config_path.exists()) {
        tlog::error() << "Network config path " << network_config_path
                      << " does not exist.";
        return 1;
      }
    ...
    while (testbed.frame()) {
      if (!gui) {
        ...
        // << testbed.m_loss_scalar.val() << " lr=" <<
        // testbed.m_optimizer.learning_rate();
      }
    }
