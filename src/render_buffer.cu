...
void CudaSurface2D::resize(const Vector2i &size) {
  ...
  CUDA_CHECK_THROW(cudaMallocArray(&m_array, &desc, size.x(), size.y(),
...
void GLTexture::blit_from_cuda_mapping() {
  ...
  if (m_internal_format != GL_RGBA32F || m_format != GL_RGBA || m_is_8bit) {
    throw std::runtime_error{
        "Can only blit from CUDA mapping if the texture is RGBA float."};
  }

  const float *data_cpu = m_cuda_mapping->data_cpu();
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, m_size.x(), m_size.y(), 0, GL_RGBA,
               GL_FLOAT, data_cpu);
}
...
void GLTexture::load(const float *data, Vector2i new_size, int n_channels) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, m_internal_format, new_size.x(), new_size.y(),
               0, m_format, GL_FLOAT, data);
}
...
void GLTexture::resize(const Vector2i &new_size, int n_channels, bool is_8bit) {
  ...
  switch (n_channels) {
    ...
    case 4:
      ...
      break;
    default:
      tlog::error() << "Unsupported number of channels: " << n_channels;
  }
  m_is_8bit = is_8bit;
  m_size = new_size;
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, m_internal_format, new_size.x(), new_size.y(),
               0, m_format, is_8bit ? GL_UNSIGNED_BYTE : GL_FLOAT, nullptr);
...
GLTexture::CUDAMapping::CUDAMapping(GLuint texture_id, const Vector2i &size)
    : m_size{size} {
  static bool s_is_cuda_interop_supported = true;
  if (s_is_cuda_interop_supported) {
    cudaError_t err = cudaGraphicsGLRegisterImage(
        &m_graphics_resource, texture_id, GL_TEXTURE_2D,
  ...
  CUDA_CHECK_THROW(cudaGraphicsSubResourceGetMappedArray(
      &m_mapped_array, m_graphics_resource, 0, 0));
...
const float *GLTexture::CUDAMapping::data_cpu() {
  CUDA_CHECK_THROW(cudaMemcpy2DFromArray(
      m_data_cpu.data(), m_size.x() * sizeof(float) * 4, array(), 0, 0,
      m_size.x() * sizeof(float) * 4, m_size.y(), cudaMemcpyDeviceToHost));
  return m_data_cpu.data();
}
#endif  // NGP_GUI

__global__ void accumulate_kernel(Vector2i resolution, Array4f *frame_buffer,
                                  Array4f *accumulate_buffer,
                                  float sample_count, EColorSpace color_space) {
...
__device__ Array3f tonemap(Array3f col, const Array3f &exposure,
                           ETonemapCurve tonemap_curve, EColorSpace color_space,
...
__global__ void overlay_image_kernel(
    Vector2i resolution, float alpha, Array3f exposure,
    Array4f background_color, const void *__restrict__ image,
    EImageDataType image_data_type, Vector2i image_resolution,
    ETonemapCurve tonemap_curve, EColorSpace color_space,
    EColorSpace output_color_space, int fov_axis, float zoom,
    Eigen::Vector2f screen_center, cudaSurfaceObject_t surface) {
  uint32_t x = threadIdx.x + blockDim.x * blockIdx.x;
  uint32_t y = threadIdx.y + blockDim.y * blockIdx.y;
  ...
  if (srcx >= image_resolution.x() || srcy >= image_resolution.y() ||
    ...
    val = read_rgba(Vector2i{srcx, srcy}, image_resolution, image,
                    image_data_type);
  }
  ...
  color.head<3>() = tonemap(color.head<3>(), exposure, tonemap_curve,
...
__device__ Array3f colormap_turbo(float x) {
  const Vector4f kRedVec4 =
      Vector4f(0.13572138f, 4.61539260f, -42.66032258f, 132.13108234f);
  const Vector4f kGreenVec4 =
      Vector4f(0.09140261f, 2.19418839f, 4.84296658f, -14.18503333f);
  const Vector4f kBlueVec4 =
      Vector4f(0.10667330f, 12.64194608f, -60.58204836f, 110.36276771f);
  ...
  return Array3f{v4.dot(kRedVec4) + v2.dot(kRedVec2),
...
__device__ Array3f colormap_viridis(float x) {
  ...
  const Array3f c2 =
      Array3f{-0.3308618287255563f, 0.214847559468213f, 0.09509516302823659f};
  const Array3f c3 =
      Array3f{-4.634230498983486f, -5.799100973351585f, -19.33244095627987f};
  const Array3f c4 =
      Array3f{6.228269936347081f, 14.17993336680509f, 56.69055260068105f};
  const Array3f c5 =
      Array3f{4.776384997670288f, -13.74514537774601f, -65.35303263337234f};
  const Array3f c6 =
      Array3f{-5.435455855934631f, 4.645852612178535f, 26.3124352495832f};
  x = __saturatef(x);
  return (c0 + x * (c1 + x * (c2 + x * (c3 + x * (c4 + x * (c5 + x * c6))))));
}

__global__ void overlay_false_color_kernel(
    Vector2i resolution, Vector2i training_resolution, bool to_srgb,
    int fov_axis, cudaSurfaceObject_t surface, const float *error_map,
    Vector2i error_map_resolution, const float *average, float brightness,
    bool viridis) {
  uint32_t x = threadIdx.x + blockDim.x * blockIdx.x;
  uint32_t y = threadIdx.y + blockDim.y * blockIdx.y;
  ...
  int srcy = floorf(v * error_map_resolution.y() /
                    float(max(1.f, (float)training_resolution.y())));

  uint32_t idx = x + resolution.x() * y;
  uint32_t srcidx = srcx + error_map_resolution.x() * srcy;

  if (srcx >= error_map_resolution.x() || srcy >= error_map_resolution.y() ||
      srcx < 0 || srcy < 0) {
    return;
  }
...
__global__ void tonemap_kernel(
    Vector2i resolution, float exposure, Array4f background_color,
    Array4f *accumulate_buffer, EColorSpace color_space,
    EColorSpace output_color_space, ETonemapCurve tonemap_curve,
    bool clamp_output_color, cudaSurfaceObject_t surface) {
  uint32_t x = threadIdx.x + blockDim.x * blockIdx.x;
  uint32_t y = threadIdx.y + blockDim.y * blockIdx.y;
  ...
  color.head<3>() = tonemap(color.head<3>(), Array3f::Constant(exposure),
                            tonemap_curve, color_space, output_color_space);
...
__global__ void dlss_splat_kernel(Vector2i resolution,
...
void CudaRenderBuffer::clear_frame(cudaStream_t stream) {
  CUDA_CHECK_THROW(cudaMemsetAsync(m_frame_buffer.data(), 0,
                                   m_frame_buffer.bytes(), stream));
  CUDA_CHECK_THROW(cudaMemsetAsync(m_depth_buffer.data(), 0,
                                   m_depth_buffer.bytes(), stream));
}
...
void CudaRenderBuffer::accumulate(float exposure, cudaStream_t stream) {
  ...
  if (accum_spp == 0) {
    CUDA_CHECK_THROW(cudaMemsetAsync(m_accumulate_buffer.data(), 0,
                                     m_accumulate_buffer.bytes(), stream));
  }
  ...
  accumulate_kernel<<<blocks, threads, 0, stream>>>(
      res, frame_buffer(), accumulate_buffer(), (float)accum_spp,
      m_color_space);

  ++m_spp;
}

void CudaRenderBuffer::tonemap(float exposure, const Array4f &background_color,
                               EColorSpace output_color_space,
                               cudaStream_t stream) {
  ...
  if (m_dlss) {
    ...
    m_dlss->run(res, output_color_space == EColorSpace::Linear, /* HDR mode */
                m_dlss_sharpening,
                Vector2f::Constant(0.5f) -
                    ld_random_pixel_offset(
                        sample_index), /* jitter offset in [-0.5, 0.5] */
                sample_index == 0      /* reset history */
    );

    auto out_res = out_resolution();
    const dim3 out_blocks = {div_round_up((uint32_t)out_res.x(), threads.x),
                             div_round_up((uint32_t)out_res.y(), threads.y), 1};
    dlss_splat_kernel<<<out_blocks, threads, 0, stream>>>(
        out_res, m_dlss->output(), surface());
  }
}

void CudaRenderBuffer::overlay_image(
    float alpha, const Eigen::Array3f &exposure,
    const Array4f &background_color, EColorSpace output_color_space,
    const void *__restrict__ image, EImageDataType image_data_type,
    const Vector2i &image_resolution, int fov_axis, float zoom,
  ...
  overlay_image_kernel<<<blocks, threads, 0, stream>>>(
      res, alpha, exposure, background_color, image, image_data_type,
      image_resolution, m_tonemap_curve, m_color_space, output_color_space,
      fov_axis, zoom, screen_center, surface());
}

void CudaRenderBuffer::overlay_false_color(
    Vector2i training_resolution, bool to_srgb, int fov_axis,
    cudaStream_t stream, const float *error_map, Vector2i error_map_resolution,
  ...
  overlay_false_color_kernel<<<blocks, threads, 0, stream>>>(
      res, training_resolution, to_srgb, fov_axis, surface(), error_map,
      error_map_resolution, average, brightness, viridis);
}
