...
#define GLOBAL_MOVE_ACCUMULATION_DEBUG 1
...
void Testbed::load_training_data(const std::string &data_path) {
  ...
  m_log_path = m_output_path.str() + "/log.txt";
  m_mesh_path = m_output_path.str() + "/mesh";
  m_img_path = m_output_path.str() + "/images";
  ...
  if (!m_data_path.exists()) {
    throw std::runtime_error{std::string{"Data path '"} + m_data_path.str() +
                             "' does not exist."};
  }
...
void Testbed::reload_network_from_json(const json &json,
                                       const std::string &config_base_path) {
  // config_base_path is needed so that if the passed in json uses the 'parent'
  // feature, we know where to look... be sure to use a filename, or if a
  // directory, end with a trailing slash
...
Vector3f Testbed::look_at() const { return view_pos() + view_dir() * m_scale; }
...
void Testbed::set_scale(float scale) {
  ...
  m_camera.col(3) =
      (view_pos() - prev_look_at) * (scale / m_scale) + prev_look_at;
  m_scale = scale;
}
...
void Testbed::set_camera_to_training_view(int trainview) {
  ...
  m_camera = m_smoothed_camera = get_xform_given_rolling_shutter(
      m_nerf.training.transforms[trainview],
      m_nerf.training.dataset.metadata[trainview].rolling_shutter,
      Vector2f{0.5f, 0.5f}, 0.0f);
  m_relative_focal_length =
      m_nerf.training.dataset.metadata[trainview].focal_length /
  ...
  m_screen_center = Vector2f::Constant(1.0f) -
                    m_nerf.training.dataset.metadata[trainview].principal_point;
}
...
void Testbed::reset_camera() {
  ...
  m_camera << 1.0f, 0.0f, 0.0f, 0.5f, 0.0f, -1.0f, 0.0f, 0.5f, 0.0f, 0.0f,
      -1.0f, 0.5f;
...
void Testbed::compute_and_save_marching_cubes_mesh(const char *filename,
                                                   Vector3i res3d,
                                                   BoundingBox aabb,
                                                   float thresh,
                                                   bool unwrap_it) {
  ...
  if ((m_testbed_mode == ETestbedMode::Nerf)) {
    save_mesh(m_mesh.verts, m_mesh.vert_normals, m_mesh.vert_colors,
              m_mesh.indices, filename, unwrap_it,
              m_nerf.training.dataset.scale, m_nerf.training.dataset.offset,
              m_nerf.training.dataset.from_na);
  }
}

Eigen::Vector3i Testbed::compute_and_save_png_slices(const char *filename,
                                                     int res, BoundingBox aabb,
                                                     float thresh,
                                                     float density_range,
                                                     bool flip_y_and_z_axes) {
  if (aabb.is_empty()) {
    aabb = (m_testbed_mode == ETestbedMode::Nerf) ? m_render_aabb : m_aabb;
  }
  ...
  if (m_testbed_mode == ETestbedMode::Sdf)
    range *=
        -aabb.diag().x() /
        res3d
            .x();  // rescale the range to be in output voxels. ie this scale
                   // factor is mapped back to the original world space
                   // distances. negated so that black = outside, white = inside
  char fname[128];
  snprintf(fname, sizeof(fname), ".density_slices_%dx%dx%d.png", res3d.x(),
           res3d.y(), res3d.z());
  GPUMemory<float> density =
      (m_render_ground_truth && m_testbed_mode == ETestbedMode::Sdf)
          ? get_sdf_gt_on_grid(res3d, aabb)
          : get_density_on_grid(res3d, aabb);
  save_density_grid_to_png(density, (std::string(filename) + fname).c_str(),
                           res3d, thresh, flip_y_and_z_axes, range);
  return res3d;
}
...
void Testbed::imgui() {
  ...
  if (int read = ImGui::Begin("Camera Path", 0, ImGuiWindowFlags_NoScrollbar)) {
    ...
    if (path_filename_buf[0] == '\0') {
      snprintf(path_filename_buf, sizeof(path_filename_buf), "%s",
               get_filename_in_data_path_with_suffix(
                   m_data_path, m_network_config_path, "_cam.json")
                   .c_str());
    }
    if ((m_testbed_mode == ETestbedMode::Nerf)) {
      if (m_camera_path.imgui(path_filename_buf, m_render_ms.val(), m_camera,
                              m_slice_plane_z, m_scale, fov(), m_dof,
                              m_bounding_radius,
                              !m_nerf.training.dataset.xforms.empty()
                                  ? m_nerf.training.dataset.xforms[0].start
    ...
    if (!m_camera_path.m_keyframes.empty()) {
      ...
      m_picture_in_picture_res =
          (float)std::min((int(w) + 31) & (~31), 1920 / 4);
      if (m_camera_path.m_update_cam_from_path)
        ImGui::Image((ImTextureID)(size_t)m_render_textures.front()->texture(),
                     ImVec2(w, w * 9.f / 16.f));
      else
  ...
  ImGui::Begin("NeuS2 v" NGP_VERSION);

  size_t n_bytes = tcnn::total_n_bytes_allocated() + g_total_n_bytes_allocated;
  ImGui::Text("Frame: %.2f ms (%.1f FPS); Mem: %s", m_frame_ms.ema_val(),
              1000.0f / m_frame_ms.ema_val(), bytes_to_string(n_bytes).c_str());
  ...
  if (ImGui::CollapsingHeader("Training", m_training_data_available
                                              ? ImGuiTreeNodeFlags_DefaultOpen
                                              : 0)) {
    if (imgui_colored_button(m_train ? "Stop training" : "Start training",
                             0.4)) {
      set_train(!m_train);
    }
    ImGui::SameLine();
    ...
    if ((m_testbed_mode == ETestbedMode::Nerf)) {
      ...
      if (m_nerf.training.dataset.n_extra_learnable_dims) {
        ImGui::Checkbox("Train latent codes",
      ...
      if (imgui_colored_button("Export extrinsics", 0.4f)) {
        m_nerf.training.export_camera_extrinsics(
            opt_extr_filename_buf, export_extrinsics_in_quat_format);
      }
      ImGui::SameLine();
      ImGui::PushItemWidth(400.f);
      ImGui::InputText("File", opt_extr_filename_buf,
                       sizeof(opt_extr_filename_buf));
      ImGui::PopItemWidth();
      ImGui::SameLine();
      ImGui::Checkbox("Quaternion format", &export_extrinsics_in_quat_format);
    }

    if (imgui_colored_button("Next Frame", 0.f)) {
      m_training_to_next_frame = true;
    }
    ImGui::SameLine();
    ImGui::Text("current frame: %d", current_training_time_frame);

    if (imgui_colored_button("Reset training", 0.f)) {
      reload_network_from_file("");
    }
    ImGui::SameLine();
    ImGui::DragInt("Seed", (int *)&m_seed, 1.0f, 0,
                   std::numeric_limits<int>::max());
    ImGui::SliderInt("Batch size", (int *)&m_training_batch_size, 1 << 12,
                     1 << 22, "%d", ImGuiSliderFlags_Logarithmic);
    m_training_batch_size =
        next_multiple(m_training_batch_size, batch_size_granularity);
    ...
    if (m_train) {
      ...
      if ((m_testbed_mode == ETestbedMode::Nerf)) {
        ...
        timings.emplace_back(
            to_string("Datagen", m_training_prep_ms.ema_val()));
      }
    ...
    if ((m_testbed_mode == ETestbedMode::Nerf)) {
      ImGui::Text(
          ...
          (float)m_nerf.training.counters_rgb.measured_batch_size /
    ...
    ImGui::Text("Steps: %d, Loss: %0.6f (%0.2f dB), Elapsed: %.1fs",
                m_training_step, m_loss_scalar.ema_val(),
                linear_to_db(m_loss_scalar.ema_val()), elapsed_training);

    ImGui::PlotLines("loss graph", m_loss_graph.data(),
                     std::min(m_loss_graph_samples, m_loss_graph.size()),
                     (m_loss_graph_samples < m_loss_graph.size())
                         ? 0
                         : (m_loss_graph_samples % m_loss_graph.size()),
                     0, FLT_MAX, FLT_MAX, ImVec2(0, 50.f));

    if ((m_testbed_mode == ETestbedMode::Nerf) &&
        ImGui::TreeNode("NeRF training options")) {
      ImGui::Checkbox("Random bg color", &m_nerf.training.random_bg_color);
      ImGui::SameLine();
      ImGui::Checkbox("Snap to pixel centers",
                      &m_nerf.training.snap_to_pixel_centers);
      ImGui::SliderFloat("Near distance", &m_nerf.training.near_distance, 0.0f,
                         1.0f);
      accum_reset |=
          ImGui::Checkbox("Linear colors", &m_nerf.training.linear_colors);
      ImGui::Combo("Loss", (int *)&m_nerf.training.loss_type, LossTypeStr);
      ImGui::Combo("RGB activation", (int *)&m_nerf.rgb_activation,
                   NerfActivationStr);
      ImGui::Combo("Density activation", (int *)&m_nerf.density_activation,
                   NerfActivationStr);
      ImGui::SliderFloat("Cone angle", &m_nerf.cone_angle_constant, 0.0f,
                         1.0f / 128.0f);
      ImGui::SliderFloat("Depth supervision strength",
                         &m_nerf.training.depth_supervision_lambda, 0.f, 1.f);

      // Importance sampling options, but still related to training
      ImGui::Checkbox(
          "Sample focal plane ~error",
          &m_nerf.training.sample_focal_plane_proportional_to_error);
      ImGui::SameLine();
      ImGui::Checkbox("Sample focal plane ~sharpness",
                      &m_nerf.training.include_sharpness_in_error);
      ImGui::Checkbox("Sample image ~error",
                      &m_nerf.training.sample_image_proportional_to_error);
      ImGui::Text("%dx%d error res w/ %d steps between updates",
      ...
      ImGui::Checkbox("Display error overlay",
                      &m_nerf.training.render_error_overlay);
      if (m_nerf.training.render_error_overlay) {
        ImGui::SliderFloat("Error overlay brightness",
                           &m_nerf.training.error_overlay_brightness, 0.f, 1.f);
      }
      ImGui::SliderFloat("Density grid decay",
                         &m_nerf.training.density_grid_decay, 0.f, 1.f, "%.4f");
      ImGui::SliderFloat(
          "Extrinsic L2 reg.", &m_nerf.training.extrinsic_l2_reg, 1e-8f, 0.1f,
          "%.6f",
          ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
      ImGui::SliderFloat(
          "Intrinsic L2 reg.", &m_nerf.training.intrinsic_l2_reg, 1e-8f, 0.1f,
          "%.6f",
          ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
      ImGui::SliderFloat(
          "Exposure L2 reg.", &m_nerf.training.exposure_l2_reg, 1e-8f, 0.1f,
          "%.6f",
          ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
      ImGui::TreePop();
    }
    ...
    if (m_testbed_mode == ETestbedMode::Sdf &&
        ImGui::TreeNode("SDF training options")) {
      accum_reset |= ImGui::Checkbox("Use octree for acceleration",
                                     &m_sdf.use_triangle_octree);
      accum_reset |= ImGui::Combo("Mesh SDF mode", (int *)&m_sdf.mesh_sdf_mode,
                                  MeshSdfModeStr);

      accum_reset |= ImGui::SliderFloat(
          "Surface offset scale", &m_sdf.training.surface_offset_scale, 0.125f,
          1024.0f, "%.4f",
          ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
    ...
    if (m_testbed_mode == ETestbedMode::Image &&
        ImGui::TreeNode("Image training options")) {
      ImGui::Combo("Training coords", (int *)&m_image.random_mode,
                   RandomModeStr);
      ImGui::Checkbox("Snap to pixel centers",
                      &m_image.training.snap_to_pixel_centers);
      accum_reset |=
          ImGui::Checkbox("Linear colors", &m_image.training.linear_colors);
      ImGui::TreePop();
    }

    if (m_testbed_mode == ETestbedMode::Volume &&
        ImGui::CollapsingHeader("Volume training options")) {
      accum_reset |= ImGui::SliderFloat("Albedo", &m_volume.albedo, 0.f, 1.f);
      accum_reset |=
          ImGui::SliderFloat("Scattering", &m_volume.scattering, -2.f, 2.f);
      accum_reset |= ImGui::SliderFloat(
          "Distance scale", &m_volume.inv_distance_scale, 1.f, 100.f, "%.3g",
          ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
      ImGui::TreePop();
    }
  }
  ...
  if (ImGui::CollapsingHeader("Rendering", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    std::string spp_string =
        m_dlss ? std::string{""}
               : (std::string{"("} +
                  std::to_string(std::max(render_tex.spp(), 1u)) + " spp)");
    ImGui::Text(": %.01fms for %dx%d %s", m_render_ms.ema_val(),
                render_tex.in_resolution().x(), render_tex.in_resolution().y(),
                spp_string.c_str());
    ...
    if (m_dlss_supported) {
      ...
      if (render_tex.dlss()) {
        ImGui::SameLine();
        ImGui::Text("(automatic quality setting: %s)",
                    DlssQualityStrArray[(int)render_tex.dlss()->quality()]);
        ImGui::SliderFloat("DLSS sharpening", &m_dlss_sharpening, 0.0f, 1.0f,
                           "%.02f");
      }
    ...
    ImGui::SliderFloat(
        "Target FPS", &m_dynamic_res_target_fps, 2.0f, 144.0f, "%.01f",
        ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
    ImGui::SliderInt(
        "Max spp", &m_max_spp, 0, 1024, "%d",
        ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
    ...
    if ((m_testbed_mode == ETestbedMode::Nerf) &&
      ...
      if (ImGui::TreeNodeEx("Light Dir (Polar)",
        ...
        if (spin) {
          ...
          m_nerf.light_dir = {sin_phi * cos_theta, sinf(theta),
                              cos_phi * cos_theta};
          accum_reset = true;
        }
        ImGui::TreePop();
      ...
      if (ImGui::TreeNode("Light Dir (Cartesian)")) {
        accum_reset |= ImGui::SliderFloat(
            "Light Dir X", ((float *)(&m_nerf.light_dir)) + 0, -1.0f, 1.0f);
        accum_reset |= ImGui::SliderFloat(
            "Light Dir Y", ((float *)(&m_nerf.light_dir)) + 1, -1.0f, 1.0f);
        accum_reset |= ImGui::SliderFloat(
            "Light Dir Z", ((float *)(&m_nerf.light_dir)) + 2, -1.0f, 1.0f);
        ImGui::TreePop();
      }
    }
    ...
    if ((m_testbed_mode == ETestbedMode::Nerf) &&
      ...
      accum_reset |=
          ImGui::SliderInt("training image latent code for inference",
                           (int *)&m_nerf.extra_dim_idx_for_inference, 0,
    ...
    accum_reset |=
        ImGui::Combo("Color space", (int *)&m_color_space, ColorSpaceStr);
    accum_reset |=
        ImGui::Combo("Tonemap curve", (int *)&m_tonemap_curve, TonemapCurveStr);
    ...
    accum_reset |=
        ImGui::Checkbox("Snap to pixel centers", &m_snap_to_pixel_centers);
    ...
    if (ImGui::SliderFloat(
            "Crop size", &render_diam, 0.1f, max_diam, "%.3f",
            ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat)) {
      accum_reset = true;
      if (old_render_diam > 0.f && render_diam > 0.f) {
        ...
        m_render_aabb.max = ((m_render_aabb.max - center) * scale + center)
                                .cwiseMin(m_aabb.max);
        m_render_aabb.min = ((m_render_aabb.min - center) * scale + center)
                                .cwiseMax(m_aabb.min);
      }
    }
    ...
    if (ImGui::TreeNode("Crop aabb")) {
      accum_reset |=
          ImGui::SliderFloat("Min x", ((float *)&m_render_aabb.min) + 0,
                             m_aabb.min.x(), m_render_aabb.max.x(), "%.3f");
      accum_reset |=
          ImGui::SliderFloat("Min y", ((float *)&m_render_aabb.min) + 1,
                             m_aabb.min.y(), m_render_aabb.max.y(), "%.3f");
      accum_reset |=
          ImGui::SliderFloat("Min z", ((float *)&m_render_aabb.min) + 2,
                             m_aabb.min.z(), m_render_aabb.max.z(), "%.3f");
      ImGui::Separator();
      accum_reset |=
          ImGui::SliderFloat("Max x", ((float *)&m_render_aabb.max) + 0,
                             m_render_aabb.min.x(), m_aabb.max.x(), "%.3f");
      accum_reset |=
          ImGui::SliderFloat("Max y", ((float *)&m_render_aabb.max) + 1,
                             m_render_aabb.min.y(), m_aabb.max.y(), "%.3f");
      accum_reset |=
          ImGui::SliderFloat("Max z", ((float *)&m_render_aabb.max) + 2,
                             m_render_aabb.min.z(), m_aabb.max.z(), "%.3f");
      ImGui::TreePop();
    }

    if ((m_testbed_mode == ETestbedMode::Nerf) &&
        ImGui::TreeNode("NeRF rendering options")) {
      accum_reset |= ImGui::Checkbox("Apply lens distortion",
                                     &m_nerf.render_with_camera_distortion);

      if (m_nerf.render_with_camera_distortion) {
        accum_reset |= ImGui::Combo("Distortion mode",
                                    (int *)&m_nerf.render_distortion.mode,
                                    "None\0Iterative\0F-Theta\0");
        if (m_nerf.render_distortion.mode == ECameraDistortionMode::Iterative) {
          accum_reset |= ImGui::InputFloat(
              "k1", &m_nerf.render_distortion.params[0], 0.f, 0.f, "%.5f");
          accum_reset |= ImGui::InputFloat(
              "k2", &m_nerf.render_distortion.params[1], 0.f, 0.f, "%.5f");
          accum_reset |= ImGui::InputFloat(
              "p1", &m_nerf.render_distortion.params[2], 0.f, 0.f, "%.5f");
          accum_reset |= ImGui::InputFloat(
              "p2", &m_nerf.render_distortion.params[3], 0.f, 0.f, "%.5f");
        } else if (m_nerf.render_distortion.mode ==
                   ECameraDistortionMode::FTheta) {
          accum_reset |= ImGui::InputFloat(
              "width", &m_nerf.render_distortion.params[5], 0.f, 0.f, "%.0f");
          accum_reset |= ImGui::InputFloat(
              "height", &m_nerf.render_distortion.params[6], 0.f, 0.f, "%.0f");
          accum_reset |= ImGui::InputFloat("f_theta p0",
                                           &m_nerf.render_distortion.params[0],
                                           0.f, 0.f, "%.5f");
          accum_reset |= ImGui::InputFloat("f_theta p1",
                                           &m_nerf.render_distortion.params[1],
                                           0.f, 0.f, "%.5f");
          accum_reset |= ImGui::InputFloat("f_theta p2",
                                           &m_nerf.render_distortion.params[2],
                                           0.f, 0.f, "%.5f");
          accum_reset |= ImGui::InputFloat("f_theta p3",
                                           &m_nerf.render_distortion.params[3],
                                           0.f, 0.f, "%.5f");
          accum_reset |= ImGui::InputFloat("f_theta p4",
                                           &m_nerf.render_distortion.params[4],
                                           0.f, 0.f, "%.5f");
        }
      }
      ImGui::TreePop();
    ...
    if (m_testbed_mode == ETestbedMode::Sdf &&
        ImGui::TreeNode("SDF rendering options")) {
      accum_reset |= ImGui::Combo("Ground Truth Rendering Mode",
      ...
      if (m_sdf.groundtruth_mode == ESDFGroundTruthMode::SDFBricks) {
        accum_reset |= ImGui::SliderInt("Brick Octree Level",
                                        (int *)&m_sdf.brick_level, 1, 10);
        accum_reset |= ImGui::Checkbox("Brick Normals track Octree Level",
                                       &m_sdf.brick_smooth_normals);
        accum_reset |= ImGui::SliderInt(
            "Brick Quantize Bits", (int *)&m_sdf.brick_quantise_bits, 0, 16);
      }
      accum_reset |=
          ImGui::Checkbox("Analytic normals", &m_sdf.analytic_normals);

      accum_reset |= ImGui::SliderFloat(
          "Normals epsilon", &m_sdf.fd_normals_epsilon, 0.00001f, 0.1f, "%.6g",
          ImGuiSliderFlags_Logarithmic);
      accum_reset |= ImGui::SliderFloat("Maximum distance",
                                        &m_sdf.maximum_distance, 0.00001f, 0.1f,
                                        "%.6g", ImGuiSliderFlags_Logarithmic);
      accum_reset |=
          ImGui::SliderFloat("Shadow sharpness", &m_sdf.shadow_sharpness, 0.1f,
                             2048.0f, "%.6g", ImGuiSliderFlags_Logarithmic);

      accum_reset |= ImGui::SliderFloat("Inflate (offset the zero set)",
                                        &m_sdf.zero_offset, -0.25f, 0.25f);
      accum_reset |= ImGui::SliderFloat("Distance scale", &m_sdf.distance_scale,
                                        0.25f, 1.f);
    ...
    if (m_testbed_mode == ETestbedMode::Image &&
        ImGui::TreeNode("Image rendering options")) {
    ...
    if (ImGui::TreeNode("Debug visualization")) {
      ...
      if ((m_testbed_mode == ETestbedMode::Nerf)) {
        ...
        accum_reset |=
            ImGui::SliderInt("Show acceleration", &m_nerf.show_accel, -1, 7);
      }

      if (!m_single_view) {
        ImGui::BeginDisabled();
      }
      if (ImGui::SliderInt("Visualized dimension", &m_visualized_dimension, -1,
                           (int)network_width(m_visualized_layer) - 1)) {
      ...
      if (ImGui::SliderInt("Visualized layer", &m_visualized_layer, 0,
                           (int)network_num_forward_activations() - 1)) {
      ...
      if ((m_testbed_mode == ETestbedMode::Nerf)) {
        if (ImGui::SliderInt("Training view", &m_nerf.training.view, 0,
                             (int)m_nerf.training.dataset.n_images - 1)) {
          set_camera_to_training_view(m_nerf.training.view);
          accum_reset = true;
        }
        ImGui::PlotLines("Training view error",
                         m_nerf.training.error_map.pmf_img_cpu.data(),
                         m_nerf.training.error_map.pmf_img_cpu.size(), 0,
                         nullptr, 0.0f, FLT_MAX, ImVec2(0, 60.f));
        ...
        if (m_nerf.training.optimize_exposure) {
          ...
          ImGui::PlotLines("Training view exposures", exposures.data(),
                           exposures.size(), 0, nullptr, FLT_MAX, FLT_MAX,
                           ImVec2(0, 60.f));
        }
        ...
        if (m_nerf.m_glow_mode &&
            ImGui::SliderFloat("glow pos", &m_nerf.m_glow_y_cutoff, -2.f,
                               3.f)) {
          accum_reset = true;
        }
      }
  ...
  if (ImGui::CollapsingHeader("Camera", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::TreeNode("Advanced camera settings")) {
      accum_reset |=
          ImGui::SliderFloat2("Screen center", &m_screen_center.x(), 0.f, 1.f);
      accum_reset |= ImGui::SliderFloat2("Parallax shift",
                                         &m_parallax_shift.x(), -1.f, 1.f);
      accum_reset |= ImGui::SliderFloat("Slice / focus depth", &m_slice_plane_z,
                                        -m_bounding_radius, m_bounding_radius);
      char buf[2048];
      Vector3f v = view_dir();
      Vector3f p = look_at();
      ...
      snprintf(
          ...
          "testbed.autofocus = %s\n\n",
          b.x(), b.y(), b.z(), b.w(), m_exposure, s.x(), s.y(), s.z(), u.x(),
          u.y(), u.z(), v.x(), v.y(), v.z(), p.x(), p.y(), p.z(), scale(),
          fov(), m_dof, m_slice_plane_z, m_autofocus_target.x(),
          m_autofocus_target.y(), m_autofocus_target.z(),
      ...
      if (m_testbed_mode == ETestbedMode::Sdf) {
        ...
        snprintf(
            ...
            m_sdf.shadow_sharpness, m_sdf.analytic_normals ? "True" : "False",
            m_sdf.use_triangle_octree ? "True" : "False", m_sdf.brdf.metallic,
            m_sdf.brdf.subsurface, m_sdf.brdf.specular, m_sdf.brdf.roughness,
            m_sdf.brdf.sheen, m_sdf.brdf.clearcoat, m_sdf.brdf.clearcoat_gloss,
            m_sdf.brdf.basecolor.x(), m_sdf.brdf.basecolor.y(),
  ...
  if (ImGui::CollapsingHeader("Snapshot")) {
    ...
    if (snapshot_filename_buf[0] == '\0') {
      snprintf(snapshot_filename_buf, sizeof(snapshot_filename_buf), "%s",
               get_filename_in_data_path_with_suffix(
                   m_data_path, m_network_config_path, ".msgpack")
                   .c_str());
    }
    ...
    if (ImGui::Button("Save")) {
      save_snapshot(snapshot_filename_buf,
    ...
    if (ImGui::Button("Load")) {
      try {
        ...
        snapshot_load_error_string =
            std::string{"Failed to load snapshot: "} + e.what();
      }
    }
    if (ImGui::BeginPopupModal("Snapshot load error", NULL,
    ...
    ImGui::SameLine();
    ImGui::Checkbox("w/ Optimizer State",
                    &m_include_optimizer_state_in_snapshot);
    ImGui::InputText("File", snapshot_filename_buf,
                     sizeof(snapshot_filename_buf));
  }
  ...
  if ((m_testbed_mode == ETestbedMode::Nerf) ||
    ...
    if (ImGui::CollapsingHeader("Marching Cubes Mesh Output")) {
      ...
      BoundingBox aabb =
          (m_testbed_mode == ETestbedMode::Nerf) ? m_render_aabb : m_aabb;
      ...
      if (imgui_colored_button("Save density PNG", -0.4f)) {
        Testbed::compute_and_save_png_slices(m_data_path.str().c_str(),
                                             m_mesh.res, {}, m_mesh.thresh,
                                             density_range, flip_y_and_z_axes);
      }

      if ((m_testbed_mode == ETestbedMode::Nerf)) {
        ImGui::SameLine();
        if (imgui_colored_button("Save RGBA PNG sequence", 0.2f)) {
          auto effective_view_dir = flip_y_and_z_axes
          ...
          save_rgba_grid_to_png_sequence(rgba, dir.str().c_str(), res3d,
                                         flip_y_and_z_axes);
        }
      }
      ...
      ImGui::SliderInt("Res:", &m_mesh.res, 16, 2048, "%d",
                       ImGuiSliderFlags_Logarithmic);
      ImGui::SameLine();

      ImGui::Text("%dx%dx%d", res3d.x(), res3d.y(), res3d.z());
      if (obj_filename_buf[0] == '\0') {
        snprintf(obj_filename_buf, sizeof(obj_filename_buf), "%s",
                 get_filename_in_data_path_with_suffix(
                     m_data_path, m_network_config_path, ".obj")
                     .c_str());
      }
      float thresh_range = (m_testbed_mode == ETestbedMode::Sdf) ? 0.5f : 10.f;
      ImGui::SliderFloat("MC density threshold", &m_mesh.thresh, -thresh_range,
                         thresh_range);
      ImGui::Combo("Mesh render mode", (int *)&m_mesh_render_mode,
                   "Off\0Vertex Colors\0Vertex Normals\0Face IDs\0");
      ImGui::Checkbox("Unwrap mesh", &m_mesh.unwrap);
      if (uint32_t tricount = m_mesh.indices.size() / 3) {
        ImGui::InputText("##OBJFile", obj_filename_buf,
                         sizeof(obj_filename_buf));
        if (ImGui::Button("Save it!")) {
          save_mesh(m_mesh.verts, m_mesh.vert_normals, m_mesh.vert_colors,
                    m_mesh.indices, obj_filename_buf, m_mesh.unwrap,
        ...
        ImGui::SliderFloat("Laplacian smoothing", &m_mesh.smooth_amount, 0.f,
                           2048.f);
  ...
  if (m_testbed_mode == ETestbedMode::Sdf) {
    if (ImGui::CollapsingHeader("BRDF parameters")) {
      accum_reset |=
          ImGui::ColorEdit3("Base color", (float *)&m_sdf.brdf.basecolor);
      accum_reset |=
          ImGui::SliderFloat("Roughness", &m_sdf.brdf.roughness, 0.f, 1.f);
      accum_reset |=
          ImGui::SliderFloat("Specular", &m_sdf.brdf.specular, 0.f, 1.f);
      accum_reset |=
          ImGui::SliderFloat("Metallic", &m_sdf.brdf.metallic, 0.f, 1.f);
      ImGui::Separator();
      accum_reset |=
          ImGui::SliderFloat("Subsurface", &m_sdf.brdf.subsurface, 0.f, 1.f);
      accum_reset |= ImGui::SliderFloat("Sheen", &m_sdf.brdf.sheen, 0.f, 1.f);
      accum_reset |=
          ImGui::SliderFloat("Clearcoat", &m_sdf.brdf.clearcoat, 0.f, 1.f);
      accum_reset |= ImGui::SliderFloat("Clearcoat gloss",
                                        &m_sdf.brdf.clearcoat_gloss, 0.f, 1.f);
    }
    m_sdf.brdf.ambientcolor =
        (m_background_color * m_background_color).head<3>();
  }
  ...
  if (ImGui::CollapsingHeader("Histograms of trainable encoding parameters")) {
    ...
    ImGui::PlotHistogram("Grid means", f.data(), m_num_levels, 0, "means",
                         FLT_MAX, FLT_MAX, ImVec2(0, 60.f));
    for (int i = 0; i < m_num_levels; ++i) {
      f[i] = m_level_stats[i].sigma();
    }
    ImGui::PlotHistogram("Grid sigmas", f.data(), m_num_levels, 0, "sigma",
                         FLT_MAX, FLT_MAX, ImVec2(0, 60.f));
    ImGui::Separator();

    // Histogram of trained hashgrid params
    ImGui::SliderInt("Show details for level", &m_histo_level, 0,
                     m_num_levels - 1);
    if (m_histo_level < m_num_levels) {
      LevelStats &s = m_level_stats[m_histo_level];
      static bool excludezero = false;
      if (excludezero) m_histo[128] = 0.f;
      ImGui::PlotHistogram("Values histogram", m_histo, 257, 0, "", FLT_MAX,
                           FLT_MAX, ImVec2(0, 120.f));
      ImGui::SliderFloat("Histogram horizontal scale", &m_histo_scale, 0.01f,
                         2.f);
...
void Testbed::visualize_nerf_cameras(ImDrawList *list,
                                     const Matrix<float, 4, 4> &world2proj) {
  if (m_testbed_mode == ETestbedMode::Nerf) {
    for (int i = 0; i < m_nerf.training.n_images_for_training; ++i) {
      auto res = m_nerf.training.dataset.metadata[i].resolution;
      float aspect = float(res.x()) / float(res.y());
      visualize_nerf_camera(list, world2proj,
                            m_nerf.training.dataset.xforms[i].start, aspect,
                            0x40ffff40);
      visualize_nerf_camera(list, world2proj,
                            m_nerf.training.dataset.xforms[i].end, aspect,
                            0x40ffff40);
      visualize_nerf_camera(list, world2proj,
                            m_nerf.training.transforms[i].start, aspect,
                            0x80ffffff);

      add_debug_line(list, world2proj,
                     m_nerf.training.dataset.xforms[i].start.col(3),
                     m_nerf.training.transforms[i].start.col(3),
                     0xffff40ff);  // 1% loss change offset

      // Visualize near distance
      add_debug_line(list, world2proj,
                     m_nerf.training.transforms[i].start.col(3),
                     m_nerf.training.transforms[i].start.col(3) +
                         m_nerf.training.transforms[i].start.col(2) *
...
void Testbed::draw_visualizations(ImDrawList *list,
  ...
  if (m_testbed_mode != ETestbedMode::Image) {
    ...
    view2proj << xyscale, 0,
        (float)m_window_res.x() * screen_center.x() * zscale, 0, 0, xyscale,
        (float)m_window_res.y() * screen_center.y() * zscale, 0, 0, 0, 1, 0, 0,
        0, zscale, 0;
    ...
    if (m_camera_path.imgui_viz(list, view2proj, world2proj, world2view, focal,
                                aspect)) {
      m_pip_render_surface->reset_accumulation();
    }
  }
...
bool Testbed::keyboard_event() {
  ...
  for (int idx = 0; idx < std::min((int)ERenderMode::NumRenderModes, 10);
       ++idx) {
    char c[] = {"1234567890"};
  ...
  if (ImGui::IsKeyPressed('.')) {
    if (m_single_view) {
      if (m_visualized_dimension == m_network->width(m_visualized_layer) - 1 &&
          m_visualized_layer < m_network->num_forward_activations() - 1) {
        set_visualized_layer(
            std::max(0, std::min((int)m_network->num_forward_activations() - 1,
                                 m_visualized_layer + 1)));
        set_visualized_dim(0);
      } else {
        set_visualized_dim(
            std::max(-1, std::min((int)m_network->width(m_visualized_layer) - 1,
                                  m_visualized_dimension + 1)));
      }
    } else {
      set_visualized_layer(
          std::max(0, std::min((int)m_network->num_forward_activations() - 1,
...
void Testbed::mouse_drag(const Vector2f &rel, int button) {
  ...
  if (is_left_held) {
    if (shift) {
      ...
      Matrix3f rot =
          (AngleAxisf(static_cast<float>(-rel.x() * 2 * PI() * rot_sensitivity),
                      up) *  // Scroll sideways around up vector
           AngleAxisf(static_cast<float>(-rel.y() * 2 * PI() * rot_sensitivity),
                      side))
              .matrix();  // Scroll around side vector
  ...
  if (is_right_held) {
    Matrix3f rot = (AngleAxisf(static_cast<float>(-rel.x() * 2 * PI()),
                               up) *  // Scroll sideways around up vector
                    AngleAxisf(static_cast<float>(-rel.y() * 2 * PI()), side))
                       .matrix();  // Scroll around side vector
...
bool Testbed::begin_frame_and_handle_user_input() {
  if (glfwWindowShouldClose(m_glfw_window) ||
      ImGui::IsKeyDown(GLFW_KEY_ESCAPE) || ImGui::IsKeyDown(GLFW_KEY_Q)) {
    destroy_window();
    return false;
  }
  ...
  if (ImGui::IsKeyPressed(GLFW_KEY_TAB) ||
      ImGui::IsKeyPressed(GLFW_KEY_GRAVE_ACCENT)) {
  ...
  if ((m_testbed_mode == ETestbedMode::Nerf) &&
      (m_render_ground_truth || m_nerf.training.render_error_overlay)) {
  ...
  keyboard_event();
  mouse_wheel({m.x / (float)m_window_res.y(), m.y / (float)m_window_res.y()},
              mw);
...
void Testbed::draw_gui() {
  ...
  if (m_single_view) {
    list->AddImageQuad(
        (ImTextureID)(size_t)m_render_textures.front()->texture(),
        ImVec2{0.f, 0.f}, ImVec2{(float)display_w, 0.f},
        ImVec2{(float)display_w, (float)display_h},
        ImVec2{0.f, (float)display_h}, ImVec2(0, 0), ImVec2(1, 0), ImVec2(1, 1),
        ImVec2(0, 1));
  } else {
    m_dlss = false;
    ...
    for (int y = 0; y < m_n_views.y(); ++y) {
      for (int x = 0; x < m_n_views.x(); ++x) {
        ...
        list->AddImageQuad(
            ...
            ImVec2{top_left.x() + (float)m_view_size.x(),
                   top_left.y() + (float)m_view_size.y()},
  ...
  auto draw_mesh = [&]() {
    ...
    Vector2f screen_center = render_screen_center();
    draw_mesh_gl(m_mesh.verts, m_mesh.vert_normals, m_mesh.vert_colors,
                 m_mesh.indices, res, focal_length, m_smoothed_camera,
                 screen_center, (int)m_mesh_render_mode);
  };
...
void Testbed::train_and_render(bool skip_rendering) {
  ...
  if (m_single_view) {
    ...
    float render_time_per_fullres_frame =
        m_render_ms.val() / (float)render_res.x() / (float)render_res.y() *
        (float)m_window_res.x() * (float)m_window_res.y();

    // Make sure we don't starve training with slow rendering
    float factor = std::sqrt(1000.0f / m_dynamic_res_target_fps /
                             render_time_per_fullres_frame);
    if (!m_dynamic_res) {
    ...
    if (factor > m_last_render_res_factor * 1.2f ||
        factor < m_last_render_res_factor * 0.8f || factor == 1.0f ||
        !m_dynamic_res) {
      render_res = (m_window_res.cast<float>() * factor)
                       .cast<int>()
                       .cwiseMin(m_window_res)
                       .cwiseMax(m_window_res / 16);
    ...
    if (m_dlss || m_max_spp <= 0 || render_buffer.spp() < m_max_spp) {
      render_frame(m_smoothed_camera, m_smoothed_camera,
                   Eigen::Vector4f::Zero(), render_buffer);
    }
    ...
    if (m_picture_in_picture_res > 0) {
      ...
      if (m_pip_render_surface->spp() < 8) {
        ...
        render_frame(pip_kf.m(), pip_kf.m(), Eigen::Vector4f::Zero(),
    ...
    float d = std::sqrt((float)m_window_res.x() * (float)m_window_res.y() /
                        (float)n_views);
    ...
    for (int y = 0; y < ny; ++y) {
      for (int x = 0; x < nx; ++x) {
        ...
        render_frame(m_smoothed_camera, m_smoothed_camera,
                     Eigen::Vector4f::Zero(), m_render_surfaces[i]);
...
void Testbed::init_window(int resw, int resh, bool hidden) {
#ifndef NGP_GUI
  throw std::runtime_error{
      "init_window failed: NGP was built without GUI support"};
#else
  ...
  try {
    ...
    tlog::warning()
        << "Could not initialize Vulkan and NGX. DLSS not supported. ("
        << e.what() << ")";
  }
#else
  ...
  title += ")";
  m_glfw_window = glfwCreateWindow(m_window_res.x(), m_window_res.y(),
  ...
  glfwSetDropCallback(
      m_glfw_window, [](GLFWwindow *window, int count, const char **paths) {
  ...
  glfwSetKeyCallback(m_glfw_window, [](GLFWwindow *window, int key,
  ...
  glfwSetCursorPosCallback(
      m_glfw_window, [](GLFWwindow *window, double xpos, double ypos) {
        Testbed *testbed = (Testbed *)glfwGetWindowUserPointer(window);
        if (testbed) {
          testbed->redraw_gui_next_frame();
        }
  ...
  glfwSetMouseButtonCallback(
      m_glfw_window, [](GLFWwindow *window, int button, int action, int mods) {
        Testbed *testbed = (Testbed *)glfwGetWindowUserPointer(window);
        if (testbed) {
          testbed->redraw_gui_next_frame();
        }
  ...
  glfwSetScrollCallback(
      m_glfw_window, [](GLFWwindow *window, double xoffset, double yoffset) {
        Testbed *testbed = (Testbed *)glfwGetWindowUserPointer(window);
        if (testbed) {
          testbed->redraw_gui_next_frame();
        }
  ...
  glfwSetWindowSizeCallback(
      m_glfw_window, [](GLFWwindow *window, int width, int height) {
        Testbed *testbed = (Testbed *)glfwGetWindowUserPointer(window);
        if (testbed) {
  ...
  io.ConfigInputTrickleEventQueue =
      false;  // new ImGui event handling seems to make camera controls laggy if
              // this is true.
  ...
  m_pip_render_surface =
      std::make_unique<CudaRenderBuffer>(m_pip_render_texture);
...
#ifndef NGP_GUI
  throw std::runtime_error{
      "destroy_window failed: NGP was built without GUI support"};
#else
  if (!m_render_window) {
...
bool Testbed::frame() {
  ...
  // change uint32_t n_to_skip = m_train ? tcnn::clamp(m_training_step / 16u,
  // 15u, 255u) : 0;
  uint32_t n_to_skip =
      m_train ? tcnn::clamp(m_canonical_training_step / 16u, 15u, 255u) : 0;
  ...
  bool skip_rendering =
      m_render_skip_due_to_lack_of_camera_movement_counter++ != 0;
  if (!skip_rendering ||
      (std::chrono::steady_clock::now() - m_last_gui_draw_time_point) > 100ms) {
  ...
  if (((current_training_time_frame == 0) &&
       m_training_step >= first_frame_max_training_step) ||
      ((current_training_time_frame > 0) &&
       m_training_step >= next_frame_max_training_step)) {
    m_training_to_next_frame = true;
  }
...
void Testbed::apply_camera_smoothing(float elapsed_ms) {
  if (m_camera_smoothing) {
    ...
    m_smoothed_camera =
        log_space_lerp(m_smoothed_camera, m_camera, 1.0f - decay);
  } else {
    m_smoothed_camera = m_camera;
  }
}
...
void Testbed::set_camera_from_time(float t) {
  if (m_camera_path.m_keyframes.empty()) return;
  set_camera_from_keyframe(m_camera_path.eval_camera_path(t));
}

void Testbed::update_loss_graph() {
  m_loss_graph[m_loss_graph_samples++ % m_loss_graph.size()] =
      std::log(m_loss_scalar.val());
}
...
void Testbed::set_visualized_layer(int layer) {
  ...
  m_visualized_dimension = std::max(
      -1, std::min(m_visualized_dimension, (int)m_network->width(layer) - 1));
  reset_accumulation();
}
...
Testbed::NetworkDims Testbed::network_dims() const {
  switch (m_testbed_mode) {
    case ETestbedMode::Nerf:
      ...
      break;
    case ETestbedMode::Sdf:
      return network_dims_sdf();
      break;
    case ETestbedMode::Image:
      return network_dims_image();
      break;
    case ETestbedMode::Volume:
      return network_dims_volume();
      break;
    default:
...
__global__ void add_mesh_pos(const uint32_t n_elements, const float *mesh_vex,
                             const uint32_t dim,
                             tcnn::MatrixView<float> mesh_pos_i) {
...
void Testbed::change_to_frame(uint32_t frame_idx) {
  ...
  // logging
  float this_frame_training_itme =
      std::chrono::duration<float>(std::chrono::steady_clock::now() -
                                   m_training_one_time_frame_point)
          .count();
...
void Testbed::prepare_for_test() {
  ...
  if (current_training_time_frame == 0 || !m_train_delta) {
    ...
    printf("Error: m_train_delta is not set correctly. \n");
  }
}
...
bool Testbed::training_network_next_frame() {
  ...
  m_nerf_network->accumulate_global_movement(m_training_stream);

  // test: directly reload everything and then load checkpoint
  static char snapshot_filename_buf[128] = "";
  if (snapshot_filename_buf[0] == '\0') {
    snprintf(snapshot_filename_buf, sizeof(snapshot_filename_buf), "%s",
             get_filename_in_data_path_with_suffix(
  ...
  save_snapshot_incremental(snapshot_filename_buf, false);
  load_snapshot_incremental(snapshot_filename_buf);

  if (current_training_time_frame >= 1 && m_incremental_reinit_sdf_mlp &&
      current_training_time_frame % m_incremental_reinit_sdf_mlp_iters == 0) {
    tcnn::pcg32 sdf_rng{1337};
    m_nerf_network->initialize_sdf_mlp_params(
        sdf_rng, m_trainer->params_full_precision(), m_trainer->params(),
        m_trainer->params_inference(), m_trainer->params_backward(),
        m_trainer->param_gradients());
  }

  leaf_optimizer_config = &m_network_config["optimizer"];
  while (leaf_optimizer_config->contains("nested")) {
    leaf_optimizer_config = &(*leaf_optimizer_config)["nested"];
  }
  (*leaf_optimizer_config)["optimize_canonical_params"] = false;
  m_train_canonical = false;
  if (m_predict_global_movement) {
    m_train_delta = true;
  }
  m_optimizer->update_hyperparams(optimizer_config);
...
void Testbed::reset_network() {
  ...
  m_nerf.training.tv_loss_rng = default_rng_t{m_rng.next_uint()};
  ...
  json &optimizer_config = config["optimizer"];
  json &network_config = config["network"];
  json &hyperparams_config = config["hyperparams"];

  // set hyperparams
  first_frame_max_training_step =
      hyperparams_config.value("first_frame_max_training_step", 2000u);
  next_frame_max_training_step =
      hyperparams_config.value("next_frame_max_training_step", 1000u);
  m_training_batch_size = hyperparams_config.value("batch_size", 1 << 18);
  m_incremental_reinit_sdf_mlp =
      hyperparams_config.value("incremental_reinit_sdf_mlp", false);
  m_incremental_reinit_sdf_mlp_iters =
      hyperparams_config.value("incremental_reinit_sdf_mlp_iters", 10);
  m_reset_density_grid_after_global_movement = hyperparams_config.value(
      "reset_density_grid_after_global_movement", true);
  // finetune the global movement when optimizing the canonical network
  m_finetune_global_movement =
      hyperparams_config.value("finetune_global_movement", true);

  m_anneal_end = hyperparams_config.value("anneal_end", 0);

  m_predict_global_movement =
      hyperparams_config.value("predict_global_movement", false);
  m_predict_global_movement_training_step =
      hyperparams_config.value("predict_global_movement_training_step", 300u);
  if (!m_predict_global_movement) {
    m_predict_global_movement_training_step = 0;
  }
  ...
  if (to_lower(encoding_config.value("otype", "OneBlob")).find("grid") !=
    ...
    float desired_resolution = encoding_config.value("top_resolution", 2048.0f);
    if (m_testbed_mode == ETestbedMode::Image) {
    ...
    if (m_testbed_mode == ETestbedMode::Nerf && m_per_level_scale <= 0.0f &&
        m_num_levels > 1) {
      m_per_level_scale =
          std::exp(std::log(desired_resolution *
                            (float)m_nerf.training.dataset.aabb_scale /
                            (float)m_base_grid_resolution) /
                   (m_num_levels - 1));
      encoding_config["per_level_scale"] = m_per_level_scale;
    }

    m_top_grid_resolution =
        (uint32_t)ceilf(
            exp2f((m_num_levels - 1) * std::log2(m_per_level_scale)) *
                m_base_grid_resolution -
            1.0f) +
        1;
  ...
  if (m_testbed_mode == ETestbedMode::Nerf) {
    m_nerf.training.cam_exposure.resize(
        m_nerf.training.dataset.n_images,
        AdamOptimizer<Array3f>(1e-3f, Array3f::Zero()));
    m_nerf.training.cam_pos_offset.resize(
        m_nerf.training.dataset.n_images,
        AdamOptimizer<Vector3f>(1e-4f, Vector3f::Zero()));
    ...
    m_network = m_nerf_network = std::make_shared<NerfNetwork<precision_t>>(
        dims.n_pos, n_dir_dims, n_extra_dims,
    ...
    n_encoding_params =
        m_encoding->n_params() + m_nerf_network->dir_encoding()->n_params();
    ...
    {
      json &distortion_map_optimizer_config =
          config.contains("distortion_map") &&
                  config["distortion_map"].contains("optimizer")
              ? config["distortion_map"]["optimizer"]
              : optimizer_config;

      m_distortion.resolution = Vector2i::Constant(32);
      if (config.contains("distortion_map") &&
          config["distortion_map"].contains("resolution")) {
        from_json(config["distortion_map"]["resolution"],
                  m_distortion.resolution);
      }
      m_distortion.map = std::make_shared<TrainableBuffer<2, 2, float>>(
          m_distortion.resolution);
      m_distortion.optimizer.reset(
          create_optimizer<float>(distortion_map_optimizer_config));
      m_distortion.trainer = std::make_shared<Trainer<float, float>>(
          m_distortion.map, m_distortion.optimizer,
          std::shared_ptr<Loss<float>>{create_loss<float>(loss_config)},
          m_seed);
    }
  } else {
    uint32_t alignment =
        network_config.contains("otype") &&
                (equals_case_insensitive(network_config["otype"],
                                         "FullyFusedMLP") ||
                 equals_case_insensitive(network_config["otype"],
                                         "MegakernelMLP"))
            ? 16u
    ...
    if (encoding_config.contains("otype") &&
        equals_case_insensitive(encoding_config["otype"], "Takikawa")) {
      if (m_sdf.octree_depth_target == 0) {
        m_sdf.octree_depth_target = encoding_config["n_levels"];
      }

      if (!m_sdf.triangle_octree ||
          m_sdf.triangle_octree->depth() != m_sdf.octree_depth_target) {
        m_sdf.triangle_octree.reset(new TriangleOctree{});
        m_sdf.triangle_octree->build(*m_sdf.triangle_bvh, m_sdf.triangles_cpu,
      ...
      m_encoding.reset(new TakikawaEncoding<precision_t>(
          encoding_config["starting_level"], m_sdf.triangle_octree,
          tcnn::string_to_interpolation_type(
              encoding_config.value("interpolation", "linear"))));

      m_network = std::make_shared<NetworkWithInputEncoding<precision_t>>(
          m_encoding, dims.n_output, network_config);
      m_sdf.uses_takikawa_encoding = true;
    } else {
      m_encoding.reset(
          create_encoding<precision_t>(dims.n_input, encoding_config));
      m_network = std::make_shared<NetworkWithInputEncoding<precision_t>>(
          m_encoding, dims.n_output, network_config);
      m_sdf.uses_takikawa_encoding = false;
      if (m_sdf.octree_depth_target == 0 &&
          encoding_config.contains("n_levels")) {
        m_sdf.octree_depth_target = encoding_config["n_levels"];
      }
    }
  ...
  tlog::info() << "  total_encoding_params=" << n_encoding_params
               << " total_network_params=" << n_network_params;
  ...
  // Create global movement model
  {
    json &globalmove_loss_config =
        config.contains("globalmove") && config["globalmove"].contains("loss")
            ? config["globalmove"]["loss"]
            : loss_config;
    json &globalmove_optimizer_config =
        config.contains("globalmove") &&
                config["globalmove"].contains("optimizer")
            ? config["globalmove"]["optimizer"]
            : optimizer_config;

    m_global_move.loss_type =
        string_to_loss_type(globalmove_loss_config.value("otype", "L2"));
    m_global_move.delta_network = m_nerf_network->delta_network();
    m_global_move.optimizer.reset(
        create_optimizer<precision_t>(globalmove_optimizer_config));
    m_global_move.trainer =
        std::make_shared<Trainer<float, precision_t, float>>(
            m_global_move.delta_network, m_global_move.optimizer,
            std::shared_ptr<Loss<float>>{
                create_loss<float>(globalmove_loss_config)},
            m_seed);
  }

  // Create envmap model
  {
    json &envmap_loss_config =
        config.contains("envmap") && config["envmap"].contains("loss")
            ? config["envmap"]["loss"]
            : loss_config;
    json &envmap_optimizer_config =
        config.contains("envmap") && config["envmap"].contains("optimizer")
            ? config["envmap"]["optimizer"]
            : optimizer_config;

    m_envmap.loss_type =
        string_to_loss_type(envmap_loss_config.value("otype", "L2"));
    if (m_testbed_mode == ETestbedMode::Nerf) {
      m_envmap.resolution = m_nerf.training.dataset.envmap_resolution;
    }
    m_envmap.envmap =
        std::make_shared<TrainableBuffer<4, 2, float>>(m_envmap.resolution);
    ...
    m_envmap.trainer = std::make_shared<Trainer<float, float, float>>(
        m_envmap.envmap, m_envmap.optimizer,
        std::shared_ptr<Loss<float>>{create_loss<float>(envmap_loss_config)},
        m_seed);

    if (m_nerf.training.dataset.envmap_data.data()) {
      m_envmap.trainer->set_params_full_precision(
          m_nerf.training.dataset.envmap_data.data(),
          m_nerf.training.dataset.envmap_data.size());
    }
  }
}

void Testbed::reset_network_incremental() {
  m_sdf.iou_decay = 0;
  ...
  // change learning rate for next frame
  json *leaf_optimizer_config = &m_network_config["optimizer"];
  while (leaf_optimizer_config->contains("nested")) {
    leaf_optimizer_config = &(*leaf_optimizer_config)["nested"];
  }
  (*leaf_optimizer_config)["learning_rate"] =
      (*leaf_optimizer_config)["after_learning_rate"];
...
Testbed::Testbed(ETestbedMode mode) : m_testbed_mode(mode) {
  ...
  if (compute_capability < MIN_GPU_ARCH) {
    tlog::warning() << "Insufficient compute capability " << compute_capability
                    << " detected.";
    tlog::warning() << "This program was compiled for >=" << MIN_GPU_ARCH
                    << " and may thus behave unexpectedly.";
  }
...
void Testbed::train(uint32_t batch_size) {
  ...
  if (current_training_time_frame >= 1) {
    m_nerf_network->encoding()->set_training_step(
        m_training_step -
        m_predict_global_movement_training_step);  // for progressive learning
  } else {
    m_nerf_network->encoding()->set_training_step(m_training_step);
  }

  if (m_training_step == m_predict_global_movement_training_step &&
      current_training_time_frame >= 1) {
    m_train_canonical = true;
    if (!m_finetune_global_movement) {
      m_train_delta = false;
    }
    if (m_reset_density_grid_after_global_movement) {
      reset_density_grid_nerf(m_training_stream);
    }
  }

  uint32_t n_prep_to_skip =
      (m_testbed_mode == ETestbedMode::Nerf)
          ? tcnn::clamp(m_canonical_training_step / 16u, 1u, 16u)
          : 1u;
  if (m_canonical_training_step % n_prep_to_skip == 0) {
    auto start = std::chrono::steady_clock::now();
    ScopeGuard timing_guard{[&]() {
      m_training_prep_ms.update(std::chrono::duration<float, std::milli>(
                                    std::chrono::steady_clock::now() - start)
                                    .count() /
                                n_prep_to_skip);
    }};
    ...
    switch (m_testbed_mode) {
      case ETestbedMode::Nerf:
        ...
        break;
      case ETestbedMode::Sdf:
        training_prep_sdf(batch_size, m_training_stream);
        break;
      case ETestbedMode::Image:
        training_prep_image(batch_size, m_training_stream);
        break;
      case ETestbedMode::Volume:
        training_prep_volume(batch_size, m_training_stream);
        break;
      default:
  ...
  (*leaf_optimizer_config)["optimize_canonical_params"] = m_train_canonical;
  if (current_training_time_frame != 0) {
    (*leaf_optimizer_config)["learning_rate"] =
        (*leaf_optimizer_config)["after_learning_rate"];
  }

  m_trainer->optimizer()->update_hyperparams(m_network_config["optimizer"]);
  if (current_training_time_frame != 0) {
    m_trainer->optimizer()->set_learning_rate(
        (*leaf_optimizer_config)["after_learning_rate"]);
  }
...

  {
    auto start = std::chrono::steady_clock::now();
    ScopeGuard timing_guard{[&]() {
      m_training_ms.update(std::chrono::duration<float, std::milli>(
                               std::chrono::steady_clock::now() - start)
                               .count());
    }};
    ...
    switch (m_testbed_mode) {
      case ETestbedMode::Nerf:
        ...
        break;
      case ETestbedMode::Sdf:
        train_sdf(batch_size, get_loss_scalar, m_training_stream);
        break;
      case ETestbedMode::Image:
        train_image(batch_size, get_loss_scalar, m_training_stream);
        break;
      case ETestbedMode::Volume:
        train_volume(batch_size, get_loss_scalar, m_training_stream);
        break;
      default:
        throw std::runtime_error{"Invalid training mode."};
    }
...
Vector2f Testbed::calc_focal_length(const Vector2i &resolution, int fov_axis,
                                    float zoom) const {
...
__global__ void dlss_prep_kernel(
    ETestbedMode mode, Vector2i resolution, uint32_t sample_index,
    Vector2f focal_length, Vector2f screen_center, Vector3f parallax_shift,
    bool snap_to_pixel_centers, float *depth_buffer, Matrix<float, 3, 4> camera,
    Matrix<float, 3, 4> prev_camera, cudaSurfaceObject_t depth_surface,
    cudaSurfaceObject_t mvec_surface, cudaSurfaceObject_t exposure_surface,
    CameraDistortion camera_distortion, const float view_dist,
    const float prev_view_dist, const Vector2f image_pos,
    const Vector2f prev_image_pos, const Vector2i image_resolution) {
  ...
  Vector2f mvec =
      mode == ETestbedMode::Image
          ? motion_vector_2d(sample_index, {x, y}, resolution, image_resolution,
                             screen_center, view_dist, prev_view_dist,
                             image_pos, prev_image_pos, snap_to_pixel_centers)
          : motion_vector_3d(sample_index, {x, y}, resolution, focal_length,
                             camera, prev_camera, screen_center, parallax_shift,
                             snap_to_pixel_centers, depth, camera_distortion);

  surf2Dwrite(make_float2(mvec.x(), mvec.y()), mvec_surface,
              x_orig * sizeof(float2), y_orig);

  // Scale depth buffer to be guaranteed in [0,1].
  surf2Dwrite(std::min(std::max(depth / 128.0f, 0.0f), 1.0f), depth_surface,
              x_orig * sizeof(float), y_orig);

  // First thread write an exposure factor of 1. Since DLSS will run on
  // tonemapped data, exposure is assumed to already have been applied to DLSS'
  // inputs.
...
void Testbed::render_frame(const Matrix<float, 3, 4> &camera_matrix0,
                           ...
                           CudaRenderBuffer &render_buffer, bool to_srgb) {
  ...
  Vector2f focal_length =
      calc_focal_length(render_buffer.in_resolution(), m_fov_axis, m_zoom);
  Vector2f screen_center = render_screen_center();
    ...
    case ETestbedMode::Nerf:
      if (!m_render_ground_truth) {
        render_nerf(render_buffer, max_res, focal_length, camera_matrix0,
                    camera_matrix1, nerf_rolling_shutter, screen_center,
                    m_inference_stream);
      }
      break;
    case ETestbedMode::Sdf: {
      if (m_render_ground_truth &&
          m_sdf.groundtruth_mode == ESDFGroundTruthMode::SDFBricks) {
        if (m_sdf.brick_data.size() == 0) {
          tlog::info() << "Building voxel brick positions for "
                       << m_sdf.triangle_octree->n_dual_nodes()
                       << " dual nodes.";
          m_sdf.brick_res = 5;
          std::vector<Eigen::Vector3f> positions =
              m_sdf.triangle_octree->build_brick_voxel_position_list(
                  m_sdf.brick_res);
          ...
          tlog::info() << positions_gpu.size()
                       << " voxel brick positions. Computing SDFs.";
          m_sdf.triangle_bvh->signed_distance_gpu(
              positions.size(),
              EMeshSdfMode::Watertight,  // m_sdf.mesh_sdf_mode, // watertight
                                         // seems to be the best method for 'one
                                         // off' SDF signing
              positions_gpu.data(), m_sdf.brick_data.data(),
              m_sdf.triangles_gpu.data(), false, m_inference_stream);
        }
      }
      distance_fun_t distance_fun = m_render_ground_truth
          ? (distance_fun_t)[&](
                uint32_t n_elements, const GPUMemory<Vector3f> &positions,
                GPUMemory<float> &distances, cudaStream_t stream) {
        if (n_elements == 0) {
          return;
        }
        if (m_sdf.groundtruth_mode == ESDFGroundTruthMode::SDFBricks) {
          ...
          // 	std::max(1u,std::min(m_sdf.triangle_octree->depth(),
          // m_sdf.brick_level)), 	m_sdf.brick_data.data(), 	m_sdf.brick_res,
          // 	m_sdf.brick_quantise_bits
          // );
        } else {
          m_sdf.triangle_bvh->signed_distance_gpu(
              n_elements, m_sdf.mesh_sdf_mode, (Vector3f *)positions.data(),
              distances.data(), m_sdf.triangles_gpu.data(), false,
              m_training_stream);
        }
        ...
        GPUMatrix<float> positions_matrix((float *)positions.data(), 3,
                                          n_elements);
      ...
      render_sdf(distance_fun, normals_fun, render_buffer, max_res,
                 focal_length, camera_matrix0, screen_center,
                 m_inference_stream);
    } break;
    case ETestbedMode::Image:
      ...
      break;
    case ETestbedMode::Volume:
      render_volume(render_buffer, focal_length, camera_matrix0, screen_center,
                    m_inference_stream);
      break;
    default:
  ...
  if (render_buffer.dlss()) {
    ...
    bool distortion = (m_testbed_mode == ETestbedMode::Nerf) &&
                      m_nerf.render_with_camera_distortion;

    const dim3 threads = {16, 8, 1};
    const dim3 blocks = {div_round_up((uint32_t)res.x(), threads.x),
                         div_round_up((uint32_t)res.y(), threads.y), 1};
    ...
    dlss_prep_kernel<<<blocks, threads, 0, m_inference_stream>>>(
        m_testbed_mode, res, render_buffer.spp(), focal_length, screen_center,
        parallax_shift, m_snap_to_pixel_centers, render_buffer.depth_buffer(),
        camera_matrix0, m_prev_camera, render_buffer.dlss()->depth(),
        render_buffer.dlss()->mvec(), render_buffer.dlss()->exposure(),
        distortion ? m_nerf.render_distortion : CameraDistortion{}, m_scale,
        m_prev_scale, m_image.pos, m_image.prev_pos, m_image.resolution);
  ...
  render_buffer.tonemap(m_exposure, m_background_color,
  ...
  if (m_testbed_mode == ETestbedMode::Nerf) {
    ...
    if (m_render_ground_truth) {
      float alpha = 1.f;
      auto const &metadata =
          m_nerf.training.dataset.metadata[m_nerf.training.view];
      render_buffer.overlay_image(
          alpha,
          Array3f::Constant(m_exposure) +
              m_nerf.training.cam_exposure[m_nerf.training.view].variable(),
          m_background_color, to_srgb ? EColorSpace::SRGB : EColorSpace::Linear,
          metadata.pixels, metadata.image_data_type, metadata.resolution,
          m_fov_axis, m_zoom, Vector2f::Constant(0.5f), m_inference_stream);
    }
    ...
    if (m_nerf.training.render_error_overlay) {
      ...
      const float *aligned_err_data_s =
          (const float *)(((size_t)err_data) & ~15);
      const float *aligned_err_data_e =
          (const float *)(((size_t)(err_data + emap_size)) & ~15);
      size_t reduce_size = aligned_err_data_e - aligned_err_data_s;
      reduce_sum(
          aligned_err_data_s,
          [reduce_size] __device__(float val) {
            return max(val, 0.f) / (reduce_size);
          },
          average_error.data(), reduce_size, m_inference_stream);
      auto const &metadata =
          m_nerf.training.dataset.metadata[m_nerf.training.view];
      render_buffer.overlay_false_color(
          metadata.resolution, to_srgb, m_fov_axis, m_inference_stream,
          err_data, error_map_res, average_error.data(),
          m_nerf.training.error_overlay_brightness, m_render_ground_truth);
    }
  }
...
void Testbed::determine_autofocus_target_from_pixel(
    const Vector2i &focus_pixel) {
  float depth;

  const auto &surface = m_render_surfaces.front();
  if (surface.depth_buffer()) {
    auto res = surface.in_resolution();
    Vector2i depth_pixel = focus_pixel.cast<float>()
                               .cwiseProduct(res.cast<float>())
                               .cwiseQuotient(m_window_res.cast<float>())
                               .cast<int>();
    depth_pixel = depth_pixel.cwiseMin(res).cwiseMax(0);

    CUDA_CHECK_THROW(cudaMemcpy(
        &depth,
        surface.depth_buffer() + depth_pixel.x() + depth_pixel.y() * res.x(),
  ...
  auto ray =
      pixel_to_ray_pinhole(0, focus_pixel, m_window_res,
                           calc_focal_length(m_window_res, m_fov_axis, m_zoom),
                           m_smoothed_camera, render_screen_center());

  m_autofocus_target = ray.o + ray.d * depth;
  m_autofocus =
      true;  // If someone shift-clicked, that means they want the AUTOFOCUS
}

void Testbed::autofocus() {
  float new_slice_plane_z =
      std::max(view_dir().dot(m_autofocus_target - view_pos()), 0.1f) - m_scale;
...
void Testbed::gather_histograms() {
  ...
  if (hg_enc && m_trainer->params()) {
    ...
    CUDA_CHECK_THROW(
        cudaMemcpyAsync(grid.data(), m_trainer->params() + first_encoder,
                        grid.size() * sizeof(float), cudaMemcpyDeviceToHost,
                        m_training_stream));
    CUDA_CHECK_THROW(cudaMemcpyAsync(first_layer_rm.data(), m_trainer->params(),
    ...
    for (int l = 0; l < m_num_levels; ++l) {
      m_level_stats[l] =
          compute_level_stats(grid.data() + hg_enc->level_params_offset(l),
                              hg_enc->level_n_params(l));
    }
    ...
    if (m_histo_level < m_num_levels) {
      ...
      float scale = 128.f / (m_histo_scale);  // fixed scale for now to make it
                                              // more comparable between levels
...
void Testbed::save_transform(const std::string &filepath_string) {
  std::FILE *fp = fopen(filepath_string.c_str(), "w");

  tcnn::GPUMemory<precision_t> save_accumulated_rotation;
  tcnn::GPUMemory<precision_t> save_accumulated_transition;

  save_accumulated_rotation.resize(sizeof(precision_t) *
                                   m_nerf_network->rotation()->n_params());
  save_accumulated_transition.resize(sizeof(precision_t) *
                                     m_nerf_network->transition()->n_params());

  tcnn::linear_kernel(save_global_movement_rotation_6d_kernel<precision_t>, 0,
                      m_training_stream, 1u,
                      m_nerf_network->delta_network()->rotation()->params(),
                      m_nerf_network->delta_network()->transition()->params(),
                      m_nerf_network->rotation()->params(),
                      m_nerf_network->transition()->params(),
                      save_accumulated_rotation.data(),
                      save_accumulated_transition.data());

  std::vector<precision_t> rotation_quat(9);
  std::vector<precision_t> transition(3);
  CUDA_CHECK_THROW(
      cudaMemcpy(rotation_quat.data(), save_accumulated_rotation.data(),
                 (9) * sizeof(precision_t), cudaMemcpyDeviceToHost));
  CUDA_CHECK_THROW(
      cudaMemcpy(transition.data(), save_accumulated_transition.data(),
                 (3) * sizeof(precision_t), cudaMemcpyDeviceToHost));
  fprintf(fp, "%f %f %f\n", (float)rotation_quat[0], (float)rotation_quat[1],
          (float)rotation_quat[2]);
  fprintf(fp, "%f %f %f\n", (float)rotation_quat[3], (float)rotation_quat[4],
          (float)rotation_quat[5]);
  fprintf(fp, "%f %f %f\n", (float)rotation_quat[6], (float)rotation_quat[7],
          (float)rotation_quat[8]);
  fprintf(fp, "%f %f %f\n\n", (float)transition[0], (float)transition[1],
          (float)transition[2]);
  fclose(fp);
}

void Testbed::save_snapshot(const std::string &filepath_string,
  ...
  m_nerf_network->save_global_movement(m_inference_stream, m_network_config);
  m_nerf_network->save_local_movement(m_inference_stream, m_network_config);
  ...
  if (m_testbed_mode == ETestbedMode::Nerf) {
    ...
    parallel_for_gpu(
        ...
        [density_grid = m_nerf.density_grid.data(),
         density_grid_fp16 = density_grid_fp16.data()] __device__(size_t i) {
  ...
  if ((m_testbed_mode == ETestbedMode::Nerf)) {
    m_network_config["snapshot"]["nerf"]["rgb"]["rays_per_batch"] =
        m_nerf.training.counters_rgb.rays_per_batch;
    m_network_config["snapshot"]["nerf"]["rgb"]["measured_batch_size"] =
        m_nerf.training.counters_rgb.measured_batch_size;
    m_network_config["snapshot"]["nerf"]["rgb"]
                    ["measured_batch_size_before_compaction"] =
                        m_nerf.training.counters_rgb
                            .measured_batch_size_before_compaction;
    m_network_config["snapshot"]["nerf"]["dataset"] = m_nerf.training.dataset;
  }

  m_network_config_path = filepath;
  std::ofstream f(m_network_config_path.str(),
                  std::ios::out | std::ios::binary);
  json::to_msgpack(m_network_config, f);
}

void Testbed::save_snapshot_incremental(const std::string &filepath_string,
                                        bool include_optimizer_state) {
  fs::path filepath = filepath_string;
  m_network_config["snapshot"] = m_trainer->serialize(include_optimizer_state);
  // does not save snapshot of local transformation (a.k.a delta_network)
...
void Testbed::load_snapshot(const std::string &filepath_string) {
  ...
  if (!config.contains("snapshot")) {
    throw std::runtime_error{std::string{"File '"} + filepath_string +
  ...
  if ((m_testbed_mode == ETestbedMode::Nerf)) {
    ...
    m_nerf.training.counters_rgb.rays_per_batch =
        snapshot["nerf"]["rgb"]["rays_per_batch"];
    m_nerf.training.counters_rgb.measured_batch_size =
        snapshot["nerf"]["rgb"]["measured_batch_size"];
    m_nerf.training.counters_rgb.measured_batch_size_before_compaction =
    ...
    if (m_nerf.density_grid.size() == NERF_GRIDSIZE() * NERF_GRIDSIZE() *
                                          NERF_GRIDSIZE() *
                                          (m_nerf.max_cascade + 1)) {
      update_density_grid_mean_and_bitfield(nullptr);
    } else if (m_nerf.density_grid.size() != 0) {
      // A size of 0 indicates that the density grid was never populated, which
      // is a valid state of a (yet) untrained model.
  ...
  m_nerf_network->load_global_movement(m_network_config);
  m_nerf_network->load_local_movement(m_network_config);
}

void Testbed::load_snapshot_incremental(const std::string &filepath_string) {
  if ((m_testbed_mode == ETestbedMode::Nerf)) {
    m_nerf.training.counters_rgb.rays_per_batch =
        m_network_config["snapshot"]["nerf"]["rgb"]["rays_per_batch"];
    m_nerf.training.counters_rgb.measured_batch_size =
