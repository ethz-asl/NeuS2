...
class Gas {
 public:
  Gas(const GPUMemory<Triangle> &triangles, OptixDeviceContext optix,
      cudaStream_t stream) {
    ...
    OPTIX_CHECK_THROW(optixAccelComputeMemoryUsage(
        optix, &accel_options, &triangle_input, 1, &gas_buffer_sizes));
    ...
    OPTIX_CHECK_THROW(optixAccelBuild(
        optix, stream, &accel_options, &triangle_input,
        1,  // num build inputs
  ...
  OptixTraversableHandle handle() const { return m_gas_handle; }
...
__global__ void signed_distance_watertight_kernel(
    ...
    const Triangle *__restrict__ triangles, float *__restrict__ distances,
...
__global__ void unsigned_distance_kernel(
    ...
    const Triangle *__restrict__ triangles, float *__restrict__ distances,
    bool use_existing_distances_as_upper_bounds = false);
__global__ void raytrace_kernel(uint32_t n_elements,
                                Vector3f *__restrict__ positions,
                                Vector3f *__restrict__ directions,
...
class TriangleBvhWithBranchingFactor : public TriangleBvh {
 public:
  __host__ __device__ static std::pair<int, float> ray_intersect(
      const Vector3f &ro, const Vector3f &rd,
      const TriangleBvhNode *__restrict__ bvhnodes,
      const Triangle *__restrict__ triangles) {
    ...
    while (!query_stack.empty()) {
      ...
      if (node.left_idx < 0) {
        ...
        for (uint32_t i = 0; i < BRANCHING_FACTOR; ++i) {
          children[i] = {bvhnodes[i + first_child].bb.ray_intersect(ro, rd).x(),
                         i + first_child};
        }
  ...
  __host__ __device__ static std::pair<int, float> closest_triangle(
      const Vector3f &point, const TriangleBvhNode *__restrict__ bvhnodes,
      const Triangle *__restrict__ triangles,
  ...
  __host__ __device__ static Vector3f avg_normal_around_point(
      const Vector3f &point, const TriangleBvhNode *__restrict__ bvhnodes,
      const Triangle *__restrict__ triangles) {
    FixedIntStack query_stack;
    query_stack.push(0);
  ...
  __host__ __device__ static float unsigned_distance(
      const Vector3f &point, const TriangleBvhNode *__restrict__ bvhnodes,
      const Triangle *__restrict__ triangles,
      float max_distance_sq = MAX_DIST_SQ) {
  ...
  __host__ __device__ static float signed_distance_watertight(
      const Vector3f &point, const TriangleBvhNode *__restrict__ bvhnodes,
      const Triangle *__restrict__ triangles,
      float max_distance_sq = MAX_DIST_SQ) {
    ...
    Vector3f avg_normal =
        avg_normal_around_point(closest_point, bvhnodes, triangles);

    return std::copysignf(p.second, avg_normal.dot(point - closest_point));
  }

  __host__ __device__ static float signed_distance_raystab(
      const Vector3f &point, const TriangleBvhNode *__restrict__ bvhnodes,
      const Triangle *__restrict__ triangles,
      float max_distance_sq = MAX_DIST_SQ, default_rng_t rng = {}) {
    float distance =
  ...
  // Assumes that "point" is a location on a triangle
  Vector3f avg_normal_around_point(
      const Vector3f &point, const Triangle *__restrict__ triangles) const {
    return avg_normal_around_point(point, m_nodes.data(), triangles);
  }

  float signed_distance(EMeshSdfMode mode, const Vector3f &point,
                        const std::vector<Triangle> &triangles) const {
    if (mode == EMeshSdfMode::Watertight) {
      return signed_distance_watertight(point, m_nodes.data(),
                                        triangles.data());
    } else {
  ...
  void signed_distance_gpu(uint32_t n_elements, EMeshSdfMode mode,
                           const Vector3f *gpu_positions, float *gpu_distances,
                           const Triangle *gpu_triangles,
                           bool use_existing_distances_as_upper_bounds,
                           cudaStream_t stream) override {
    if (mode == EMeshSdfMode::Watertight) {
      linear_kernel(signed_distance_watertight_kernel, 0, stream, n_elements,
                    gpu_positions, m_nodes_gpu.data(), gpu_triangles,
                    gpu_distances, use_existing_distances_as_upper_bounds);
    } else {
#ifdef NGP_OPTIX
      if (m_optix.available) {
        linear_kernel(unsigned_distance_kernel, 0, stream, n_elements,
                      gpu_positions, m_nodes_gpu.data(), gpu_triangles,
                      gpu_distances, use_existing_distances_as_upper_bounds);

        if (mode == EMeshSdfMode::Raystab) {
          m_optix.raystab->invoke(
              {gpu_positions, gpu_distances, m_optix.gas->handle()},
              {n_elements, 1, 1}, stream);
        } else if (mode == EMeshSdfMode::PathEscape) {
          m_optix.pathescape->invoke({gpu_positions, gpu_triangles,
                                      gpu_distances, m_optix.gas->handle()},
                                     {n_elements, 1, 1}, stream);
        }
      } else
#endif  // NGP_OPTIX
      {
        if (mode == EMeshSdfMode::Raystab) {
          linear_kernel(signed_distance_raystab_kernel, 0, stream, n_elements,
                        gpu_positions, m_nodes_gpu.data(), gpu_triangles,
                        gpu_distances, use_existing_distances_as_upper_bounds);
        } else if (mode == EMeshSdfMode::PathEscape) {
  ...
  void ray_trace_gpu(uint32_t n_elements, Vector3f *gpu_positions,
                     Vector3f *gpu_directions, const Triangle *gpu_triangles,
                     cudaStream_t stream) override {
#ifdef NGP_OPTIX
    if (m_optix.available) {
...
#endif  // NGP_OPTIX
    {
      linear_kernel(raytrace_kernel, 0, stream, n_elements, gpu_positions,
                    gpu_directions, m_nodes_gpu.data(), gpu_triangles);
    }
  }

  bool touches_triangle(const BoundingBox &bb, const TriangleBvhNode &node,
  ...
  bool touches_triangle(const BoundingBox &bb,
                        const Triangle *__restrict__ triangles) const override {
  ...
  void build_optix(const GPUMemory<Triangle> &triangles,
                   cudaStream_t stream) override {
#ifdef NGP_OPTIX
    ...
    if (m_optix.available) {
      ...
      m_optix.raystab = std::make_unique<optix::Program<Raystab>>(
          (const char *)optix_ptx::raystab_ptx, sizeof(optix_ptx::raystab_ptx),
          g_optix);
      m_optix.raytrace = std::make_unique<optix::Program<Raytrace>>(
          (const char *)optix_ptx::raytrace_ptx,
          sizeof(optix_ptx::raytrace_ptx), g_optix);
      m_optix.pathescape = std::make_unique<optix::Program<PathEscape>>(
          (const char *)optix_ptx::pathescape_ptx,
          sizeof(optix_ptx::pathescape_ptx), g_optix);
...
__global__ void signed_distance_watertight_kernel(
  ...
  float max_distance =
      use_existing_distances_as_upper_bounds ? distances[i] : MAX_DIST;
  distances[i] = TriangleBvh4::signed_distance_watertight(
      positions[i], bvhnodes, triangles, max_distance * max_distance);
}
...
__global__ void signed_distance_raystab_kernel(
  ...
  distances[i] = TriangleBvh4::signed_distance_raystab(
      positions[i], bvhnodes, triangles, max_distance * max_distance, rng);
}
...
__global__ void unsigned_distance_kernel(
  ...
  float max_distance =
      use_existing_distances_as_upper_bounds ? distances[i] : MAX_DIST;
  distances[i] = TriangleBvh4::unsigned_distance(
      positions[i], bvhnodes, triangles, max_distance * max_distance);
}
