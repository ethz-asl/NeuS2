...
void Testbed::Nerf::Training::set_image(int frame_idx,
  ...
  dataset.set_training_image(frame_idx, {img_buf.shape[1], img_buf.shape[0]},
                             (const void *)img_buf.ptr,
                             (const float *)depth_buf.ptr, depth_scale, false,
                             EImageDataType::Float, EDepthDataType::Float);
}

void Testbed::override_sdf_training_data(py::array_t<float> points,
  ...
  for (size_t i = 0; i < points_cpu.size(); ++i) {
    ...
    pos = (pos - m_raw_aabb.min) / m_sdf.mesh_scale +
          0.5f * (Vector3f::Ones() -
                  (m_raw_aabb.max - m_raw_aabb.min) / m_sdf.mesh_scale);
  ...
  CUDA_CHECK_THROW(
      cudaMemcpyAsync(m_sdf.training.positions.data(), points_cpu.data(),
                      points_buf.shape[0] * points_buf.shape[1] * sizeof(float),
                      cudaMemcpyHostToDevice, m_training_stream));
...
pybind11::dict Testbed::compute_marching_cubes_mesh(Eigen::Vector3i res3d,
                                                    BoundingBox aabb,
                                                    float thresh) {
  if (aabb.is_empty()) {
  ...
  CUDA_CHECK_THROW(cudaMemcpy(
      cpunormals.request().ptr, m_mesh.vert_normals.data(),
      m_mesh.vert_normals.size() * 3 * sizeof(float), cudaMemcpyDeviceToHost));
  CUDA_CHECK_THROW(cudaMemcpy(
      cpucolors.request().ptr, m_mesh.vert_colors.data(),
      m_mesh.vert_colors.size() * 3 * sizeof(float), cudaMemcpyDeviceToHost));
  CUDA_CHECK_THROW(cudaMemcpy(cpuindices.request().ptr, m_mesh.indices.data(),
                              m_mesh.indices.size() * sizeof(int),
                              cudaMemcpyDeviceToHost));
...
py::array_t<float> Testbed::render_to_cpu(int width, int height, int spp,
  ...
  for (int i = 0; i < spp; ++i) {
    ...
    auto sample_start_cam_matrix =
        log_space_lerp(start_cam_matrix, end_cam_matrix, start_alpha);
    auto sample_end_cam_matrix =
        log_space_lerp(start_cam_matrix, end_cam_matrix, end_alpha);
    ...
    render_frame(sample_start_cam_matrix, sample_end_cam_matrix,
                 Eigen::Vector4f::Zero(), m_windowless_render_surface, !linear);
  }
  ...
  CUDA_CHECK_THROW(cudaMemcpy2DFromArray(
      buf.ptr, width * sizeof(float) * 4,
      m_windowless_render_surface.surface_provider().array(), 0, 0,
      width * sizeof(float) * 4, height, cudaMemcpyDeviceToHost));
  return result;
}

py::array_t<float> Testbed::render_with_rolling_shutter_to_cpu(
  ...
  for (int i = 0; i < spp; ++i) {
    ...
    render_frame(
        ...
        rolling_shutter, m_windowless_render_surface, !linear);
  }
  py::array_t<float> result({height, width, 4});
  py::buffer_info buf = result.request();
  CUDA_CHECK_THROW(cudaMemcpy2DFromArray(
...
py::array_t<float> Testbed::screenshot(bool linear) const {
  ...
  glReadPixels(0, 0, m_window_res.x(), m_window_res.y(), GL_RGBA, GL_FLOAT,
               tmp.data());
  ...
  pool.parallelFor<size_t>(0, m_window_res.y(), [&](size_t y) {
    ...
    for (uint32_t x = 0; x < m_window_res.x(); ++x) {
      ...
      data[px_reverse * 4 + 0] =
          linear ? srgb_to_linear(tmp[px * 4 + 0]) : tmp[px * 4 + 0];
      data[px_reverse * 4 + 1] =
          linear ? srgb_to_linear(tmp[px * 4 + 1]) : tmp[px * 4 + 1];
      data[px_reverse * 4 + 2] =
          linear ? srgb_to_linear(tmp[px * 4 + 2]) : tmp[px * 4 + 2];
  ...
  throw std::runtime_error{
      "testbed.screenshot() in only supported when compiling with NGP_GUI."};
#endif
}
...
PYBIND11_MODULE(pyngp, m) {
  ...
  py::class_<BoundingBox>(m, "BoundingBox")
      ...
      .def("intersects", py::overload_cast<const BoundingBox &>(
                             &BoundingBox::intersects, py::const_))
  ...
  testbed.def(py::init<ETestbedMode>())
      .def(py::init<ETestbedMode, const std::string &, const std::string &>())
      .def(py::init<ETestbedMode, const std::string &, const json &>())
      .def("create_empty_nerf_dataset", &Testbed::create_empty_nerf_dataset,
           "Allocate memory for a nerf dataset with a given size",
           py::arg("n_images"), py::arg("aabb_scale") = 1,
           py::arg("is_hdr") = false)
      .def("load_training_data", &Testbed::load_training_data,
           py::call_guard<py::gil_scoped_release>(),
           "Load training data from a given path.")
      .def("clear_training_data", &Testbed::clear_training_data,
           "Clears training data to free up GPU memory.")
      // General control
      .def("init_window", &Testbed::init_window,
           "Init a GLFW window that shows real-time progress and a GUI.",
           py::arg("width"), py::arg("height"), py::arg("hidden") = false)
      .def("want_repl", &Testbed::want_repl,
           "returns true if the user clicked the 'I want a repl' button")
      .def(
          "frame", &Testbed::frame, py::call_guard<py::gil_scoped_release>(),
          "Process a single frame. Renders if a window was previously created.")
      .def("render", &Testbed::render_to_cpu,
           "Renders an image at the requested resolution. Does not require a "
           "window.",
           py::arg("width") = 1920, py::arg("height") = 1080,
           py::arg("spp") = 1, py::arg("linear") = true,
      ...
      .def("render_with_rolling_shutter",
           ...
           "Renders an image at the requested resolution. Does not require a "
           "window. Supports rolling shutter, with per ray time being computed "
           "as A+B*u+C*v+D*t for [A,B,C,D]",
           py::arg("transform_matrix_start"), py::arg("transform_matrix_end"),
           py::arg("rolling_shutter") = Eigen::Vector4f::Zero(),
           py::arg("width") = 1920, py::arg("height") = 1080,
           py::arg("spp") = 1, py::arg("linear") = true)
      .def("screenshot", &Testbed::screenshot,
           "Takes a screenshot of the current window contents.",
           py::arg("linear") = true)
      .def("destroy_window", &Testbed::destroy_window,
           "Destroy the window again.")
      .def("train", &Testbed::train, py::call_guard<py::gil_scoped_release>(),
      ...
      .def("reload_network_from_file", &Testbed::reload_network_from_file,
           py::arg("path") = "", py::arg("output_path") = "",
           "Reload the network from a config file.")
      .def("reload_network_from_json", &Testbed::reload_network_from_json,
           "Reload the network from a json object.")
      .def("override_sdf_training_data", &Testbed::override_sdf_training_data,
           "Override the training data for learning a signed distance function")
      .def("calculate_iou", &Testbed::calculate_iou,
           "Calculate the intersection over union error value",
           py::arg("n_samples") = 128 * 1024 * 1024,
           py::arg("scale_existing_results_factor") = 0.0f,
           py::arg("blocking") = true, py::arg("force_use_octree") = true)
      .def("n_params", &Testbed::n_params, "Number of trainable parameters")
      .def("n_encoding_params", &Testbed::n_encoding_params,
           "Number of trainable parameters in the encoding")
      .def("save_transform", &Testbed::save_transform, py::arg("path"),
           "Save a snapshot of the predicted transformation of currently "
           "trained model")
      .def("save_snapshot", &Testbed::save_snapshot, py::arg("path"),
           py::arg("include_optimizer_state") = false,
           "Save a snapshot of the currently trained model")
      .def("load_snapshot", &Testbed::load_snapshot, py::arg("path"),
           "Load a previously saved snapshot")
      .def("load_camera_path", &Testbed::load_camera_path, "Load a camera path",
           py::arg("path"))
      .def("compute_and_save_png_slices", &Testbed::compute_and_save_png_slices,
           ...
           py::arg("density_range") = 4.f, py::arg("flip_y_and_z_axes") = false,
           "Compute & save a PNG file representing the 3D density or distance "
           "field from the current SDF or NeRF model. ")
      .def("compute_and_save_marching_cubes_mesh",
           &Testbed::compute_and_save_marching_cubes_mesh, py::arg("filename"),
           py::arg("resolution") = Eigen::Vector3i::Constant(256),
           py::arg("aabb") = BoundingBox{},
           py::arg("thresh") = std::numeric_limits<float>::max(),
           py::arg("generate_uvs_for_obj_file") = false,
           "Compute & save a marching cubes mesh from the current SDF or NeRF "
           "model. "
  ...
  testbed.def_readwrite("dynamic_res", &Testbed::m_dynamic_res)
      ...
      .def("reset_camera", &Testbed::reset_camera)
      .def("compute_image_mse", &Testbed::compute_image_mse,
           py::arg("quantize") = false)
      .def("training_network_next_frame", &Testbed::training_network_next_frame)
      .def("change_to_frame", &Testbed::change_to_frame)
      .def("prepare_for_test", &Testbed::prepare_for_test)
      ...
      .def_property_readonly("loss",
                             [](py::object &obj) {
                               return obj.cast<Testbed &>().m_loss_scalar.val();
                             })
      .def_property_readonly(
          "ek_loss",
          [](py::object &obj) {
            return obj.cast<Testbed &>().m_ek_loss_scalar.val();
          })
      .def_property_readonly(
          "mask_loss",
          [](py::object &obj) {
            return obj.cast<Testbed &>().m_mask_loss_scalar.val();
          })
      .def_readonly("training_step", &Testbed::m_training_step)

      // dynamic scene
      .def_readonly("current_training_time_frame",
                    &Testbed::current_training_time_frame)
      .def_readonly("first_frame_max_training_step",
                    &Testbed::first_frame_max_training_step)
      .def_readonly("next_frame_max_training_step",
                    &Testbed::next_frame_max_training_step)
      .def_readonly("all_training_time_frame",
                    &Testbed::all_training_time_frame)
      .def_readonly("all_json_paths", &Testbed::all_json_paths)
      .def_readonly("m_mesh_path", &Testbed::m_mesh_path)
      .def_readonly("m_log_path", &Testbed::m_log_path)
  ...
  nerf.def_readonly("training", &Testbed::Nerf::training)
      ...
      .def_readwrite("rendering_min_transmittance",
                     &Testbed::Nerf::rendering_min_transmittance)
  ...
  nerfdataset.def_readonly("metadata", &NerfDataset::metadata)
      ...
      .def_readonly("from_na", &NerfDataset::from_na)
  ...
  py::class_<Testbed::Nerf::Training>(nerf, "Training")
      ...
      .def_readwrite("optimize_extra_dims",
                     &Testbed::Nerf::Training::optimize_extra_dims)
      ...
      .def_readwrite("optimize_focal_length",
                     &Testbed::Nerf::Training::optimize_focal_length)
      .def_readwrite("n_steps_between_cam_updates",
      ...
      //// use training.dataset.metadata instead
      ...
      .def_readwrite("depth_supervision_lambda",
                     &Testbed::Nerf::Training::depth_supervision_lambda)
      .def_readonly("dataset", &Testbed::Nerf::Training::dataset)
      .def("set_camera_intrinsics",
           &Testbed::Nerf::Training::set_camera_intrinsics,
           py::arg("frame_idx"), py::arg("fx") = 0.f, py::arg("fy") = 0.f,
           py::arg("cx") = -0.5f, py::arg("cy") = -0.5f, py::arg("k1") = 0.f,
           py::arg("k2") = 0.f, py::arg("p1") = 0.f, py::arg("p2") = 0.f,
      ...
      .def("set_camera_extrinsics",
           ...
           py::arg("frame_idx"), py::arg("camera_to_world"),
      ...
      .def("set_image", &Testbed::Nerf::Training::set_image,
           py::arg("frame_idx"), py::arg("img"), py::arg("depth_img"),
           py::arg("depth_scale") = 1.0f,
           "set one of the training images. must be a floating point numpy "
           "array of (H,W,C) with 4 channels; linear color space; W and H must "
           "match image size of the rest of the dataset");
  ...
  py::class_<Testbed::Sdf::Training>(sdf, "Training")
      .def_readwrite("generate_sdf_data_online",
                     &Testbed::Sdf::Training::generate_sdf_data_online)
      .def_readwrite("surface_offset_scale",
                     &Testbed::Sdf::Training::surface_offset_scale);

  py::class_<Testbed::Image> image(testbed, "Image");
  image.def_readonly("training", &Testbed::Image::training)
