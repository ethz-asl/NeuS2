...
__global__ void halton23_kernel(uint32_t n_elements, size_t base_idx,
...
__global__ void sobol2_kernel(uint32_t n_elements, size_t base_idx,
                              uint32_t seed, Vector2f *__restrict__ output) {
  uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n_elements) return;
...
__global__ void zip_kernel(uint32_t n_elements, const float *__restrict__ in,
                           Vector2f *__restrict__ output) {
  uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n_elements) return;
...
__global__ void stratify2_kernel(uint32_t n_elements, uint32_t log2_batch_size,
                                 Vector2f *__restrict__ inout) {
  uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n_elements) return;
...
__global__ void init_image_coords(Vector2f *__restrict__ positions,
                                  Vector2i resolution,
                                  Vector2i image_resolution, float view_dist,
                                  Vector2f image_pos, Vector2f screen_center,
  ...
  positions[idx] = pixel_to_image_uv(sample_index, {x, y}, resolution,
                                     image_resolution, screen_center, view_dist,
                                     image_pos, snap_to_pixel_centers);
}
...
__global__ void colorspace_convert_image_half(
    Vector2i resolution, const char *__restrict__ texture) {
...
__global__ void colorspace_convert_image_float(
    Vector2i resolution, const char *__restrict__ texture) {
  uint32_t x = blockIdx.x * blockDim.x + threadIdx.x;
  uint32_t y = blockIdx.y * blockDim.y + threadIdx.y;
  if (x >= resolution.x() || y >= resolution.y()) return;
...
__global__ void shade_kernel_image(Vector2i resolution,
  ...
  float Y = color.x(), Cb = color.y() * (1.f / CHROMA_SCALE) - 0.f,
        Cr = color.z() * (1.f / CHROMA_SCALE) - 0.f;
...
__global__ void eval_image_kernel_and_snap(uint32_t n_elements,
  ...
  if (snap_to_pixel_centers) {
    Vector2i pos_int = pos.cwiseProduct(resolution.cast<float>())
                           .array()
                           .floor()
                           .matrix()
                           .cast<int>();
    positions[i] = (pos_int.cast<float>() + Vector2f::Constant(0.5f))
    ...
    val = (1 - weight.x()) * (1 - weight.y()) * read_val(idx.x(), idx.y()) +
...
void Testbed::train_image(size_t target_batch_size, bool get_loss_scalar,
                          cudaStream_t stream) {
  ...
  if (m_image.random_mode == ERandomMode::Halton) {
    linear_kernel(halton23_kernel, 0, stream, n_elements,
                  (size_t)batch_size * m_training_step,
    ...
    linear_kernel(sobol2_kernel, 0, stream, n_elements,
                  (size_t)batch_size * m_training_step, m_seed,
                  m_image.training.positions.data());
  } else {
  ...
  if (m_image.type == EDataType::Float) {
    linear_kernel(
        eval_image_kernel_and_snap<float, 3>, 0, stream, n_elements,
        (float *)m_image.data.data(), m_image.training.positions.data(),
        m_image.resolution, (float *)m_image.training.targets.data(),
        m_image.training.snap_to_pixel_centers, m_image.training.linear_colors);
  } else {
    linear_kernel(
        eval_image_kernel_and_snap<__half, 3>, 0, stream, n_elements,
        (__half *)m_image.data.data(), m_image.training.positions.data(),
        m_image.resolution, (float *)m_image.training.targets.data(),
        m_image.training.snap_to_pixel_centers, m_image.training.linear_colors);
  }

  GPUMatrix<float> training_batch_matrix(
      (float *)(m_image.training.positions.data()), n_input_dims, batch_size);
  GPUMatrix<float> training_target_matrix(
      (float *)(m_image.training.targets.data()), n_output_dims, batch_size);
...
void Testbed::render_image(CudaRenderBuffer &render_buffer,
                           cudaStream_t stream) {
  ...
  init_image_coords<<<blocks, threads, 0, stream>>>(
      m_image.render_coords.data(), res, m_image.resolution, m_scale,
      m_image.pos, m_screen_center - Vector2f::Constant(0.5f),
      m_snap_to_pixel_centers, render_buffer.spp());

  // Obtain colors for each 2D coord
  if (m_image.type == EDataType::Float) {
    linear_kernel(eval_image_kernel_and_snap<float, 3>, 0, stream, n_elements,
                  (float *)m_image.data.data(), m_image.render_coords.data(),
                  m_image.resolution, (float *)m_image.render_out.data(),
                  m_image.training.snap_to_pixel_centers,
                  m_image.training.linear_colors);
  } else {
    linear_kernel(eval_image_kernel_and_snap<__half, 3>, 0, stream, n_elements,
                  (__half *)m_image.data.data(), m_image.render_coords.data(),
                  m_image.resolution, (float *)m_image.render_out.data(),
                  m_image.training.snap_to_pixel_centers,
                  m_image.training.linear_colors);
  }
  ...
  if (!m_render_ground_truth) {
    if (m_visualized_dimension >= 0) {
      GPUMatrix<float> positions_matrix((float *)m_image.render_coords.data(),
                                        2, n_elements);
      GPUMatrix<float> colors_matrix((float *)m_image.render_out.data(), 3,
                                     n_elements);
  ...
  shade_kernel_image<<<blocks, threads, 0, stream>>>(
      res, m_image.render_coords.data(), m_image.render_out.data(),
...
void Testbed::load_image() {
  ...
  const dim3 blocks = {
      ...
      div_round_up((uint32_t)m_image.resolution.x(), threads.y), 1};
  ...
  tlog::success() << "Loaded a "
                  << (m_image.type == EDataType::Half ? "half" : "full")
                  << "-precision image with " << m_image.resolution.x() << "x"
                  << m_image.resolution.y() << " pixels.";
}
...
void Testbed::load_exr_image() {
  ...
  GPUMemory<float> image = load_exr(m_data_path.str(), m_image.resolution.x(),
                                    m_image.resolution.y());
  m_image.data.resize(image.size() * sizeof(float));
  CUDA_CHECK_THROW(cudaMemcpy(m_image.data.data(), image.data(),
...
void Testbed::load_stbi_image() {
  ...
  // First step: load an image that we'd like to learn
  GPUMemory<float> image = load_stbi(m_data_path.str(), m_image.resolution.x(),
                                     m_image.resolution.y());
  m_image.data.resize(image.size() * sizeof(float));
  CUDA_CHECK_THROW(cudaMemcpy(m_image.data.data(), image.data(),
                              image.size() * sizeof(float),
...
void Testbed::load_binary_image() {
  ...
  // 		close(fd);
  // 		throw std::runtime_error{std::string{"cuFileHandleRegister fd "}
  ...
  // 	cuFileRead(cf_handle, m_image.data.data(), m_image.data.get_bytes(), 2 *
  // sizeof(int), 0);
  ...
  // 		cuFileHandleDeregister(cf_handle);
  // 		close(fd);
  // 		throw std::runtime_error{std::string{"buffer deregistration
  // failed "} + status.err};
  // 	}
...
__global__ void image_coords_from_idx(const uint32_t n_elements,
                                      uint32_t offset,
                                      Vector2f *__restrict__ pos,
                                      Vector2i resolution) {
  uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n_elements) return;
...
__global__ void image_mse_kernel(const uint32_t n_elements,
                                 const Array3f *__restrict__ target,
                                 const Array3f *__restrict__ prediction,
                                 float *__restrict__ result,
                                 bool quantize_to_byte) {
  uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n_elements) return;

  Array3f pred = prediction[i];
  if (quantize_to_byte) {
    pred = (pred * 255.0f + Array3f::Constant(0.5f))
               .cast<int>()
               .cwiseMax(0)
               .cwiseMin(255)
               .cast<float>() /
...
float Testbed::compute_image_mse(bool quantize_to_byte) {
  ...
  for (uint32_t i = 0; i < n_batches; ++i) {
    ...
    GPUMatrix<float> pos_matrix((float *)(pos.data()), n_input_dims,
                                batch_size);
    GPUMatrix<float> targets_matrix((float *)(targets.data()), n_output_dims,
                                    batch_size);
    GPUMatrix<float> predictions_matrix((float *)(predictions.data()),
                                        n_output_dims, batch_size);

    linear_kernel(image_coords_from_idx, 0, nullptr, batch_size, offset,
                  pos.data(), m_image.resolution);

    if (m_image.type == EDataType::Float) {
      linear_kernel(eval_image_kernel_and_snap<float, 3>, 0, nullptr,
                    batch_size, (float *)m_image.data.data(), pos.data(),
                    m_image.resolution, (float *)targets.data(), true,
                    m_image.training.linear_colors);
    } else {
      linear_kernel(eval_image_kernel_and_snap<__half, 3>, 0, nullptr,
                    batch_size, (__half *)m_image.data.data(), pos.data(),
                    m_image.resolution, (float *)targets.data(), true,
                    m_image.training.linear_colors);
    }
    ...
    linear_kernel(image_mse_kernel, 0, nullptr, batch_size, targets.data(),
                  predictions.data(), se.data() + offset, quantize_to_byte);
  }
