...
__device__ Array4f proc_envmap(const Vector3f &dir, const Vector3f &up_dir,
                               const Vector3f &sun_dir, const Array3f &skycol) {
...
__device__ Array4f proc_envmap_render(const Vector3f &dir,
...
__device__ inline bool walk_to_next_event(default_rng_t &rng,
                                          const BoundingBox &aabb,
                                          Vector3f &pos, const Vector3f &dir,
                                          const uint8_t *bitgrid, float scale) {
  while (1) {
    float zeta1 =
        random_val(rng);  // sample a free flight distance and go there!
    float dt =
        -std::log(1.0f - zeta1) *
        scale;  // todo - for spatially varying majorant, we must check dt
                // against the range over which the majorant is defined. we can
                // turn this into an optical thickness accumulating loop...
    pos += dir * dt;
    if (!aabb.contains(pos)) return false;  // escape to the mooon!
    uint32_t bitidx =
        tcnn::morton3D(int(pos.x() * 128.f + 0.5f), int(pos.y() * 128.f + 0.5f),
...
__global__ void volume_generate_training_data_kernel(
    uint32_t n_elements, Vector3f *pos_out, Array4f *target_out,
    const void *nanovdb, const uint8_t *bitgrid, Vector3f world2index_offset,
    float world2index_scale, BoundingBox aabb, default_rng_t rng, float albedo,
    float scattering, float distance_scale, float global_majorant,
    Vector3f up_dir, Vector3f sun_dir, Array3f sky_col) {
  ...
  const nanovdb::FloatGrid *grid =
      reinterpret_cast<const nanovdb::FloatGrid *>(nanovdb);
  ...
  while (numout < MAX_TRAIN_VERTICES) {
    ...
    for (int iter = 0; iter < 128; ++iter) {
      ...
      if (zeta2 >= extinction_prob) continue;  // null collision
...
void Testbed::train_volume(size_t target_batch_size, bool get_loss_scalar,
                           cudaStream_t stream) {
  ...
  linear_kernel(volume_generate_training_data_kernel, 0, stream,
                n_elements / MAX_TRAIN_VERTICES,
                m_volume.training.positions.data(),
                m_volume.training.targets.data(), m_volume.nanovdb_grid.data(),
                m_volume.bitgrid.data(), m_volume.world2index_offset,
                m_volume.world2index_scale, m_render_aabb, m_rng,
                m_volume.albedo, m_volume.scattering, distance_scale,
                m_volume.global_majorant, m_up_dir, m_sun_dir, sky_col);
  m_rng.advance(n_elements * 256);

  GPUMatrix<float> training_batch_matrix(
      (float *)(m_volume.training.positions.data()), n_input_dims, batch_size);
  GPUMatrix<float> training_target_matrix(
      (float *)(m_volume.training.targets.data()), n_output_dims, batch_size);
...
__global__ void init_rays_volume(
    ...
    Testbed::VolPayload *__restrict__ payloads, uint32_t *pixel_counter,
    ...
    float plane_z, float dof, const float *__restrict__ envmap_data,
    const Vector2i envmap_resolution, Array4f *__restrict__ framebuffer,
    float *__restrict__ depthbuffer, default_rng_t rng, const uint8_t *bitgrid,
    float distance_scale, float global_majorant, Vector3f up_dir,
    Vector3f sun_dir, Array3f sky_col) {
  ...
  float scale = distance_scale / global_majorant;
  if (t >= box_intersection.y() ||
      !walk_to_next_event(rng, aabb, ray.o, ray.d, bitgrid, scale)) {
...
__global__ void volume_render_kernel_gt(
    uint32_t n_pixels, Vector2i resolution, default_rng_t rng, BoundingBox aabb,
    const Vector3f *__restrict__ positions_in,
    const Testbed::VolPayload *__restrict__ payloads_in,
    const uint32_t *pixel_counter_in, const Vector3f up_dir,
    const Vector3f sun_dir, const Array3f sky_col, const void *nanovdb,
    const uint8_t *bitgrid, float global_majorant, Vector3f world2index_offset,
    float world2index_scale, float distance_scale, float albedo,
    float scattering, Array4f *__restrict__ framebuffer,
    float *__restrict__ depthbuffer) {
  uint32_t idx = threadIdx.x + blockDim.x * blockIdx.x;
  if (idx >= n_pixels || idx >= pixel_counter_in[0]) return;
...
__global__ void volume_render_kernel_step(
    ...
    const Vector3f *__restrict__ positions_in,
    const Testbed::VolPayload *__restrict__ payloads_in,
    const uint32_t *pixel_counter_in, Vector3f *__restrict__ positions_out,
    Testbed::VolPayload *__restrict__ payloads_out, uint32_t *pixel_counter_out,
    const Array4f *network_outputs_in, const Vector3f up_dir,
    const Vector3f sun_dir, const Array3f sky_col, const void *nanovdb,
    const uint8_t *bitgrid, float global_majorant, Vector3f world2index_offset,
    float world2index_scale, float distance_scale, float albedo,
    float scattering, Array4f *__restrict__ framebuffer,
    float *__restrict__ depthbuffer, bool force_finish_ray) {
  uint32_t idx = threadIdx.x + blockDim.x * blockIdx.x;
  if (idx >= n_pixels || idx >= pixel_counter_in[0]) return;
...
void Testbed::render_volume(CudaRenderBuffer &render_buffer,
  ...
  init_rays_volume<<<blocks, threads, 0, stream>>>(
      render_buffer.spp(), m_volume.pos[0].data(), m_volume.payload[0].data(),
      m_volume.hit_counter.data(), res, focal_length, camera_matrix,
      screen_center, get_scaled_parallax_shift(), m_snap_to_pixel_centers,
      m_render_aabb, plane_z, m_dof, m_envmap.envmap->params_inference(),
      m_envmap.resolution, render_buffer.frame_buffer(),
      render_buffer.depth_buffer(), m_rng, m_volume.bitgrid.data(),
      distance_scale, m_volume.global_majorant, m_up_dir, m_sun_dir, sky_col);
  m_rng.advance(n_pixels * 256);

  uint32_t n = n_pixels;
  CUDA_CHECK_THROW(cudaDeviceSynchronize());
  cudaMemcpy(&n, m_volume.hit_counter.data(), sizeof(uint32_t),
             cudaMemcpyDeviceToHost);

  if (m_render_ground_truth) {
    linear_kernel(volume_render_kernel_gt, 0, stream, n, res, m_rng,
                  m_render_aabb, m_volume.pos[0].data(),
                  m_volume.payload[0].data(), m_volume.hit_counter.data(),

                  m_up_dir, m_sun_dir, sky_col, m_volume.nanovdb_grid.data(),
                  m_volume.bitgrid.data(), m_volume.global_majorant,
                  m_volume.world2index_offset, m_volume.world2index_scale,
                  distance_scale, std::min(m_volume.albedo, 0.995f),
                  m_volume.scattering, render_buffer.frame_buffer(),
    ...
    for (int iter = 0; iter < max_iter && n > 0; ++iter) {
      ...
      GPUMatrix<float> positions_matrix((float *)m_volume.pos[srcbuf].data(), 3,
                                        n_elements);
      ...
      cudaMemsetAsync(m_volume.hit_counter.data() + dstbuf, 0,
                      sizeof(uint32_t));

      linear_kernel(
          volume_render_kernel_step, 0, stream, n, res, m_rng, m_render_aabb,
          m_volume.pos[srcbuf].data(), m_volume.payload[srcbuf].data(),
          m_volume.hit_counter.data() + srcbuf, m_volume.pos[dstbuf].data(),
          m_volume.payload[dstbuf].data(), m_volume.hit_counter.data() + dstbuf,
          m_volume.radiance_and_density.data(), m_up_dir, m_sun_dir, sky_col,
          m_volume.nanovdb_grid.data(), m_volume.bitgrid.data(),
          m_volume.global_majorant, m_volume.world2index_offset,
          m_volume.world2index_scale, distance_scale,
          std::min(m_volume.albedo, 0.995f), m_volume.scattering,
          render_buffer.frame_buffer(), render_buffer.depth_buffer(),
          (iter >= max_iter - 1));
      m_rng.advance(n_pixels * 256);
      if (((iter + 1) % 4) == 0) {
        // periodically tell the cpu how many pixels are left
        CUDA_CHECK_THROW(
            cudaMemcpyAsync(&n, m_volume.hit_counter.data() + dstbuf,
...
void Testbed::load_volume() {
  ...
  const nanovdb::FloatGrid *grid =
      reinterpret_cast<const nanovdb::FloatGrid *>(cpugrid.data());
  ...
  m_aabb = m_render_aabb = BoundingBox{
      Vector3f{0.5f - xsize * scale * 0.5f, 0.5f - ysize * scale * 0.5f,
  ...
  for (int i = metadata.indexBBox[0][0]; i < metadata.indexBBox[1][0]; ++i)
    for (int j = metadata.indexBBox[0][1]; j < metadata.indexBBox[1][1]; ++j)
      for (int k = metadata.indexBBox[0][2]; k < metadata.indexBBox[1][2];
        ...
        if (d < mn) mn = d;
        if (d > 0.001f) {
          float fx = ((i + 0.5f) - m_volume.world2index_offset.x()) /
                     m_volume.world2index_scale;
          float fy = ((j + 0.5f) - m_volume.world2index_offset.y()) /
                     m_volume.world2index_scale;
          float fz = ((k + 0.5f) - m_volume.world2index_offset.z()) /
                     m_volume.world2index_scale;
          uint32_t bitidx =
              tcnn::morton3D(int(fx * 128.f + 0.5f), int(fy * 128.f + 0.5f),
                             int(fz * 128.f + 0.5f));
