...
__device__ inline Vector3f mix(const Vector3f &a, const Vector3f &b, float t) {
  return a + (b - a) * t;
}
...
__device__ Vector3f evaluate_shading(const Vector3f &base_color,
                                     const Vector3f &ambient_color,  // :)
                                     const Vector3f &light_color,    // :)
                                     float metallic, float subsurface,
                                     float specular, float roughness,
                                     float specular_tint, float sheen,
                                     float sheen_tint, float clearcoat,
                                     float clearcoat_gloss, Vector3f L,
                                     Vector3f V, Vector3f N) {
  float NdotL = N.dot(L);
  float NdotV = N.dot(V);
  ...
  Vector3f Cspec0 = mix(
      mix(Vector3f(1.0f, 1.0f, 1.0f), Ctint, specular_tint) * specular * 0.08f,
      base_color, metallic);
...
__global__ void advance_pos_kernel_sdf(
    const uint32_t n_elements, const float zero_offset,
    Vector3f *__restrict__ positions, float *__restrict__ distances,
    SdfPayload *__restrict__ payloads, BoundingBox aabb, float floor_y,
    const TriangleOctreeNode *__restrict__ octree_nodes, int max_depth,
    float distance_scale, float maximum_distance, float k,
    float *__restrict__ prev_distances, float *__restrict__ total_distances,
  ...
  if (octree_nodes && !TriangleOctree::contains(octree_nodes, max_depth, pos)) {
    float octree_distance = (TriangleOctree::ray_intersect(
                                 octree_nodes, max_depth, pos, payload.dir) +
                             1e-6f);
...
__global__ void perturb_sdf_samples(uint32_t n_elements,
...
__global__ void prepare_shadow_rays(
    const uint32_t n_elements, Vector3f sun_dir,
    Vector3f *__restrict__ positions, Vector3f *__restrict__ normals,
    float *__restrict__ distances, float *__restrict__ prev_distances,
    float *__restrict__ total_distances, float *__restrict__ min_visibility,
    SdfPayload *__restrict__ payloads, BoundingBox aabb,
    const TriangleOctreeNode *__restrict__ octree_nodes, int max_depth) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
  ...
  if (octree_nodes &&
      !TriangleOctree::contains(octree_nodes, max_depth, view_pos)) {
    t = fmaxf(0.0f, TriangleOctree::ray_intersect(octree_nodes, max_depth,
                                                  view_pos, dir) +
                        1e-6f);
    view_pos += t * dir;
  }
...
__global__ void shade_kernel_sdf(
    const uint32_t n_elements, BoundingBox aabb, float floor_y,
    const ERenderMode mode, const BRDFParams brdf, Vector3f sun_dir,
    Vector3f up_dir, Matrix<float, 3, 4> camera_matrix,
    Vector3f *__restrict__ positions, Vector3f *__restrict__ normals,
    float *__restrict__ distances, SdfPayload *__restrict__ payloads,
  ...
  switch (mode) {
    case ERenderMode::AO:
      color = Array3f::Constant(powf(0.92f, payload.n_steps));
      break;
    case ERenderMode::Shade: {
      float skyam = normal.dot(up_dir) * -0.5f + 0.5f;
      Vector3f suncol =
          Array3f{255.f / 255.0f, 225.f / 255.0f, 195.f / 255.0f} * 4.f *
          distances[i];  // Distance encodes shadow occlusion. 0=occluded, 1=no
                         // shadow
      ...
      Vector3f col = evaluate_shading(
          ...
          brdf.ambientcolor.array() * skycol.array(), suncol,
          floor ? 0.f : brdf.metallic, floor ? 0.f : brdf.subsurface,
          floor ? 1.f : brdf.specular, floor ? 0.5f : brdf.roughness, 0.f,
          floor ? 0.f : brdf.sheen, 0.f, floor ? 0.f : brdf.clearcoat,
          brdf.clearcoat_gloss, sun_dir, -payload.dir.normalized(), normal);
      color = col.array();
    } break;
    case ERenderMode::Depth:
      color = Array3f::Constant(cam_fwd.dot(pos - cam_pos));
      break;
    case ERenderMode::Positions: {
      color = (pos.array() - Array3f::Constant(0.5f)) / 2.0f +
              Array3f::Constant(0.5f);
    } break;
    case ERenderMode::Normals:
      color = 0.5f * normal.array() + Array3f::Constant(0.5f);
      break;
    case ERenderMode::Cost:
      color = Array3f::Constant((float)payload.n_steps / 30);
      break;
...
__global__ void compact_kernel_shadow_sdf(
    const uint32_t n_elements, const float zero_offset, Vector3f *src_positions,
    float *src_distances, SdfPayload *src_payloads, float *src_prev_distances,
    float *src_total_distances, float *src_min_visibility,
    Vector3f *dst_positions, float *dst_distances, SdfPayload *dst_payloads,
    float *dst_prev_distances, float *dst_total_distances,
    float *dst_min_visibility, Vector3f *dst_final_positions,
    float *dst_final_distances, SdfPayload *dst_final_payloads,
    float *dst_final_prev_distances, float *dst_final_total_distances,
    float *dst_final_min_visibility, BoundingBox aabb, uint32_t *counter,
...
__global__ void compact_kernel_sdf(
    const uint32_t n_elements, const float zero_offset, Vector3f *src_positions,
    float *src_distances, SdfPayload *src_payloads, Vector3f *dst_positions,
    float *dst_distances, SdfPayload *dst_payloads,
    Vector3f *dst_final_positions, float *dst_final_distances,
    SdfPayload *dst_final_payloads, BoundingBox aabb, uint32_t *counter,
    uint32_t *finalCounter) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
...
__global__ void uniform_octree_sample_kernel(
    const uint32_t num_elements, default_rng_t rng,
    const TriangleOctreeNode *__restrict__ octree_nodes, uint32_t num_nodes,
    uint32_t depth, Vector3f *__restrict__ samples) {
  uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void scale_to_aabb_kernel(uint32_t n_elements, BoundingBox aabb,
...
__global__ void compare_signs_kernel(
    uint32_t n_elements, const Vector3f *positions, const float *distances_ref,
    const float *distances_model, uint32_t *counters,
    const TriangleOctreeNode *octree_nodes, int max_depth) {
  uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n_elements) return;
...
__global__ void scale_iou_counters_kernel(uint32_t n_elements,
                                          uint32_t *counters, float scale) {
  uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n_elements) return;
...
__global__ void assign_float(uint32_t n_elements, float value,
...
__global__ void init_rays_with_payload_kernel_sdf(
    uint32_t sample_index, Vector3f *__restrict__ positions,
    float *__restrict__ distances, SdfPayload *__restrict__ payloads,
    Vector2i resolution, Vector2f focal_length,
    Matrix<float, 3, 4> camera_matrix, Vector2f screen_center,
    Vector3f parallax_shift, bool snap_to_pixel_centers, BoundingBox aabb,
    float floor_y, float plane_z, float dof,
    const float *__restrict__ envmap_data, const Vector2i envmap_resolution,
    Array4f *__restrict__ framebuffer, float *__restrict__ depthbuffer,
  ...
  Ray ray = pixel_to_ray(sample_index, {x, y}, resolution, focal_length,
                         camera_matrix, screen_center, parallax_shift,
                         snap_to_pixel_centers, plane_z, dof);
  ...
  if (octree_nodes &&
      !TriangleOctree::contains(octree_nodes, max_depth, ray.o)) {
    t = max(0.0f, TriangleOctree::ray_intersect(octree_nodes, max_depth, ray.o,
                                                ray.d));
...
__host__ __device__ uint32_t sample_discrete(float uniform_sample,
...
__global__ void sample_uniform_on_triangle_kernel(
    uint32_t n_elements, const float *__restrict__ cdf, uint32_t length,
  ...
  sampled_positions[i] =
      triangles[tri_idx].sample_uniform_position(sample.tail<2>());
}

void Testbed::SphereTracer::init_rays_from_camera(
    uint32_t sample_index, const Vector2i &resolution,
    const Vector2f &focal_length, const Matrix<float, 3, 4> &camera_matrix,
    const Vector2f &screen_center, const Vector3f &parallax_shift,
    bool snap_to_pixel_centers, const BoundingBox &aabb, float floor_y,
    float plane_z, float dof, const float *envmap_data,
    const Vector2i &envmap_resolution, Array4f *frame_buffer,
    float *depth_buffer, const TriangleOctree *octree, uint32_t n_octree_levels,
    cudaStream_t stream) {
  ...
  init_rays_with_payload_kernel_sdf<<<blocks, threads, 0, stream>>>(
      sample_index, m_rays[0].pos.data(), m_rays[0].distance.data(),
      m_rays[0].payload.data(), resolution, focal_length, camera_matrix,
      screen_center, parallax_shift, snap_to_pixel_centers, aabb, floor_y,
      plane_z, dof, envmap_data, envmap_resolution, frame_buffer, depth_buffer,
      octree ? octree->nodes_gpu() : nullptr, octree ? n_octree_levels : 0);
  m_n_rays_initialized = (uint32_t)n_pixels;
}

void Testbed::SphereTracer::init_rays_from_data(uint32_t n_elements,
...
uint32_t Testbed::SphereTracer::trace_bvh(TriangleBvh *bvh,
...
uint32_t Testbed::SphereTracer::trace(
    ...
    float distance_scale, float maximum_distance, const BoundingBox &aabb,
    const float floor_y, const TriangleOctree *octree,
    const uint32_t n_octree_levels, cudaStream_t stream) {
  ...
  while (i < MARCH_ITER) {
    ...
    // Compact rays that did not diverge yet
    {
      ...
      if (m_trace_shadow_rays) {
        linear_kernel(
            compact_kernel_shadow_sdf, 0, stream, n_alive, zero_offset,
            rays_tmp.pos.data(), rays_tmp.distance.data(),
            rays_tmp.payload.data(), rays_tmp.prev_distance.data(),
            rays_tmp.total_distance.data(), rays_tmp.min_visibility.data(),
            rays_current.pos.data(), rays_current.distance.data(),
            rays_current.payload.data(), rays_current.prev_distance.data(),
            rays_current.total_distance.data(),
            rays_current.min_visibility.data(), m_rays_hit.pos.data(),
            m_rays_hit.distance.data(), m_rays_hit.payload.data(),
            m_rays_hit.prev_distance.data(), m_rays_hit.total_distance.data(),
            m_rays_hit.min_visibility.data(), aabb, m_alive_counter.data(),
            m_hit_counter.data());
      } else {
        linear_kernel(compact_kernel_sdf, 0, stream, n_alive, zero_offset,
                      rays_tmp.pos.data(), rays_tmp.distance.data(),
                      rays_tmp.payload.data(), rays_current.pos.data(),
                      rays_current.distance.data(), rays_current.payload.data(),
                      m_rays_hit.pos.data(), m_rays_hit.distance.data(),
                      m_rays_hit.payload.data(), aabb, m_alive_counter.data(),
                      m_hit_counter.data());
      }
    ...
    for (uint32_t j = 0; j < step_size; ++j) {
      distance_function(n_alive, rays_current.pos, rays_current.distance,
                        stream);
      linear_kernel(
          advance_pos_kernel_sdf, 0, stream, n_alive, zero_offset,
          rays_current.pos.data(), rays_current.distance.data(),
          rays_current.payload.data(), aabb, floor_y,
          octree ? octree->nodes_gpu() : nullptr, octree ? n_octree_levels : 0,
          distance_scale, maximum_distance, m_shadow_sharpness,
...
void Testbed::FiniteDifferenceNormalsApproximator::enlarge(
    uint32_t n_elements) {
...
void Testbed::FiniteDifferenceNormalsApproximator::normal(
    uint32_t n_elements, const distance_fun_t &distance_function,
    GPUMemory<Vector3f> &pos, GPUMemory<Vector3f> &normal, float epsilon,
    cudaStream_t stream) {
  enlarge(n_elements);

  parallel_for_gpu(stream, n_elements,
                   [pos = pos.data(), dx = dx.data(), dy = dy.data(),
                    dz = dz.data(), epsilon] __device__(size_t i) {
                     Vector3f p = pos[i];
  ...
  parallel_for_gpu(
      stream, n_elements,
      [normal = normal.data(), dist_dx_pos = dist_dx_pos.data(),
       dist_dx_neg = dist_dx_neg.data(), dist_dy_pos = dist_dy_pos.data(),
       dist_dy_neg = dist_dy_neg.data(), dist_dz_pos = dist_dz_pos.data(),
...
void Testbed::render_sdf(const distance_fun_t &distance_function,
  ...
  m_sdf.tracer.init_rays_from_camera(
      render_buffer.spp(), render_buffer.in_resolution(), focal_length,
      camera_matrix, screen_center, get_scaled_parallax_shift(),
      m_snap_to_pixel_centers, sdf_bounding_box, get_floor_y(), plane_z, m_dof,
      m_envmap.envmap->params_inference(), m_envmap.resolution,
      render_buffer.frame_buffer(), render_buffer.depth_buffer(), octree_ptr,
      n_octree_levels, stream);

  bool gt_raytrace =
      m_render_ground_truth &&
      m_sdf.groundtruth_mode == ESDFGroundTruthMode::RaytracedMesh;

  auto trace = [&](SphereTracer &tracer) {
    if (gt_raytrace) {
      return tracer.trace_bvh(m_sdf.triangle_bvh.get(),
                              m_sdf.triangles_gpu.data(), stream);
    } else {
      return tracer.trace(distance_function, m_sdf.zero_offset,
                          m_sdf.distance_scale, m_sdf.maximum_distance,
                          sdf_bounding_box, get_floor_y(), octree_ptr,
                          n_octree_levels, stream);
    }
  };
    ...
    if (m_visualized_dimension == -1) {
      ...
      extract_dimension_pos_neg_kernel<float>
          <<<n_blocks_linear(n_hit * 3), n_threads_linear, 0, stream>>>(
              n_hit * 3, 0, 1, 3, rays_hit.distance.data(), CM,
              (float *)rays_hit.normal.data());
    } else {
      // Store colors in the normal buffer
      uint32_t n_elements = next_multiple(n_hit, tcnn::batch_size_granularity);

      GPUMatrix<float> positions_matrix((float *)rays_hit.pos.data(), 3,
                                        n_elements);
      GPUMatrix<float> colors_matrix((float *)rays_hit.normal.data(), 3,
                                     n_elements);
  ...
  ERenderMode render_mode =
      (m_visualized_dimension > -1 || m_render_mode == ERenderMode::Slice)
          ...
          : m_render_mode;
  if (render_mode == ERenderMode::Shade ||
      render_mode == ERenderMode::Normals) {
    if (m_sdf.analytic_normals || gt_raytrace) {
      ...
      m_sdf.fd_normals.enlarge(render_buffer.in_resolution().x() *
      ...
      if (m_render_ground_truth &&
        ...
        fd_normals_epsilon =
            exp2f(-float(n_octree_levels)) *
            (1.f /
             (m_sdf.brick_res - 1));  // in sdf brick mode, use one voxel as the
                                      // normal central difference radius
      }
      m_sdf.fd_normals.normal(n_hit, distance_function, rays_hit.pos,
                              rays_hit.normal, fd_normals_epsilon, stream);
    }
    ...
    if (render_mode == ERenderMode::Shade && n_hit > 0) {
      ...
      linear_kernel(
          prepare_shadow_rays, 0, stream, n_hit, m_sun_dir.normalized(),
          shadow_rays_init.pos.data(), shadow_rays_init.normal.data(),
          ...
          shadow_rays_init.payload.data(), sdf_bounding_box,
          octree_ptr ? octree_ptr->nodes_gpu() : nullptr, n_octree_levels);
      ...
      linear_kernel(
          write_shadow_ray_result, 0, stream, n_hit_shadow, sdf_bounding_box,
          shadow_rays_hit.pos.data(), shadow_rays_hit.payload.data(),
          shadow_rays_hit.min_visibility.data(), rays_hit.distance.data());
  ...
  linear_kernel(shade_kernel_sdf, 0, stream, n_hit, m_aabb, get_floor_y(),
                render_mode, m_sdf.brdf, m_sun_dir.normalized(),
                m_up_dir.normalized(), camera_matrix, rays_hit.pos.data(),
                rays_hit.normal.data(), rays_hit.distance.data(),
                rays_hit.payload.data(), render_buffer.frame_buffer(),
...
void Testbed::load_mesh() {
  ...
  if (equals_case_insensitive(m_data_path.extension(), "obj")) {
    ...
    throw std::runtime_error{
        "Sdf data path must be a mesh in ascii .obj or binary .stl format."};
  }
  ...
  for (size_t i = 0; i < n_vertices; i += 3) {
    m_sdf.triangles_cpu[i / 3] = {vertices[i + 0], vertices[i + 1],
                                  vertices[i + 2]};
  }
...
void Testbed::generate_training_samples_sdf(Vector3f *positions,
                                            float *distances,
  ...
  generate_random_uniform<float>(stream, m_rng, n_to_generate * 3,
                                 (float *)positions);

  linear_kernel(sample_uniform_on_triangle_kernel, 0, stream,
                n_to_generate_surface, m_sdf.triangle_cdf.data(),
                (uint32_t)m_sdf.triangle_cdf.size(), m_sdf.triangles_gpu.data(),
                positions);
  ...
  float stddev =
      m_bounding_radius / 1024.0f * m_sdf.training.surface_offset_scale;
  if (!uniform_only &&
      (m_sdf.uses_takikawa_encoding || m_sdf.use_triangle_octree)) {
    linear_kernel(
        uniform_octree_sample_kernel, 0, stream, n_to_generate_uniform, m_rng,
        m_sdf.triangle_octree->nodes_gpu(), m_sdf.triangle_octree->n_nodes(),
        m_sdf.triangle_octree->depth(), positions + n_to_generate_surface);
    m_rng.advance();
    ...
    linear_kernel(assign_float, 0, stream, n_to_generate_uniform,
    ...
    linear_kernel(scale_to_aabb_kernel, 0, stream, n_to_generate_uniform,
                  sdf_aabb, positions + n_to_generate_surface);
  ...
  generate_random_logistic<float>(
      stream, m_rng, n_to_generate_surface_offset * 3,
      (float *)m_sdf.training.perturbations.data(), 0.0f, stddev);

  linear_kernel(perturb_sdf_samples, 0, stream, n_to_generate_surface_offset,
  ...
  m_sdf.triangle_bvh->signed_distance_gpu(
      n_to_generate_uniform + n_to_generate_surface_offset, m_sdf.mesh_sdf_mode,
      positions + n_to_generate_surface_exact,
      distances + n_to_generate_surface_exact, m_sdf.triangles_gpu.data(), true,
      stream);

  CUDA_CHECK_THROW(cudaStreamSynchronize(stream));
}

__global__ void generate_grid_samples_sdf_uniform(Eigen::Vector3i res_3d,
                                                  BoundingBox aabb,
...
GPUMemory<float> Testbed::get_sdf_gt_on_grid(Vector3i res3d,
                                             const BoundingBox &aabb) {
  const uint32_t n_elements = (res3d.x() * res3d.y() * res3d.z());
  GPUMemory<float> density(n_elements);
  GPUMemoryArena::Allocation alloc;
  auto scratch = allocate_workspace_and_distribute<Vector3f>(
  ...
  generate_grid_samples_sdf_uniform<<<blocks, threads, 0, m_inference_stream>>>(
      res3d, aabb, positions);
  CUDA_CHECK_THROW(cudaStreamSynchronize(m_inference_stream));
  m_sdf.triangle_bvh->signed_distance_gpu(
      n_elements, m_sdf.mesh_sdf_mode, positions, sdf_out,
...
void Testbed::train_sdf(size_t target_batch_size, bool get_loss_scalar,
                        cudaStream_t stream) {
  ...
  if (m_sdf.training.size >= target_batch_size) {
    ...
    linear_kernel(shuffle<Vector3f>, 0, stream, m_sdf.training.size, 1,
                  m_training_step, m_sdf.training.positions.data(),
                  m_sdf.training.positions_shuffled.data());
    linear_kernel(shuffle<float>, 0, stream, m_sdf.training.size, 1,
                  m_training_step, m_sdf.training.distances.data(),
                  m_sdf.training.distances_shuffled.data());

    GPUMatrix<float> training_target_matrix(
        m_sdf.training.distances_shuffled.data(), n_output_dims, batch_size);
    GPUMatrix<float> training_batch_matrix(
        (float *)(m_sdf.training.positions_shuffled.data()), n_input_dims,
        batch_size);

    auto ctx = m_trainer->training_step(stream, training_batch_matrix,
...
void Testbed::training_prep_sdf(uint32_t batch_size, cudaStream_t stream) {
  if (m_sdf.training.generate_sdf_data_online) {
    ...
    generate_training_samples_sdf(m_sdf.training.positions.data(),
                                  m_sdf.training.distances.data(), batch_size,
                                  stream, false);
  }
}
...
// from the *last* call
double Testbed::calculate_iou(uint32_t n_samples,
                              float scale_existing_results_factor,
                              bool blocking, bool force_use_octree) {
  ...
  if (!blocking)  // when not blocking, returns data from the *last* run, then
    ...
    cudaMemcpy(countercpu, m_sdf.iou_counter.data(), 8 * 4,
               cudaMemcpyDeviceToHost);

  if (scale_existing_results_factor < 1.f) {
    linear_kernel(scale_iou_counters_kernel, 0, stream, 8,
                  m_sdf.iou_counter.data(), scale_existing_results_factor);
  }
  while (n_samples > 0) {
    ...
    generate_training_samples_sdf(m_sdf.training.positions.data(),
                                  ...
                                  (uint32_t)(batch_size), stream, true);
    GPUMatrix<float> positions_matrix((float *)m_sdf.training.positions.data(),
                                      3, batch_size);
    GPUMatrix<float> distances_matrix(m_sdf.training.distances_shuffled.data(),
                                      1, batch_size);
    m_network->inference(stream, positions_matrix, distances_matrix);
    auto *octree_ptr = (m_sdf.uses_takikawa_encoding ||
                        m_sdf.use_triangle_octree || force_use_octree)
                           ? m_sdf.triangle_octree.get()
                           : nullptr;
    linear_kernel(compare_signs_kernel, 0, stream, batch_size,
  ...
  if (blocking) {
    CUDA_CHECK_THROW(cudaMemcpyAsync(countercpu, m_sdf.iou_counter.data(),
                                     8 * 4, cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK_THROW(cudaStreamSynchronize(stream));
  }
