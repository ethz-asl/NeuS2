...
static_assert(
    false,
    "DLSS can only be compiled when both Vulkan and GUI support is enabled.")
#endif
...
uint32_t vk_find_memory_type(uint32_t type_filter,
...
void vulkan_and_ngx_init() {
  ...
  if (!validation_layer_enabled) {
    tlog::warning() << "Vulkan validation layer is not available. Vulkan "
                       "errors will be difficult to diagnose.";
  }
  ...
  NVSDK_NGX_VULKAN_RequiredExtensions(
      &n_ngx_instance_extensions, &ngx_instance_extensions,
      &n_ngx_device_extensions, &ngx_device_extensions);
  ...
  if (validation_layer_enabled) {
    ...
    if (CreateDebugUtilsMessengerEXT(vk_instance, &debug_messenger_create_info,
                                     nullptr,
  ...
  auto is_same_as_cuda_device = [&](VkPhysicalDevice device) {
    ...
    return !memcmp(&cuda_device_prop.uuid,
                   physical_device_id_properties.deviceUUID, VK_UUID_SIZE) &&
           find_queue_families(device).all_family >= 0;
  };
  ...
  if (vk_physical_device == VK_NULL_HANDLE) {
    throw std::runtime_error{
        "Failed to find Vulkan device corresponding to CUDA device."};
  }
  ...
  VK_CHECK_THROW(vkCreateDevice(vk_physical_device, &device_create_info,
                                nullptr, &vk_device));
  ...
  VK_CHECK_THROW(vkCreateCommandPool(vk_device, &command_pool_info, nullptr,
                                     &vk_command_pool));
  ...
  VK_CHECK_THROW(vkAllocateCommandBuffers(vk_device, &command_buffer_alloc_info,
                                          &vk_command_buffer));
  ...
  NGX_CHECK_THROW(NVSDK_NGX_VULKAN_Init_with_ProjectID(
      "ea75345e-5a42-4037-a5c9-59bf94dee157", NVSDK_NGX_ENGINE_TYPE_CUSTOM,
      "1.0.0", path.c_str(), vk_instance, vk_physical_device, vk_device));
  ...
  NVSDK_NGX_Result result_updated_driver =
      ngx_parameters->Get(NVSDK_NGX_Parameter_SuperSampling_NeedsUpdatedDriver,
                          &needs_updated_driver);
  NVSDK_NGX_Result result_min_driver_version_major = ngx_parameters->Get(
      NVSDK_NGX_Parameter_SuperSampling_MinDriverVersionMajor,
      &min_driver_version_major);
  NVSDK_NGX_Result result_min_driver_version_minor = ngx_parameters->Get(
      NVSDK_NGX_Parameter_SuperSampling_MinDriverVersionMinor,
      &min_driver_version_minor);
  if (result_updated_driver == NVSDK_NGX_Result_Success &&
      result_min_driver_version_major == NVSDK_NGX_Result_Success &&
      result_min_driver_version_minor == NVSDK_NGX_Result_Success) {
    if (needs_updated_driver) {
      throw std::runtime_error{
          std::string{"Driver too old. Minimum version required: "} +
          std::to_string(min_driver_version_major) + "." +
          std::to_string(min_driver_version_minor)};
    }
  }

  int dlss_available = 0;
  NVSDK_NGX_Result ngx_result = ngx_parameters->Get(
      NVSDK_NGX_Parameter_SuperSampling_Available, &dlss_available);
  if (ngx_result != NVSDK_NGX_Result_Success || !dlss_available) {
    ...
    NVSDK_NGX_Parameter_GetI(
        ...
        (int *)&ngx_result);
    throw std::runtime_error{std::string{"DLSS not available. ("} +
                             ngx_error_string(ngx_result) + ")"};
  }

  tlog::success() << "Initialized Vulkan and NGX on device #" << device_id
...
void vk_synchronize() { VK_CHECK_THROW(vkDeviceWaitIdle(vk_device)); }
...
class VulkanTexture {
 public:
  VulkanTexture(const Vector2i &size, uint32_t n_channels)
      : m_size{size}, m_n_channels{n_channels} {
    ...
    switch (n_channels) {
      ...
      default:
        throw std::runtime_error{
            "VulkanTexture only supports 1, 2, 3, or 4 channels."};
    }
    ...
    mem_alloc_info.memoryTypeIndex = vk_find_memory_type(
    ...
    VK_CHECK_THROW(vkAllocateMemory(vk_device, &mem_alloc_info, nullptr,
                                    &m_vk_device_memory));
    VK_CHECK_THROW(
        vkBindImageMemory(vk_device, m_vk_image, m_vk_device_memory, 0));
    ...
    vkCmdPipelineBarrier(vk_command_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
    ...
    VK_CHECK_THROW(
        vkCreateImageView(vk_device, &view_info, nullptr, &m_vk_image_view));

    // Map to NGX
    m_ngx_resource = NVSDK_NGX_Create_ImageView_Resource_VK(
        m_vk_image_view, m_vk_image, view_info.subresourceRange,
        image_info.format, m_size.x(), m_size.y(), true);

    // Map to CUDA memory: VkDeviceMemory->FD/HANDLE->cudaExternalMemory->CUDA
    // pointer
#ifdef _WIN32
    ...
    auto pfn_vkGetMemory = (PFN_vkGetMemoryWin32HandleKHR)vkGetDeviceProcAddr(
    ...
    auto pfn_vkGetMemory = (PFN_vkGetMemoryFdKHR)vkGetDeviceProcAddr(
        vk_device, "vkGetMemoryFdKHR");
#endif
    ...
    memset(&external_memory_handle_desc, 0,
           sizeof(external_memory_handle_desc));
    ...
    CUDA_CHECK_THROW(cudaImportExternalMemory(&m_cuda_external_memory,
                                              &external_memory_handle_desc));

    cudaExternalMemoryBufferDesc external_memory_buffer_desc = {};
    memset(&external_memory_buffer_desc, 0,
           sizeof(external_memory_buffer_desc));
    external_memory_buffer_desc.offset = 0;
    external_memory_buffer_desc.size = mem_requirements.size;

    void *ptr;
    CUDA_CHECK_THROW(cudaExternalMemoryGetMappedBuffer(
        &ptr, m_cuda_external_memory, &external_memory_buffer_desc));
    m_cuda_data = (float *)ptr;

    // ----------------
    // Also get a surface object array, as the above buffer might be too
    // cumbersome to deal with
    // ----------------
    cudaExternalMemoryMipmappedArrayDesc external_memory_mipmapped_array_desc =
        {};
    memset(&external_memory_mipmapped_array_desc, 0,
           sizeof(external_memory_mipmapped_array_desc));
    ...
    cudaExternalMemoryGetMappedMipmappedArray(
        &m_cuda_mipmapped_array, m_cuda_external_memory,
        &external_memory_mipmapped_array_desc);

    cudaArray_t first_level_array;
    CUDA_CHECK_THROW(cudaGetMipmappedArrayLevel(&first_level_array,
                                                m_cuda_mipmapped_array, 0));
    ...
    CUDA_CHECK_THROW(
        cudaCreateSurfaceObject(&m_cuda_surface_object, &resource_desc));
  }
  ...
  float *data() { return m_cuda_data; }

  cudaSurfaceObject_t surface() { return m_cuda_surface_object; }

  NVSDK_NGX_Resource_VK &ngx_resource() { return m_ngx_resource; }
...
NVSDK_NGX_PerfQuality_Value ngx_dlss_quality(EDlssQuality quality) {
  switch (quality) {
    case EDlssQuality::UltraPerformance:
      return NVSDK_NGX_PerfQuality_Value_UltraPerformance;
    case EDlssQuality::MaxPerformance:
      return NVSDK_NGX_PerfQuality_Value_MaxPerf;
    case EDlssQuality::Balanced:
      return NVSDK_NGX_PerfQuality_Value_Balanced;
    case EDlssQuality::MaxQuality:
      return NVSDK_NGX_PerfQuality_Value_MaxQuality;
    case EDlssQuality::UltraQuality:
      return NVSDK_NGX_PerfQuality_Value_UltraQuality;
    default:
      throw std::runtime_error{"Unknown DLSS quality setting."};
  }
}
...
struct DlssFeatureSpecs {
  ...
  float distance(const Vector2i &resolution) const {
    return (min_in_resolution - resolution)
        .cwiseMax(resolution - max_in_resolution)
        .cwiseMax(0)
        .cast<float>()
        .norm();
  }
...
DlssFeatureSpecs dlss_feature_specs(const Eigen::Vector2i &out_resolution,
                                    EDlssQuality quality) {
  ...
  NGX_CHECK_THROW(NGX_DLSS_GET_OPTIMAL_SETTINGS(
      ngx_parameters, specs.out_resolution.x(), specs.out_resolution.y(),
      ngx_dlss_quality(quality), (uint32_t *)&specs.optimal_in_resolution.x(),
      (uint32_t *)&specs.optimal_in_resolution.y(),
      (uint32_t *)&specs.max_in_resolution.x(),
      (uint32_t *)&specs.max_in_resolution.y(),
      (uint32_t *)&specs.min_in_resolution.x(),
      (uint32_t *)&specs.min_in_resolution.y(), &specs.optimal_sharpness));

  // Don't permit input resolutions larger than the output. (Just in case DLSS
  // allows it.)
  specs.optimal_in_resolution =
      specs.optimal_in_resolution.cwiseMin(out_resolution);
...
class DlssFeature {
 public:
  DlssFeature(const DlssFeatureSpecs &specs, bool is_hdr, bool sharpen)
      : m_specs{specs}, m_is_hdr{is_hdr}, m_sharpen{sharpen} {
    // Initialize DLSS
    ...
    dlss_create_feature_flags |=
        true ? NVSDK_NGX_DLSS_Feature_Flags_MVLowRes : 0;
    dlss_create_feature_flags |=
        false ? NVSDK_NGX_DLSS_Feature_Flags_MVJittered : 0;
    dlss_create_feature_flags |=
        is_hdr ? NVSDK_NGX_DLSS_Feature_Flags_IsHDR : 0;
    dlss_create_feature_flags |=
        false ? NVSDK_NGX_DLSS_Feature_Flags_DepthInverted : 0;
    dlss_create_feature_flags |=
        sharpen ? NVSDK_NGX_DLSS_Feature_Flags_DoSharpening : 0;
    dlss_create_feature_flags |=
        false ? NVSDK_NGX_DLSS_Feature_Flags_AutoExposure : 0;
    ...
    dlss_create_params.Feature.InPerfQualityValue =
        ngx_dlss_quality(m_specs.quality);
    dlss_create_params.InFeatureCreateFlags = dlss_create_feature_flags;

    {
      vk_command_buffer_begin();
      ScopeGuard command_buffer_guard{
          [&]() { vk_command_buffer_end_and_submit_sync(); }};

      NGX_CHECK_THROW(NGX_VULKAN_CREATE_DLSS_EXT(
          vk_command_buffer, creation_node_mask, visibility_node_mask,
          &m_ngx_dlss, ngx_parameters, &dlss_create_params));
    }
  }

  DlssFeature(const Eigen::Vector2i &out_resolution, bool is_hdr, bool sharpen,
              EDlssQuality quality)
  ...
  void run(const Vector2i &in_resolution, const Vector2f &jitter_offset,
           float sharpening, bool shall_reset, NVSDK_NGX_Resource_VK &frame,
           NVSDK_NGX_Resource_VK &depth, NVSDK_NGX_Resource_VK &mvec,
           NVSDK_NGX_Resource_VK &exposure, NVSDK_NGX_Resource_VK &output) {
    if (!m_sharpen && sharpening != 0.0f) {
      throw std::runtime_error{
          "May only specify non-zero sharpening, when DlssFeature has been "
          "created with sharpen option."};
    }
    ...
    dlss_params.InRenderSubrectDimensions = {(uint32_t)in_resolution.x(),
                                             (uint32_t)in_resolution.y()};

    NGX_CHECK_THROW(NGX_VULKAN_EVALUATE_DLSS_EXT(vk_command_buffer, m_ngx_dlss,
                                                 ngx_parameters, &dlss_params));
  ...
  bool sharpen() const { return m_sharpen; }

  EDlssQuality quality() const { return m_specs.quality; }
...
class Dlss : public IDlss {
 public:
  ...
  void run(const Vector2i &in_resolution, bool is_hdr, float sharpening,
           const Vector2f &jitter_offset, bool shall_reset) override {
    ...
    if (!found) {
      throw std::runtime_error{
          "Dlss::run called with invalid input resolution."};
    }

    bool sharpen = sharpening != 0.0f;
    if (!m_dlss_feature || m_dlss_feature->is_hdr() != is_hdr ||
        m_dlss_feature->sharpen() != sharpen ||
        m_dlss_feature->quality() != quality) {
      m_dlss_feature.reset(
          new DlssFeature{m_out_resolution, is_hdr, sharpen, quality});
    }

    m_dlss_feature->run(
        in_resolution, jitter_offset, sharpening, shall_reset,
        m_frame_buffer.ngx_resource(), m_depth_buffer.ngx_resource(),
        m_mvec_buffer.ngx_resource(), m_exposure_buffer.ngx_resource(),
        m_output_buffer.ngx_resource());
  }

  cudaSurfaceObject_t frame() override { return m_frame_buffer.surface(); }

  cudaSurfaceObject_t depth() override { return m_depth_buffer.surface(); }

  cudaSurfaceObject_t mvec() override { return m_mvec_buffer.surface(); }

  cudaSurfaceObject_t exposure() override {
    return m_exposure_buffer.surface();
  }

  cudaSurfaceObject_t output() override { return m_output_buffer.surface(); }
  ...
  Vector2i out_resolution() const override { return m_out_resolution; }
