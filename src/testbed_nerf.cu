...
#define rotation_reprensentation \
  1  // 1 refers to rotation 6d, 0 refers to quaternion

#define PRINT_PART_TIME 0

inline constexpr __device__ float NERF_RENDERING_NEAR_DISTANCE() {
  return 0.2f;
}
...
template <typename T>
__global__ void transform_mesh_with_quaternion(const uint32_t n_elements,
                                               const T *rotation,
                                               const T *transition,
                                               float *mesh_vex) {
  ...
  float vx = mesh_vex[i * 3 + 0], vy = mesh_vex[i * 3 + 1],
        vz = mesh_vex[i * 3 + 2];
  ...
  mesh_vex[i * 3 + 0] = qw * (2 * qy * vz - 2 * qz * vy) +
                        qy * (2 * qx * vy - 2 * qy * vx) -
                        qz * (-2 * qx * vz + 2 * qz * vx) + vx;
  mesh_vex[i * 3 + 1] = qw * (-2 * qx * vz + 2 * qz * vx) -
                        qx * (2 * qx * vy - 2 * qy * vx) +
                        qz * (2 * qy * vz - 2 * qz * vy) + vy;
  mesh_vex[i * 3 + 2] = qw * (2 * qx * vy - 2 * qy * vx) +
                        qx * (-2 * qx * vz + 2 * qz * vx) -
                        qy * (2 * qy * vz - 2 * qz * vy) + vz;
}

template <typename T>
__global__ void transform_mesh_with_6d(const uint32_t n_elements,
                                       const T *rotation_6d,
                                       const T *transition, float *mesh_vex) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
  ...
  rotation_matrix << (float)rotation_6d[0], (float)rotation_6d[1],
      (float)rotation_6d[2], (float)rotation_6d[3], (float)rotation_6d[4],
      (float)rotation_6d[5], (float)rotation_6d[6], (float)rotation_6d[7],
      (float)rotation_6d[8];

  Eigen::Vector3f transition_vector(transition[0], transition[1],
                                    transition[2]);
...
inline __host__ __device__ float calc_cone_angle(
    float cosine, const Eigen::Vector2f &focal_length,
...
__device__ void global_movement_with_rotation_quaternion(
    const T *rotation, const T *transition, Eigen::Vector3f &ray_o,
    Eigen::Vector3f &ray_d) {
  float qx = rotation[0], qy = rotation[1], qz = rotation[2], qw = rotation[3];
  float vx = ray_o[0], vy = ray_o[1], vz = ray_o[2];
  float tx = transition[0], ty = transition[1], tz = transition[2];

  ray_o[0] = qw * (2 * qy * vz - 2 * qz * vy) +
             qy * (2 * qx * vy - 2 * qy * vx) -
             qz * (-2 * qx * vz + 2 * qz * vx) + vx + tx;
  ray_o[1] = qw * (-2 * qx * vz + 2 * qz * vx) -
             qx * (2 * qx * vy - 2 * qy * vx) +
             qz * (2 * qy * vz - 2 * qz * vy) + vy + ty;
  ray_o[2] = qw * (2 * qx * vy - 2 * qy * vx) +
             qx * (-2 * qx * vz + 2 * qz * vx) -
             qy * (2 * qy * vz - 2 * qz * vy) + vz + tz;

  vx = ray_d[0], vy = ray_d[1], vz = ray_d[2];
  ray_d[0] = qw * (2 * qy * vz - 2 * qz * vy) +
             qy * (2 * qx * vy - 2 * qy * vx) -
             qz * (-2 * qx * vz + 2 * qz * vx) + vx;
  ray_d[1] = qw * (-2 * qx * vz + 2 * qz * vx) -
             qx * (2 * qx * vy - 2 * qy * vx) +
             qz * (2 * qy * vz - 2 * qz * vy) + vy;
  ray_d[2] = qw * (2 * qx * vy - 2 * qy * vx) +
             qx * (-2 * qx * vz + 2 * qz * vx) -
             qy * (2 * qy * vz - 2 * qz * vy) + vz;
}

template <typename T>
__device__ void global_movement_with_rotation_6d(const T *rotation_6d,
                                                 const T *transition,
...
template <typename T>
__device__ void local_movement_with_rotation_6d(const T *rotation_6d,
                                                Eigen::Vector3f &ray_o,
                                                Eigen::Vector3f &ray_d) {
  Eigen::Matrix3f rotation_matrix;
...
__device__ Array3f network_to_pos_gradient(
    const tcnn::vector_t<tcnn::network_precision_t, 16> &local_network_output,
    ENerfActivation activation) {
  Vector3f raw_gradient{float(local_network_output[4]),
                        float(local_network_output[5]),
                        float(local_network_output[6])};
  return {raw_gradient.x(), raw_gradient.y(), raw_gradient.z()};
}

__device__ float network_to_pos_gradient(float val,
                                         ENerfActivation activation) {
  switch (activation) {
...
inline __device__ float advance_to_next_voxel(float t, float cone_angle,
                                              const Vector3f &pos,
                                              const Vector3f &dir,
                                              const Vector3f &idir,
                                              uint32_t res) {
  ...
  // cone_angle); return t + ceilf(fmaxf(distance_to_next_voxel(pos, dir, idir,
  // res) / dt, 0.5f)) * dt;
...
// __device__ Array3f network_to_rgb(const
// tcnn::vector_t<tcnn::network_precision_t, 8>& local_network_output,
// ENerfActivation activation) {
__device__ Array3f network_to_rgb(
    const tcnn::vector_t<tcnn::network_precision_t, 16> &local_network_output,
    ENerfActivation activation) {
...
__device__ Vector3f unwarp_position_derivative(const Vector3f &pos,
                                               const BoundingBox &aabb) {
  // return {logit(pos.x()) + 0.5f, logit(pos.y()) + 0.5f, logit(pos.z()) +
  // 0.5f}; return pos;
...
__device__ Vector3f warp_position_derivative(const Vector3f &pos,
                                             const BoundingBox &aabb) {
...
__device__ float cascaded_grid_at(Vector3f pos, const float *cascaded_grid,
                                  uint32_t mip) {
  uint32_t idx = cascaded_grid_idx_at(pos, mip);
  return cascaded_grid[idx + grid_mip_offset(mip)];
}

__device__ float &cascaded_grid_at(Vector3f pos, float *cascaded_grid,
                                   uint32_t mip) {
  uint32_t idx = cascaded_grid_idx_at(pos, mip);
  return cascaded_grid[idx + grid_mip_offset(mip)];
}
...
__global__ void mark_untrained_density_grid(
    const uint32_t n_elements, float *__restrict__ grid_out,
    const uint32_t n_training_images,
    const TrainingImageMetadata *__restrict__ metadata,
    const TrainingXForm *training_xforms, bool clear_visible_voxels) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
  ...
  Vector3f pos =
      ((Vector3f{(float)x + 0.5f, (float)y + 0.5f, (float)z + 0.5f}) /
           NERF_GRIDSIZE() -
...
__global__ void generate_grid_samples_nerf_uniform(
    Eigen::Vector3i res_3d, const uint32_t step, BoundingBox render_aabb,
    BoundingBox train_aabb, NerfPosition *__restrict__ out) {
...
__global__ void generate_grid_samples_nerf_uniform_dir(
    Eigen::Vector3i res_3d, const uint32_t step, BoundingBox render_aabb,
    BoundingBox train_aabb, Eigen::Vector3f ray_dir,
  ...
  Vector3f pos = Array3f{(float)x, (float)y, (float)z} *
                 Array3f{1.f / res_3d.x(), 1.f / res_3d.y(), 1.f / res_3d.z()};
  pos = pos.cwiseProduct(render_aabb.max - render_aabb.min) + render_aabb.min;
  network_input[i] = {warp_position(pos, train_aabb), warp_direction(ray_dir),
...
__global__ void generate_grid_samples_nerf_nonuniform(
    ...
    BoundingBox aabb, const float *__restrict__ grid_in,
    NerfPosition *__restrict__ out, uint32_t *__restrict__ indices,
    uint32_t n_cascades, float thresh) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
  ...
  uint32_t pos_idx =
      idx % (NERF_GRIDSIZE() * NERF_GRIDSIZE() * NERF_GRIDSIZE());
  ...
  Vector3f pos =
      ((Vector3f{(float)x, (float)y, (float)z} + random_val_3d(rng)) /
           NERF_GRIDSIZE() -
       Vector3f::Constant(0.5f)) *
          scalbnf(1.0f, level) +
      Vector3f::Constant(0.5f);
...
__global__ void splat_grid_samples_nerf_max_nearest_neighbor(
    const uint32_t n_elements, const uint32_t *__restrict__ indices,
    const tcnn::network_precision_t *network_output,
    float *__restrict__ grid_out, ENerfActivation rgb_activation,
  ...
  // observer is in the middle of the scene uint32_t level = 0;//local_idx /
  // (NERF_GRIDSIZE() * NERF_GRIDSIZE() * NERF_GRIDSIZE());
  ...
  atomicMax((uint32_t *)&grid_out[local_idx],
            __float_as_uint(optical_thickness));
}

__global__ void grid_samples_half_to_float(
    const uint32_t n_elements, BoundingBox aabb, float *dst,
    const tcnn::network_precision_t *network_output,
    ENerfActivation density_activation,
    const NerfPosition *__restrict__ coords_in,
    const float *__restrict__ grid_in, uint32_t max_cascade) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
  ...
  if (grid_in) {
    ...
    float grid_density =
        cascaded_grid_at(pos, grid_in, mip_from_pos(pos, max_cascade));
...
__global__ void ema_grid_samples_nerf(const uint32_t n_elements, float decay,
  ...
  // Maximum instead of EMA allows
  // splat_grid_samples_nerf_max_nearest_neighborcapture of very thin features.
  // Basically, we want the grid cell turned on as soon as _ANYTHING_ visible is
  // in there.
...
__global__ void decay_sharpness_grid_nerf(const uint32_t n_elements,
                                          float decay,
...
__global__ void advance_pos_nerf(const uint32_t n_elements,
                                 BoundingBox render_aabb, Vector3f camera_fwd,
                                 Vector2f focal_length, uint32_t sample_index,
                                 NerfPayload *__restrict__ payloads,
                                 const uint8_t *__restrict__ density_grid,
                                 uint32_t min_mip, float cone_angle_constant) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
...
__global__ void generate_nerf_network_inputs_from_positions(
    const uint32_t n_elements, BoundingBox aabb,
    const Vector3f *__restrict__ pos, PitchedPtr<NerfCoordinate> network_input,
    const float *extra_dims) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;

  Vector3f dir = (pos[i] - Vector3f::Constant(0.5f))
                     .normalized();  // choose outward pointing directions, for
                                     // want of a better choice
  network_input(i)->set_with_optional_extra_dims(
      warp_position(pos[i], aabb), warp_direction(dir),
      warp_dt(MIN_CONE_STEPSIZE()), extra_dims, network_input.stride_in_bytes);
}

__global__ void generate_nerf_network_inputs_at_current_position(
    const uint32_t n_elements, BoundingBox aabb,
    const NerfPayload *__restrict__ payloads,
    PitchedPtr<NerfCoordinate> network_input, const float *extra_dims) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
  ...
  network_input(i)->set_with_optional_extra_dims(
      ...
      warp_direction(dir), warp_dt(MIN_CONE_STEPSIZE()), extra_dims,
      network_input.stride_in_bytes);
}

__global__ void compute_nerf_density(const uint32_t n_elements,
...
__global__ void generate_next_nerf_network_inputs(
    const uint32_t n_elements, BoundingBox render_aabb, BoundingBox train_aabb,
    Vector2f focal_length, Vector3f camera_fwd,
    NerfPayload *__restrict__ payloads,
    PitchedPtr<NerfCoordinate> network_input, uint32_t n_steps,
    const uint8_t *__restrict__ density_grid, uint32_t min_mip,
    float cone_angle_constant, const float *extra_dims) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
  ...
  for (uint32_t j = 0; j < n_steps; ++j) {
    ...
    network_input(i + j * n_elements)
        ->set_with_optional_extra_dims(
            warp_position(pos, train_aabb), warp_direction(dir), warp_dt(dt),
            extra_dims, network_input.stride_in_bytes);  // XXXCONE
    t += dt;
  }
...
__global__ void composite_kernel_nerf(
    const uint32_t n_elements, const uint32_t stride,
    const uint32_t current_step, BoundingBox aabb, float glow_y_cutoff,
    int glow_mode, const uint32_t n_training_images,
    const TrainingXForm *__restrict__ training_xforms,
    Matrix<float, 3, 4> camera_matrix, Vector2f focal_length, float depth_scale,
    Array4f *__restrict__ rgba, float *__restrict__ depth,
    NerfPayload *payloads, PitchedPtr<NerfCoordinate> network_input,
    const tcnn::network_precision_t *__restrict__ network_output,
    uint32_t padded_output_width, uint32_t n_steps, ERenderMode render_mode,
    const uint8_t *__restrict__ density_grid, ENerfActivation rgb_activation,
    ENerfActivation density_activation, int show_accel, float min_transmittance,
    float variance, uint32_t training_step, const float cos_anneal_ratio) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
  ...
  for (; j < actual_n_steps; ++j) {
    ...
    float inv_s =
        __expf((tcnn::network_precision_t)10 * local_network_output[7]);
#if BENT_DIR
    Vector3f deformed_viewdir{float(local_network_output[8]),
                              float(local_network_output[9]),
                              float(local_network_output[10])};
    dir = unwarp_direction(deformed_viewdir).normalized();
#endif
    // Neus rendering

    float sdf_value = float(local_network_output[3]);
    Array3f pos_gradient =
        network_to_pos_gradient(local_network_output, ENerfActivation::None);
#if NORMAL_VECTORS_NORMALIZED
    float gradient_norm_for_dir = std::sqrt(
        pos_gradient[0] * pos_gradient[0] + pos_gradient[1] * pos_gradient[1] +
        pos_gradient[2] * pos_gradient[2] + 1e-5);
    float true_cos = (dir[0] * pos_gradient[0] + dir[1] * pos_gradient[1] +
                      dir[2] * pos_gradient[2]) /
                     gradient_norm_for_dir;
#else
    float true_cos = (dir[0] * pos_gradient[0] + dir[1] * pos_gradient[1] +
                      dir[2] * pos_gradient[2]);
#endif
    float iter_cos =
        -(activation_function(-true_cos * 0.5 + 0.5, ENerfActivation::ReLU) *
              (1.0 - cos_anneal_ratio) +
          activation_function(-true_cos, ENerfActivation::ReLU) *
              cos_anneal_ratio);

    float estimated_next_sdf = sdf_value + iter_cos * dt * 0.5;
    float estimated_prev_sdf = sdf_value - iter_cos * dt * 0.5;

    float next_cdf = activation_function(estimated_next_sdf * inv_s,
                                         ENerfActivation::Logistic);
    float prev_cdf = activation_function(estimated_prev_sdf * inv_s,
                                         ENerfActivation::Logistic);

    float p = prev_cdf - next_cdf;
    float c = prev_cdf;
    float p_div_c = (p + 1e-5f) / (c + 1e-5f);
    float alpha = tcnn::clamp(
        p_div_c, 0.0f,
        1.0f);  // alpha = 1.f - sigmoid(next_sdf / s) / sigmoid(prev_sdf / s);
    ...
    if (glow_mode) {  // random grid visualizations ftw!
      ...
      if (glow > 0.f) {
        float line;
        line =
            max(0.f, cosf(pos.y() * 2.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.x() * 2.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.z() * 2.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.y() * 4.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.x() * 4.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.z() * 4.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.y() * 8.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.x() * 8.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.z() * 8.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.y() * 16.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.x() * 16.f * 3.141592653589793f * 16.f) - 0.975f);
        line +=
            max(0.f, cosf(pos.z() * 16.f * 3.141592653589793f * 16.f) - 0.975f);
        if (grid_mode) {
    ...
    if (render_mode == ERenderMode::Normals) {
      ...
      Vector3f normal =
          -network_to_density_derivative(float(local_network_output[3]),
                                         density_activation) *
          warped_pos;
      ...
      if (show_accel >= 0) {
        ...
        rgb = (pos.array() - Array3f::Constant(0.5f)) / 2.0f +
              Array3f::Constant(0.5f);
      }
...
inline __device__ Vector2f sample_cdf_2d(Vector2f sample, uint32_t img,
                                         const Vector2i &res,
  ...
  sample.x() = (sample.x() - UNIFORM_SAMPLING_FRACTION) /
               (1.0f - UNIFORM_SAMPLING_FRACTION);
...
inline __device__ float pdf_2d(Vector2f sample, uint32_t img,
                               const Vector2i &res,
                               const float *__restrict__ cdf_x_cond_y,
                               const float *__restrict__ cdf_y) {
...
inline __device__ Vector2f nerf_random_image_pos_training(
    default_rng_t &rng, const Vector2i &resolution, bool snap_to_pixel_centers,
...
inline __device__ uint32_t image_idx(uint32_t base_idx, uint32_t n_rays,
...
__global__ void generate_training_samples_nerf_with_global_movement(
    const uint32_t n_rays, BoundingBox aabb, const uint32_t max_samples,
    const uint32_t n_rays_total, default_rng_t rng,
    uint32_t *__restrict__ ray_counter, uint32_t *__restrict__ numsteps_counter,
    uint32_t *__restrict__ ray_indices_out,
    Ray *__restrict__ rays_out_unnormalized,
    uint32_t *__restrict__ numsteps_out, PitchedPtr<NerfCoordinate> coords_out,
    const uint32_t n_training_images,
    const TrainingImageMetadata *__restrict__ metadata,
    const TrainingXForm *training_xforms,
    const uint8_t *__restrict__ density_grid, bool max_level_rand_training,
    float *__restrict__ max_level_ptr, bool snap_to_pixel_centers,
    bool train_envmap, float cone_angle_constant,
    const float *__restrict__ distortion_data,
    const Vector2i distortion_resolution,
    const float *__restrict__ cdf_x_cond_y, const float *__restrict__ cdf_y,
    const float *__restrict__ cdf_img, const Vector2i cdf_res,
    const float *__restrict__ extra_dims_gpu, uint32_t n_extra_dims,
    uint32_t cur_frame, uint32_t training_step,
    const network_precision_t *rotation, const network_precision_t *transition,
  ...
  Vector2f xy =
      nerf_random_image_pos_training(rng, resolution, snap_to_pixel_centers,
                                     cdf_x_cond_y, cdf_y, cdf_res, img);

  // Negative values indicate masked-away regions
  size_t pix_idx = pixel_idx(xy, resolution, 0);

  if (read_rgba(xy, resolution, metadata[img].pixels,
  ...
  if (rays_in_unnormalized) {  // always false;
    ...
    metadata[img].rolling_shutter, xy, 0.f); Ray ray2; ray2.o = xform.col(3);
    ray2.d = f_theta_distortion(xy, principal_point, camera_distortion);
    ray2.d = (xform.block<3, 3>(0, 0) * ray2.d).normalized();
    if (i==1000) {
            printf("\n%d uv %0.3f,%0.3f pixel %0.2f,%0.2f transform from [%0.5f
    %0.5f %0.5f] to [%0.5f %0.5f %0.5f]\n" " origin    [%0.5f %0.5f %0.5f] vs
    [%0.5f %0.5f %0.5f]\n" " direction [%0.5f %0.5f %0.5f] vs [%0.5f %0.5f
    ...
    */
    ...
    if (camera_distortion.mode == ECameraDistortionMode::FTheta) {
      ray_unnormalized.d = f_theta_undistortion(
          xy - principal_point, camera_distortion.params, {0.f, 0.f, 1.f});
    } else {
      ray_unnormalized.d = {
      ...
      if (camera_distortion.mode == ECameraDistortionMode::Iterative) {
        iterative_camera_undistortion(camera_distortion.params,
                                      &ray_unnormalized.d.x(),
                                      &ray_unnormalized.d.y());
      }
    }

    if (distortion_data) {
      ray_unnormalized.d.head<2>() +=
          read_image<2>(distortion_data, distortion_resolution, xy);
    }

    ray_unnormalized.d =
        (xform.block<3, 3>(0, 0) * ray_unnormalized.d);  // NOT normalized
  }
...
// apply global rotation and transition
    ...
    global_movement_with_rotation_6d<network_precision_t>(rotation, transition,
                                                          ray_o, dir);
#else
    global_movement_with_rotation_quaternion<network_precision_t>(
        rotation, transition, ray_o, dir);
#endif
  }

  Vector2f tminmax =
      aabb.ray_intersect(ray_o, dir);  // ray depth min and ray depth max
  float cone_angle =
      calc_cone_angle(dir.dot(xform.col(2)), focal_length, cone_angle_constant);

  // The near distance prevents learning of camera-specific fudge right in front
  // of the camera
  ...
  uint32_t numsteps = j;
  uint32_t base = atomicAdd(numsteps_counter,
                            numsteps);  // first entry in the array is a counter
  ...
  while (aabb.contains(pos = ray_o + t * dir) && j < numsteps) {
    ...
    if (density_grid_occupied_at(pos, density_grid, mip)) {
      coords_out(j)->set_with_optional_extra_dims(
          warp_position(pos, aabb), warped_dir, warp_dt(dt), extra_dims,
...
__global__ void compute_loss_kernel_train_nerf_with_global_movement(
    const uint32_t n_rays, BoundingBox aabb, const uint32_t n_rays_total,
    default_rng_t rng, const uint32_t max_samples_compacted,
    const uint32_t *__restrict__ rays_counter, float original_loss_scale,
    int padded_output_width, const float *__restrict__ envmap_data,
    float *__restrict__ envmap_gradient, const Vector2i envmap_resolution,
    ELossType envmap_loss_type, Array3f background_color,
    EColorSpace color_space, bool train_with_random_bg_color,
    bool train_in_linear_colors, const uint32_t n_training_images,
    const TrainingImageMetadata *__restrict__ metadata,
    const tcnn::network_precision_t *network_output,
    ...
    tcnn::network_precision_t *dloss_doutput, ELossType loss_type,
    float *__restrict__ loss_output, float *__restrict__ ek_loss_output,
    float *__restrict__ mask_loss_output, bool max_level_rand_training,
    float *__restrict__ max_level_compacted_ptr, ENerfActivation rgb_activation,
    ENerfActivation density_activation, bool snap_to_pixel_centers,
    float *__restrict__ error_map, const float *__restrict__ cdf_x_cond_y,
    const float *__restrict__ cdf_y, const float *__restrict__ cdf_img,
    const Vector2i error_map_res, const Vector2i error_map_cdf_res,
    const float *__restrict__ sharpness_data,
    Eigen::Vector2i sharpness_resolution, float *__restrict__ sharpness_grid,
    float *__restrict__ density_grid,
    const float *__restrict__ mean_density_ptr,
    const Eigen::Array3f *__restrict__ exposure,
    Eigen::Array3f *__restrict__ exposure_gradient,
    float depth_supervision_lambda, float near_distance, float variance,
    uint32_t training_step, const network_precision_t *rotation,
    const network_precision_t *transition, Eigen::Vector3f first_frame_offset,
    const float mask_loss_weight, const float ek_loss_weight,
...
// apply global rotation and transition
  ...
  for (; compacted_numsteps < numsteps; ++compacted_numsteps) {
    ...
    const tcnn::vector_t<tcnn::network_precision_t, 16> local_network_output =
        *(tcnn::vector_t<tcnn::network_precision_t, 16> *)network_output;
    ...
    // Neus rendering
    float sdf_value = float(local_network_output[3]);
    Array3f pos_gradient = network_to_pos_gradient(
        local_network_output, ENerfActivation::None);  // dsdf_dpos => normal
  ...
  Vector2f xy = nerf_random_image_pos_training(
      rng, resolution, snap_to_pixel_centers, cdf_x_cond_y, cdf_y,
      error_map_cdf_res, img, &xy_pdf);
  float max_level =
      max_level_rand_training
          ? (random_val(rng) * 2.0f)
          : 1.0f;  // Multiply by 2 to ensure 50% of training is at max level
  ...
  Array4f texsamp = read_rgba(xy, resolution, metadata[img].pixels,
                              metadata[img].image_data_type);
  ...
  network_output -=
      padded_output_width * compacted_numsteps;  // rewind the pointer
  ...
  float target_depth = rays_in_unnormalized[i].d.norm() *
                       ((depth_supervision_lambda > 0.0f && metadata[img].depth)
                            ? read_depth(xy, resolution, metadata[img].depth)
                            : -1.0f);
  float depth_loss_gradient =
      target_depth > 0.0f
          ? (depth_ray - target_depth) * 2.f * depth_supervision_lambda
          : 0.0f;
  ...
  if (weight_sum >= 1.0 - 1e-4) {
    ...
    gradient_weight_sum =
        (mask_gt - sigmoid_weight_sum) * weight_sum * mask_loss_weight;
  }
  ...
  // want to change the weighting of the loss function. So don't divide.
  ...
  if (mask_loss_output) {
    mask_loss_output[i] =
        -(mask_gt * log(weight_sum) + (1 - mask_gt) * log(1 - weight_sum));
  }
  ...
  if (error_map) {
    const Vector2f pos = (xy.cwiseProduct(error_map_res.cast<float>()) -
                          Vector2f::Constant(0.5f))
                             .cwiseMax(0.0f)
    ...
    if (sharpness_data && aabb.contains(hitpoint)) {
      ...
      float sharp =
          sharpness_data[img * sharpness_resolution.prod() +
      ...
      float grid_sharp = __uint_as_float(
          atomicMax((uint32_t *)&cascaded_grid_at(hitpoint, sharpness_grid,
                                                  mip_from_pos(hitpoint)),
                    __float_as_uint(sharp)));
      grid_sharp =
          fmaxf(sharp, grid_sharp);  // atomicMax returns the old value, so
                                     // compute the new one locally.

      mean_loss *= fmaxf(sharp / grid_sharp, 0.01f);
    }

    deposit_val(idx.x(), idx.y(),
                (1 - weight.x()) * (1 - weight.y()) * mean_loss);
    deposit_val(idx.x() + 1, idx.y(),
                weight.x() * (1 - weight.y()) * mean_loss);
    deposit_val(idx.x(), idx.y() + 1,
                (1 - weight.x()) * weight.y() * mean_loss);
  ...
  for (uint32_t j = 0; j < compacted_numsteps; ++j) {
    ...
    local_dL_doutput[0] =
        loss_scale *
        (dloss_by_drgb.x() * network_to_rgb_derivative(local_network_output[0],
                                                       rgb_activation) +
         fmaxf(0.0f,
               output_l2_reg *
    ...
    local_dL_doutput[1] =
        loss_scale *
        (dloss_by_drgb.y() * network_to_rgb_derivative(local_network_output[1],
                                                       rgb_activation) +
         fmaxf(0.0f, output_l2_reg * (float)local_network_output[1]));
    local_dL_doutput[2] =
        loss_scale *
        (dloss_by_drgb.z() * network_to_rgb_derivative(local_network_output[2],
                                                       rgb_activation) +
    ...
    const float depth_supervision =
        depth_loss_gradient * (T * depth - depth_suffix);
    ...
    if (p_div_c <= 0.0f || p_div_c >= 1.0f) {
      ...
      dalpha_d_e_minus_sigmoidx =
          -(plus_e - 1 - 1e-5) /
          ((1 + 1e-5 + e_minus_sigmoid) * (1 + 1e-5 + e_minus_sigmoid));
    ...
    float dloss_dinvs =
        dloss_dalpha * (dalpha_d_e_minus_sigmoidx * d_e_minus_sigmoidx_dinvs +
                        d_alpha_d_plus_e * d_plus_e_dinvs);
    ...
    float gradient_norm = std::sqrt(pos_gradient[0] * pos_gradient[0] +
                                    pos_gradient[1] * pos_gradient[1] +
                                    pos_gradient[2] * pos_gradient[2] + 1e-6);
    float pos_gradient_norm_inv = 1 - 1 / gradient_norm;
    float dloss_dnormal_norm =
        dloss_dalpha *
        (dalpha_d_e_minus_sigmoidx * d_e_minus_sigmoidx_diter_cos +
         d_plus_e_iter_cos * d_alpha_d_plus_e) *
        d_iter_cos_true_cos;

    float dloss_dsdf =
        dloss_dalpha * dalpha_d_e_minus_sigmoidx * d_e_minus_sigmoidx_dsdf;
    ...
    BoundingBox ek_aabb =
        BoundingBox{Vector3f::Constant(0.5f), Vector3f::Constant(0.5f)};
    ...
    // ek_loss
    local_dL_doutput[4] =
        (tcnn::network_precision_t)(ek_loss_weight * 2 * original_loss_scale *
                                    pos_gradient_norm_inv * pos_gradient[0]);
    local_dL_doutput[5] =
        (tcnn::network_precision_t)(ek_loss_weight * 2 * original_loss_scale *
                                    pos_gradient_norm_inv * pos_gradient[1]);
    local_dL_doutput[6] =
        (tcnn::network_precision_t)(ek_loss_weight * 2 * original_loss_scale *
                                    pos_gradient_norm_inv * pos_gradient[2]);

    local_dL_doutput[7] =
        (tcnn::network_precision_t)(loss_scale * dloss_dvariance);
    ...
    float jacobian_x_x = (pos_gradient[1] * pos_gradient[1] +
                          pos_gradient[2] * pos_gradient[2]) /
                         (gradient_norm_3);
    ...
    local_dL_doutput[8] = (tcnn::network_precision_t)(dnormal_x * jacobian_x_x +
                                                      dnormal_y * jacobian_y_x +
                                                      dnormal_z * jacobian_z_x);

    float jacobian_y_y = (pos_gradient[0] * pos_gradient[0] +
                          pos_gradient[2] * pos_gradient[2]) /
                         (gradient_norm_3);
    ...
    local_dL_doutput[9] = (tcnn::network_precision_t)(dnormal_x * jacobian_x_y +
                                                      dnormal_y * jacobian_y_y +
                                                      dnormal_z * jacobian_z_y);

    float jacobian_z_z = (pos_gradient[0] * pos_gradient[0] +
                          pos_gradient[1] * pos_gradient[1]) /
                         (gradient_norm_3);
    ...
    local_dL_doutput[10] = (tcnn::network_precision_t)(
        dnormal_x * jacobian_x_z + dnormal_y * jacobian_y_z +
        dnormal_z * jacobian_z_z);

#else
    local_dL_doutput[8] =
        (tcnn::network_precision_t)(loss_scale * dloss_dnormal_norm * dir[0]);
    local_dL_doutput[9] =
        (tcnn::network_precision_t)(loss_scale * dloss_dnormal_norm * dir[1]);
    local_dL_doutput[10] =
        (tcnn::network_precision_t)(loss_scale * dloss_dnormal_norm * dir[2]);
#endif

    *(tcnn::vector_t<tcnn::network_precision_t, 16> *)dloss_doutput =
        local_dL_doutput;
  ...
  if (compacted_numsteps == numsteps && envmap_gradient) {
    ...
    float dloss_by_denvmap_alpha = dloss_by_dbackground.matrix().dot(
        -pre_envmap_background_color.matrix());

    dL_denvmap[3] = (tcnn::network_precision_t)0;

    deposit_envmap_gradient(dL_denvmap, envmap_gradient, envmap_resolution,
                            dir);
  }
}

__global__ void compute_cam_gradient_train_nerf(
    const uint32_t n_rays, const uint32_t n_rays_total, default_rng_t rng,
    const BoundingBox aabb, const uint32_t *__restrict__ rays_counter,
    const TrainingXForm *training_xforms, bool snap_to_pixel_centers,
    Vector3f *cam_pos_gradient, Vector3f *cam_rot_gradient,
    const uint32_t n_training_images,
    const TrainingImageMetadata *__restrict__ metadata,
    const uint32_t *__restrict__ ray_indices_in,
    const Ray *__restrict__ rays_in_unnormalized,
    uint32_t *__restrict__ numsteps_in, PitchedPtr<NerfCoordinate> coords,
    ...
    const Vector2i distortion_resolution, Vector2f *cam_focal_length_gradient,
    const float *__restrict__ cdf_x_cond_y, const float *__restrict__ cdf_y,
    const float *__restrict__ cdf_img, const Vector2i error_map_res) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= *rays_counter) {
    return;
  }
  ...
  uint32_t ray_idx = ray_indices_in[i];
  uint32_t img =
      image_idx(ray_idx, n_rays, n_rays_total, n_training_images, cdf_img);
  Eigen::Vector2i resolution = metadata[img].resolution;
  ...
  Vector2f xy = nerf_random_image_pos_training(
      rng, resolution, snap_to_pixel_centers, cdf_x_cond_y, cdf_y,
      error_map_res, img, &xy_pdf);

  if (distortion_gradient) {
    // Rotate ray gradient to obtain image plane gradient.
    // This has the effect of projecting the (already projected) ray gradient
    // from the tangent plane of the sphere onto the image plane (which is
    // correct!).
    ...
    deposit_image_gradient<2>(image_plane_gradient.head<2>() / xy_pdf,
                              distortion_gradient, distortion_gradient_weight,
                              distortion_resolution, xy);
  }
...
__global__ void compute_extra_dims_gradient_train_nerf(
    ...
    const uint32_t *__restrict__ rays_counter, float *extra_dims_gradient,
    uint32_t n_extra_dims, const uint32_t n_training_images,
    const uint32_t *__restrict__ ray_indices_in,
    uint32_t *__restrict__ numsteps_in,
    PitchedPtr<NerfCoordinate> coords_gradient,
...
__global__ void shade_kernel_nerf(
    const uint32_t n_elements, Array4f *__restrict__ rgba,
    float *__restrict__ depth, NerfPayload *__restrict__ payloads,
    ERenderMode render_mode, bool train_in_linear_colors,
    Array4f *__restrict__ frame_buffer, float *__restrict__ depth_buffer) {
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= n_elements) return;
  NerfPayload &payload = payloads[i];
  ...
  if (!train_in_linear_colors && (render_mode == ERenderMode::Shade ||
                                  render_mode == ERenderMode::Slice)) {
    // Accumulate in linear colors
    tmp.head<3>() = srgb_to_linear(tmp.head<3>());
  }

  frame_buffer[payload.idx] =
      tmp + frame_buffer[payload.idx] * (1.0f - tmp.w());
...
__global__ void compact_kernel_nerf(
    const uint32_t n_elements, Array4f *src_rgba, float *src_depth,
    NerfPayload *src_payloads, Array4f *dst_rgba, float *dst_depth,
    NerfPayload *dst_payloads, Array4f *dst_final_rgba, float *dst_final_depth,
    NerfPayload *dst_final_payloads, uint32_t *counter,
...
__global__ void init_rays_with_payload_kernel_nerf(
    uint32_t sample_index, NerfPayload *__restrict__ payloads,
    Vector2i resolution, Vector2f focal_length,
    Matrix<float, 3, 4> camera_matrix0, Matrix<float, 3, 4> camera_matrix1,
    Vector4f rolling_shutter, Vector2f screen_center, Vector3f parallax_shift,
    bool snap_to_pixel_centers, BoundingBox aabb, float plane_z, float dof,
    CameraDistortion camera_distortion, const float *__restrict__ envmap_data,
    const Vector2i envmap_resolution, Array4f *__restrict__ framebuffer,
    float *__restrict__ depthbuffer, const float *__restrict__ distortion_data,
    const Vector2i distortion_resolution, ERenderMode render_mode,
    const network_precision_t *rotation, const network_precision_t *transition,
    const Eigen::Vector3f first_frame_offset) {
  ...
  float ray_time =
      rolling_shutter.x() + rolling_shutter.y() * u + rolling_shutter.z() * v +
      rolling_shutter.w() * ld_random_val(sample_index, idx * 72239731);
  Ray ray = pixel_to_ray(
      sample_index, {x, y}, resolution, focal_length,
      camera_matrix0 * ray_time + camera_matrix1 * (1.f - ray_time),
      screen_center, parallax_shift, snap_to_pixel_centers, plane_z, dof,
      camera_distortion, distortion_data, distortion_resolution);
  ...
  if (rotation != nullptr && transition != nullptr) {
// global_movement_with_rotation<network_precision_t>(rotation, transition,
// ray.o, ray.d); apply global rotation and transition
#if rotation_reprensentation
    global_movement_with_rotation_6d<network_precision_t>(rotation, transition,
                                                          ray.o, ray.d);
#else
    global_movement_with_rotation_quaternion<network_precision_t>(
        rotation, transition, ray.o, ray.d);
#endif
    // ray.d = ray.d.normalized();
  }
  ...
  float t = fmaxf(aabb.ray_intersect(ray.o, ray.d).x(),
  ...
  if (render_mode == ERenderMode::Distortion) {
    ...
    if (distortion_data) {
      offset += read_image<2>(distortion_data, distortion_resolution,
...
__global__ void construct_cdf_2d(uint32_t n_images, uint32_t height,
                                 uint32_t width, const float *__restrict__ data,
...
__global__ void construct_cdf_1d(uint32_t n_images, uint32_t height,
                                 float *__restrict__ cdf_y,
...
__global__ void safe_divide(const uint32_t num_elements,
...
void Testbed::NerfTracer::init_rays_from_camera(
    uint32_t sample_index, uint32_t padded_output_width, uint32_t n_extra_dims,
    const Vector2i &resolution, const Vector2f &focal_length,
    const Matrix<float, 3, 4> &camera_matrix0,
    const Matrix<float, 3, 4> &camera_matrix1, const Vector4f &rolling_shutter,
    Vector2f screen_center, Vector3f parallax_shift, bool snap_to_pixel_centers,
    const BoundingBox &render_aabb, float plane_z, float dof,
    const CameraDistortion &camera_distortion, const float *envmap_data,
    const Vector2i &envmap_resolution, const float *distortion_data,
    const Vector2i &distortion_resolution, Eigen::Array4f *frame_buffer,
    float *depth_buffer, uint8_t *grid, int show_accel,
    float cone_angle_constant, ERenderMode render_mode, cudaStream_t stream,
    const network_precision_t *rotation, const network_precision_t *transition,
    const Eigen::Vector3f first_frame_offset) {
  ...
  init_rays_with_payload_kernel_nerf<<<blocks, threads, 0, stream>>>(
      sample_index, m_rays[0].payload, resolution, focal_length, camera_matrix0,
      camera_matrix1, rolling_shutter, screen_center, parallax_shift,
      snap_to_pixel_centers, render_aabb, plane_z, dof, camera_distortion,
      envmap_data, envmap_resolution, frame_buffer, depth_buffer,
      distortion_data, distortion_resolution, render_mode, rotation, transition,
      first_frame_offset);
  ...
  CUDA_CHECK_THROW(cudaMemsetAsync(
      m_rays[0].depth, 0, m_n_rays_initialized * sizeof(float), stream));

  linear_kernel(advance_pos_nerf, 0, stream, m_n_rays_initialized, render_aabb,
                camera_matrix1.col(2), focal_length, sample_index,
                m_rays[0].payload, grid, (show_accel >= 0) ? show_accel : 0,
                cone_angle_constant);
}

uint32_t Testbed::NerfTracer::trace(
    NerfNetwork<network_precision_t> &network, const BoundingBox &render_aabb,
    const BoundingBox &train_aabb, const uint32_t n_training_images,
    const TrainingXForm *training_xforms, const Vector2f &focal_length,
    float cone_angle_constant, const uint8_t *grid, ERenderMode render_mode,
    const Eigen::Matrix<float, 3, 4> &camera_matrix, float depth_scale,
    int visualized_layer, int visualized_dim, ENerfActivation rgb_activation,
    ENerfActivation density_activation, int show_accel, float min_transmittance,
    float glow_y_cutoff, int glow_mode, const float *extra_dims_gpu,
    cudaStream_t stream) {
  ...
  while (i < MARCH_ITER) {
    ...
    // Compact rays that did not diverge yet
    {
      ...
      linear_kernel(compact_kernel_nerf, 0, stream, n_alive, rays_tmp.rgba,
                    rays_tmp.depth, rays_tmp.payload, rays_current.rgba,
                    rays_current.depth, rays_current.payload, m_rays_hit.rgba,
                    m_rays_hit.depth, m_rays_hit.payload,
                    m_alive_counter.data(), m_hit_counter.data());
      CUDA_CHECK_THROW(cudaMemcpyAsync(&n_alive, m_alive_counter.data(),
    ...
    PitchedPtr<NerfCoordinate> input_data((NerfCoordinate *)m_network_input, 1,
                                          0, extra_stride);
    linear_kernel(generate_next_nerf_network_inputs, 0, stream, n_alive,
                  render_aabb, train_aabb, focal_length, camera_matrix.col(2),
                  rays_current.payload, input_data, n_steps_between_compaction,
                  grid, (show_accel >= 0) ? show_accel : 0, cone_angle_constant,
                  extra_dims_gpu);
    ...
    GPUMatrix<network_precision_t, RM> rgbsigma_matrix(
        (network_precision_t *)m_network_output, network.padded_output_width(),
        n_elements);
    ...
    if (render_mode == ERenderMode::Normals) {
      ...
      network.visualize_activation(stream, visualized_layer, visualized_dim,
                                   positions_matrix, positions_matrix);
    }
    linear_kernel(composite_kernel_nerf, 0, stream, n_alive, n_elements, i,
                  train_aabb, glow_y_cutoff, glow_mode, n_training_images,
                  training_xforms, camera_matrix, focal_length, depth_scale,
                  rays_current.rgba, rays_current.depth, rays_current.payload,
                  input_data, m_network_output, network.padded_output_width(),
                  n_steps_between_compaction, render_mode, grid, rgb_activation,
                  density_activation, show_accel, min_transmittance,
                  network.variance(), network.training_step(),
                  network.cos_anneal_ratio());

    i += n_steps_between_compaction;
  }
  uint32_t n_hit;
  CUDA_CHECK_THROW(cudaMemcpyAsync(&n_hit, m_hit_counter.data(),
                                   sizeof(uint32_t), cudaMemcpyDeviceToHost,
                                   stream));
  CUDA_CHECK_THROW(cudaStreamSynchronize(stream));
  return n_hit;
...
void Testbed::NerfTracer::enlarge(size_t n_elements,
  ...
  auto scratch = allocate_workspace_and_distribute<
      ...
      network_precision_t, float>(
      stream, &m_scratch_alloc, n_elements, n_elements, n_elements, n_elements,
  ...
  m_rays[0].set(std::get<0>(scratch), std::get<1>(scratch),
                std::get<2>(scratch), n_elements);
  m_rays[1].set(std::get<3>(scratch), std::get<4>(scratch),
                std::get<5>(scratch), n_elements);
  m_rays_hit.set(std::get<6>(scratch), std::get<7>(scratch),
                 std::get<8>(scratch), n_elements);
...
void Testbed::Nerf::Training::reset_extra_dims(default_rng_t &rng) {
  ...
  std::vector<float> extra_dims_cpu(
      n_extra_dims *
      (dataset.n_images + 1));  // n_images + 1 since we use an extra 'slot' for
                                // the inference latent code
  ...
  for (uint32_t i = 0; i < dataset.n_images; ++i) {
    Eigen::Vector3f light_dir =
        warp_direction(dataset.metadata[i].light_dir.normalized());
...
const float *Testbed::get_inference_extra_dims(cudaStream_t stream) const {
  ...
  const float *extra_dims_src = m_nerf.training.extra_dims_gpu.data() +
                                m_nerf.extra_dim_idx_for_inference *
                                    m_nerf.training.dataset.n_extra_dims();
  if (!m_nerf.training.dataset.has_light_dirs) {
    return extra_dims_src;
  }

  // the dataset has light directions, so we must construct a temporary buffer
  // and fill it as requested. we use an extra 'slot' that was pre-allocated for
  // us at the end of the extra_dims array.
  size_t size = m_nerf_network->n_extra_dims() * sizeof(float);
  float *dims_gpu =
      m_nerf.training.extra_dims_gpu.data() +
      m_nerf.training.dataset.n_images * m_nerf.training.dataset.n_extra_dims();
  CUDA_CHECK_THROW(cudaMemcpyAsync(dims_gpu, extra_dims_src, size,
                                   cudaMemcpyDeviceToDevice, stream));
  Eigen::Vector3f light_dir = warp_direction(m_nerf.light_dir.normalized());
  CUDA_CHECK_THROW(cudaMemcpyAsync(dims_gpu, &light_dir,
                                   min(size, sizeof(Eigen::Vector3f)),
                                   cudaMemcpyHostToDevice, stream));
  return dims_gpu;
}

void Testbed::render_nerf(CudaRenderBuffer &render_buffer,
                          const Vector2i &max_res, const Vector2f &focal_length,
                          const Matrix<float, 3, 4> &camera_matrix0,
                          const Matrix<float, 3, 4> &camera_matrix1,
                          const Vector4f &rolling_shutter,
                          const Vector2f &screen_center, cudaStream_t stream) {
  ...
  // Our motion vector code can't undo f-theta and grid distortions -- so don't
  // render these if DLSS is enabled.
  bool render_opencv_camera_distortion =
      m_nerf.render_with_camera_distortion &&
      (!render_buffer.dlss() ||
       m_nerf.render_distortion.mode == ECameraDistortionMode::Iterative);
  bool render_grid_camera_distortion =
      m_nerf.render_with_camera_distortion && !render_buffer.dlss();

  CameraDistortion camera_distortion = render_opencv_camera_distortion
                                           ? m_nerf.render_distortion
                                           : CameraDistortion{};

  m_nerf.tracer.init_rays_from_camera(
      render_buffer.spp(), m_network->padded_output_width(),
      m_nerf_network->n_extra_dims(), render_buffer.in_resolution(),
      focal_length, camera_matrix0, camera_matrix1, rolling_shutter,
      screen_center, get_scaled_parallax_shift(), m_snap_to_pixel_centers,
      m_render_aabb, plane_z, m_dof, camera_distortion,
      m_envmap.envmap->params_inference(), m_envmap.resolution,
      render_grid_camera_distortion ? m_distortion.map->params_inference()
                                    : nullptr,
      m_distortion.resolution, render_buffer.frame_buffer(),
      render_buffer.depth_buffer(), m_nerf.density_grid_bitfield.data(),
      m_nerf.show_accel, m_nerf.cone_angle_constant, render_mode, stream,
      m_predict_global_movement ? m_nerf_network->rotation()->params()
                                : nullptr,
      m_predict_global_movement ? m_nerf_network->transition()->params()
                                : nullptr,
  ...
  if (m_render_mode == ERenderMode::Slice) {
    ...
    n_hit = m_nerf.tracer.trace(
        *m_nerf_network, m_render_aabb, m_aabb,
        m_nerf.training.n_images_for_training,
        m_nerf.training.transforms.data(), focal_length,
        m_nerf.cone_angle_constant, m_nerf.density_grid_bitfield.data(),
        render_mode, camera_matrix1, depth_scale, m_visualized_layer,
        m_visualized_dimension, m_nerf.rgb_activation,
        m_nerf.density_activation, m_nerf.show_accel,
        m_nerf.rendering_min_transmittance, m_nerf.m_glow_y_cutoff,
        m_nerf.m_glow_mode, extra_dims_gpu, stream);
  }
  ...
  if (m_render_mode == ERenderMode::Slice) {
    ...
    linear_kernel(
        generate_nerf_network_inputs_at_current_position, 0, stream, n_hit,
        m_aabb, rays_hit.payload,
        PitchedPtr<NerfCoordinate>((NerfCoordinate *)m_nerf.vis_input.data(), 1,
                                   0, extra_stride),
        extra_dims_gpu);

    GPUMatrix<float> positions_matrix((float *)m_nerf.vis_input.data(),
                                      floats_per_coord, n_elements);
    GPUMatrix<float> rgbsigma_matrix((float *)m_nerf.vis_rgba.data(), 4,
                                     n_elements);

    if (m_visualized_dimension == -1) {
      m_network->inference(stream, positions_matrix, rgbsigma_matrix);
      linear_kernel(compute_nerf_density, 0, stream, n_hit,
                    m_nerf.vis_rgba.data(), m_nerf.rgb_activation,
                    m_nerf.density_activation);
    } else {
      m_network->visualize_activation(stream, m_visualized_layer,
                                      m_visualized_dimension, positions_matrix,
                                      rgbsigma_matrix);
    }

    linear_kernel(shade_kernel_nerf, 0, stream, n_hit, m_nerf.vis_rgba.data(),
                  nullptr, rays_hit.payload, m_render_mode,
                  m_nerf.training.linear_colors, render_buffer.frame_buffer(),
                  render_buffer.depth_buffer());
    return;
  }

  linear_kernel(shade_kernel_nerf, 0, stream, n_hit, rays_hit.rgba,
                rays_hit.depth, rays_hit.payload, m_render_mode,
                m_nerf.training.linear_colors, render_buffer.frame_buffer(),
                render_buffer.depth_buffer());
  ...
  if (render_mode == ERenderMode::Cost) {
    ...
    CUDA_CHECK_THROW(cudaMemcpyAsync(
        payloads_final_cpu.data(), rays_hit.payload,
        n_hit * sizeof(NerfPayload), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK_THROW(cudaStreamSynchronize(stream));
...
void Testbed::Nerf::Training::set_camera_intrinsics(int frame_idx, float fx,
                                                    float fy, float cx,
                                                    float cy, float k1,
                                                    float k2, float p1,
                                                    float p2) {
...
void Testbed::Nerf::Training::set_camera_extrinsics(
...
void Testbed::Nerf::Training::export_camera_extrinsics(
    const std::string &filename, bool export_extrinsics_in_quat_format) {
  tlog::info() << "Saving a total of " << n_images_for_training << " poses to "
               << filename;
  ...
  for (int i = 0; i < n_images_for_training; ++i) {
    ...
    if (export_extrinsics_in_quat_format) {
      ...
      const Eigen::Matrix<float, 4, 4> conv_coords_r{{1.f, 0.f, 0.f, 0.f},
                                                     ...
                                                     {0.f, 0.f, -1.f, 0.f},
                                                     {0.f, 0.f, 0.f, 1.f}};
      const Eigen::Matrix<float, 3, 4> p_quat =
          conv_coords_l * p_nerf * conv_coords_r;
...
Eigen::Matrix<float, 3, 4> Testbed::Nerf::Training::get_camera_extrinsics(
    int frame_idx) {
  if (frame_idx < 0 || frame_idx >= dataset.n_images) {
...
void Testbed::Nerf::Training::update_metadata(int first, int last) {
  ...
  CUDA_CHECK_THROW(
      cudaMemcpy(metadata_gpu.data() + first, dataset.metadata.data() + first,
                 n * sizeof(TrainingImageMetadata), cudaMemcpyHostToDevice));
}
...
void Testbed::Nerf::Training::update_transforms(int first, int last) {
  ...
  CUDA_CHECK_THROW(
      cudaMemcpy(transforms_gpu.data() + first, transforms.data() + first,
                 n * sizeof(TrainingXForm), cudaMemcpyHostToDevice));
}

void Testbed::create_empty_nerf_dataset(size_t n_images, int aabb_scale,
                                        bool is_hdr) {
  m_nerf.training.dataset =
      ngp::create_empty_nerf_dataset(n_images, aabb_scale, is_hdr);
  load_nerf();
...
void Testbed::load_nerf() {
  if (!m_data_path.empty()) {
    if (m_data_path.is_directory()) {
      for (const auto &path : fs::directory{m_data_path}) {
        if (path.is_file() &&
          ...
          if (idx == std::string::npos) all_json_paths.emplace_back(path);
        }
      }
    ...
    std::sort(all_json_paths.begin(), all_json_paths.end(),
  ...
  // scratch rather than starting from the distortion that's described by the
  // training data. m_nerf.training.dataset.camera_distortion = {};
  ...
  if (!is_pot(m_nerf.training.dataset.aabb_scale)) {
    throw std::runtime_error{
        std::string{
            "NeRF dataset's `aabb_scale` must be a power of two, but is "} +
...
void Testbed::load_nerf(uint32_t frame_time_idx, bool is_downsample) {
  if (!m_data_path.empty()) {
    auto transfer_to_downsample_json = [](const auto &path,
                                          const bool is_downsample) {
      if (!is_downsample) {
        return path.str();
      }
      std::string downsample_path =
          path.stem().str() + std::string{"_downsample.json"};
      printf("load downsample json: %s\n", downsample_path.c_str());
      return downsample_path;
    };

    std::vector<fs::path> tmp_json_paths;
    tmp_json_paths.emplace_back(transfer_to_downsample_json(
        all_json_paths[frame_time_idx], is_downsample));
...
void Testbed::reset_density_grid_nerf(cudaStream_t stream) {
  const uint32_t n_elements = NERF_GRIDSIZE() * NERF_GRIDSIZE() *
                              NERF_GRIDSIZE() * (m_nerf.max_cascade + 1);
  ...
  // Only cull away empty regions where no camera is looking when the cameras
  // are actually meaningful. if (!m_nerf.training.dataset.has_rays) {
  // 	linear_kernel(mark_untrained_density_grid, 0, stream, n_elements,
  // m_nerf.density_grid.data(),
  // m_nerf.training.n_images_for_training, 		m_nerf.training.metadata_gpu.data(),
  ...
  for (uint32_t i = 0; i < n_steps; ++i) {
    ...
    linear_kernel(generate_grid_samples_nerf_nonuniform, 0, stream,
                  ...
                  m_nerf.density_grid_ema_step, m_aabb,
                  m_nerf.density_grid.data(), density_grid_positions,
                  density_grid_indices, m_nerf.max_cascade + 1, -0.01f);
    m_nerf.training.density_grid_rng.advance();

    linear_kernel(
        generate_grid_samples_nerf_nonuniform, 0, stream,
        n_nonuniform_density_grid_samples, m_nerf.training.density_grid_rng,
        m_nerf.density_grid_ema_step, m_aabb, m_nerf.density_grid.data(),
        density_grid_positions + n_uniform_density_grid_samples,
        density_grid_indices + n_uniform_density_grid_samples,
        m_nerf.max_cascade + 1, NERF_MIN_OPTICAL_THICKNESS());
    m_nerf.training.density_grid_rng.advance();
    ...
    GPUMatrix<float> density_grid_position_matrix(
        (float *)density_grid_positions, sizeof(NerfPosition) / sizeof(float),
    ...
    linear_kernel(splat_grid_samples_nerf_max_nearest_neighbor, 0, stream,
                  n_density_grid_samples, density_grid_indices, mlp_out,
                  density_grid_tmp, m_nerf.rgb_activation,
                  m_nerf.density_activation);
    linear_kernel(ema_grid_samples_nerf, 0, stream, n_elements, decay,
                  m_nerf.density_grid_ema_step, m_nerf.density_grid.data(),
                  density_grid_tmp);
...
void Testbed::update_density_grid_nerf(
    float decay, uint32_t n_uniform_density_grid_samples,
    uint32_t n_nonuniform_density_grid_samples, cudaStream_t stream) {
  const uint32_t n_elements = NERF_GRIDSIZE() * NERF_GRIDSIZE() *
                              NERF_GRIDSIZE() * (m_nerf.max_cascade + 1);
  ...
  if (m_training_step == 0 || m_nerf.training.n_images_for_training !=
    ...
    if (false) {  // with bugs
      linear_kernel(
          mark_untrained_density_grid, 0, stream, n_elements,
          m_nerf.density_grid.data(), m_nerf.training.n_images_for_training,
          m_nerf.training.metadata_gpu.data(),
          m_nerf.training.transforms_gpu.data(), m_training_step == 0);
    } else {
      CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.density_grid.data(), 0,
                                       sizeof(float) * n_elements, stream));
    }
...
void Testbed::update_density_grid_mean_and_bitfield(cudaStream_t stream) {
  ...
  // { return fmaxf(val, 0.f) / (n_elements * NERF_CASCADES()); },
  // m_nerf.density_grid_mean.data(), n_elements * NERF_CASCADES(), stream);
  ...
  linear_kernel(grid_to_bitfield, 0, stream, n_elements / 8 * NERF_CASCADES(),
                ...
                m_nerf.density_grid.data(), m_nerf.density_grid_bitfield.data(),
...
void Testbed::Nerf::Training::Counters::prepare_for_training_steps(
    cudaStream_t stream) {
  ...
  CUDA_CHECK_THROW(
      cudaMemsetAsync(numsteps_counter.data(), 0, sizeof(uint32_t),
                      stream));  // clear the counter in the first slot
  CUDA_CHECK_THROW(
      cudaMemsetAsync(numsteps_counter_compacted.data(), 0, sizeof(uint32_t),
                      stream));  // clear the counter in the first slot
  CUDA_CHECK_THROW(
      cudaMemsetAsync(loss.data(), 0, sizeof(float) * rays_per_batch, stream));
  CUDA_CHECK_THROW(cudaMemsetAsync(ek_loss.data(), 0,
                                   sizeof(float) * rays_per_batch, stream));
  CUDA_CHECK_THROW(cudaMemsetAsync(mask_loss.data(), 0,
                                   sizeof(float) * rays_per_batch, stream));
}

float Testbed::Nerf::Training::Counters::update_after_training(
    uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
  ...
  if (get_loss_scalar) {
    loss_scalar = reduce_sum(loss.data(), rays_per_batch, stream) *
                  (float)measured_batch_size / (float)target_batch_size;
    ek_loss_scalar = reduce_sum(ek_loss.data(), rays_per_batch, stream) *
                     (float)measured_batch_size / (float)target_batch_size;
    mask_loss_scalar = reduce_sum(mask_loss.data(), rays_per_batch, stream) *
                       (float)measured_batch_size / (float)target_batch_size;
  }

  rays_per_batch = (uint32_t)((float)rays_per_batch * (float)target_batch_size /
                              (float)measured_batch_size);
  rays_per_batch = std::min(
      next_multiple(rays_per_batch, tcnn::batch_size_granularity), 1u << 18);

  return loss_scalar;
}

void Testbed::train_nerf(uint32_t target_batch_size, bool get_loss_scalar,
                         cudaStream_t stream) {
  ...
  if (m_nerf.training.include_sharpness_in_error) {
    ...
    if (m_nerf.training.sharpness_grid.size() < n_cells) {
      ...
      CUDA_CHECK_THROW(
          cudaMemsetAsync(m_nerf.training.sharpness_grid.data(), 0,
                          m_nerf.training.sharpness_grid.get_bytes(), stream));
    }
    ...
    if (m_training_step == 0) {
      ...
      linear_kernel(decay_sharpness_grid_nerf, 0, stream,
                    m_nerf.training.sharpness_grid.size(), 0.95f,
  ...
  if (m_nerf.training.n_steps_since_cam_update == 0) {
    CUDA_CHECK_THROW(cudaMemsetAsync(
        m_nerf.training.cam_pos_gradient_gpu.data(), 0,
        m_nerf.training.cam_pos_gradient_gpu.get_bytes(), stream));
    CUDA_CHECK_THROW(cudaMemsetAsync(
        m_nerf.training.cam_rot_gradient_gpu.data(), 0,
        m_nerf.training.cam_rot_gradient_gpu.get_bytes(), stream));
    CUDA_CHECK_THROW(cudaMemsetAsync(
        m_nerf.training.cam_exposure_gradient_gpu.data(), 0,
        m_nerf.training.cam_exposure_gradient_gpu.get_bytes(), stream));
    CUDA_CHECK_THROW(
        cudaMemsetAsync(m_distortion.map->gradients(), 0,
                        sizeof(float) * m_distortion.map->n_params(), stream));
    CUDA_CHECK_THROW(
        cudaMemsetAsync(m_distortion.map->gradient_weights(), 0,
                        sizeof(float) * m_distortion.map->n_params(), stream));
    CUDA_CHECK_THROW(cudaMemsetAsync(
        m_nerf.training.cam_focal_length_gradient_gpu.data(), 0,
        m_nerf.training.cam_focal_length_gradient_gpu.get_bytes(), stream));
  }
  ...
  if (train_extra_dims) {
    ...
    CUDA_CHECK_THROW(cudaMemsetAsync(
        m_nerf.training.extra_dims_gradient_gpu.data(), 0,
        m_nerf.training.extra_dims_gradient_gpu.get_bytes(), stream));
  }
  ...
  if (m_nerf.training.n_steps_since_error_map_update == 0 &&
    ...
    m_nerf.training.error_map.data.resize(
        ...
        m_nerf.training.dataset.n_images);
    CUDA_CHECK_THROW(cudaMemsetAsync(m_nerf.training.error_map.data.data(), 0,
  ...
  if (envmap_gradient) {
    CUDA_CHECK_THROW(
        cudaMemsetAsync(envmap_gradient, 0,
                        sizeof(float) * m_envmap.envmap->n_params(), stream));
  }

  train_nerf_step(
      target_batch_size, m_nerf.training.counters_rgb.rays_per_batch,
      ...
      m_nerf.training.counters_rgb.mask_loss.data(), m_training_stream);
  ...
  if (m_predict_global_movement) {  // we need predict global movement for the
                                    // next frames
    if (current_training_time_frame == 0) {
      m_canonical_training_step = m_training_step;
    } else {
      if (m_training_step <
          m_predict_global_movement_training_step) {  // we are still training
                                                      // the global movement
                                                      // prediction
        m_canonical_training_step ==
            min(first_frame_max_training_step,
                next_frame_max_training_step);  // the canonical training status
                                                // is still in the next frame
      } else {
        m_canonical_training_step =
            m_training_step -
            m_predict_global_movement_training_step;  // start training the
                                                      // canonical
      }
    }
  ...
  float loss_scalar = m_nerf.training.counters_rgb.update_after_training(
      target_batch_size, get_loss_scalar, stream);
  ...
  if (accumulate_error &&
    ...
    CUDA_CHECK_THROW(cudaMemsetAsync(
        m_nerf.training.error_map.cdf_x_cond_y.data(), 0,
        m_nerf.training.error_map.cdf_x_cond_y.get_bytes(), stream));
    CUDA_CHECK_THROW(
        cudaMemsetAsync(m_nerf.training.error_map.cdf_y.data(), 0,
                        m_nerf.training.error_map.cdf_y.get_bytes(), stream));
    CUDA_CHECK_THROW(
        cudaMemsetAsync(m_nerf.training.error_map.cdf_img.data(), 0,
                        m_nerf.training.error_map.cdf_img.get_bytes(), stream));

    const dim3 threads = {16, 8, 1};
    const dim3 blocks = {
        div_round_up((uint32_t)m_nerf.training.error_map.cdf_resolution.y(),
                     threads.x),
        div_round_up((uint32_t)m_nerf.training.dataset.n_images, threads.y), 1};
    ...
    linear_kernel(construct_cdf_1d, 0, stream, m_nerf.training.dataset.n_images,
                  m_nerf.training.error_map.cdf_resolution.y(),
                  m_nerf.training.error_map.cdf_y.data(),
  ...
  if (train_extra_dims) {
    std::vector<float> extra_dims_gradient(
        m_nerf.training.extra_dims_gradient_gpu.size());
    std::vector<float> &extra_dims_new_values =
        extra_dims_gradient;  // just create an alias to make the code clearer.
    ...
    for (uint32_t i = 0; i < m_nerf.training.n_images_for_training; ++i) {
      ...
      // m_nerf.training.extra_dims_opt[i].set_learning_rate(std::max(1e-3f *
      // std::pow(0.33f, (float)(m_nerf.training.extra_dims_opt[i].step() /
      // 128)), m_optimizer->learning_rate()/1000.0f));
    ...
    CUDA_CHECK_THROW(cudaMemcpyAsync(
        m_nerf.training.extra_dims_gpu.data(), extra_dims_new_values.data(),
        m_nerf.training.n_images_for_training * n_extra_dims * sizeof(float),
        cudaMemcpyHostToDevice, stream));
  }
  ...
  if (train_camera && m_nerf.training.n_steps_since_cam_update >=
    ...
    if (m_nerf.training.optimize_distortion) {
      linear_kernel(safe_divide, 0, stream, m_distortion.map->n_params(),
    ...
    if (m_nerf.training.optimize_focal_length) {
      ...
      m_nerf.training.cam_focal_length_offset.set_learning_rate(std::max(
          1e-3f *
              std::pow(0.33f,
                       (float)(m_nerf.training.cam_focal_length_offset.step() /
                               128)),
          m_optimizer->learning_rate() / 1000.0f));
    ...
    if (m_nerf.training.optimize_exposure) {
      ...
      CUDA_CHECK_THROW(cudaMemcpyAsync(
          m_nerf.training.cam_exposure_gpu.data(), cam_exposures.data(),
          m_nerf.training.cam_exposure_gpu.get_bytes(), cudaMemcpyHostToDevice,
          stream));
    }
...
void Testbed::train_nerf_step(uint32_t target_batch_size,
                              uint32_t n_rays_per_batch, uint32_t *counter,
                              uint32_t *compacted_counter, float *loss,
                              float *ek_loss, float *mask_loss,
                              cudaStream_t stream) {
  const uint32_t padded_output_width = m_network->padded_output_width();
  const uint32_t max_samples =
      target_batch_size * 16;  // Somewhat of a worst case
  const uint32_t floats_per_coord =
      sizeof(NerfCoordinate) / sizeof(float) + m_nerf_network->n_extra_dims();
  const uint32_t extra_stride =
      m_nerf_network->n_extra_dims() *
  ...
  auto scratch =
      allocate_workspace_and_distribute<uint32_t,             // ray_indices
                                        ...
                                        >(
          stream, &alloc, n_rays_per_batch, n_rays_per_batch,
          n_rays_per_batch * 2, max_samples * floats_per_coord, max_samples,
          ...
          target_batch_size * floats_per_coord, target_batch_size, 1);
  ...
  GPUMatrix<float> coords_matrix((float *)coords, floats_per_coord,
                                 max_inference);
  GPUMatrix<network_precision_t> rgbsigma_matrix(mlp_out, padded_output_width,
                                                 max_inference);

  GPUMatrix<float> compacted_coords_matrix((float *)coords_compacted,
                                           floats_per_coord, target_batch_size);
  ...
  linear_kernel(
      generate_training_samples_nerf_with_global_movement, 0, stream,
      n_rays_per_batch, m_aabb, max_inference, n_rays_total, m_rng, ray_counter,
      counter, ray_indices, rays_unnormalized, numsteps,
      PitchedPtr<NerfCoordinate>((NerfCoordinate *)coords, 1, 0, extra_stride),
      m_nerf.training.n_images_for_training,
      m_nerf.training.metadata_gpu.data(),
      m_nerf.training.transforms_gpu.data(),
      m_nerf.density_grid_bitfield.data(), m_max_level_rand_training, max_level,
      m_nerf.training.snap_to_pixel_centers, m_nerf.training.train_envmap,
      m_nerf.cone_angle_constant, m_distortion.map->params(),
      ...
      sample_image_proportional_to_error
          ...
          : nullptr,
      m_nerf.training.error_map.cdf_resolution,
      m_nerf.training.extra_dims_gpu.data(), m_nerf_network->n_extra_dims(),
      current_training_time_frame, m_training_step,
      m_predict_global_movement ? m_nerf_network->rotation()->params()
                                : nullptr,
      m_predict_global_movement ? m_nerf_network->transition()->params()
                                : nullptr,
  ...
  auto hg_enc =
      dynamic_cast<GridEncoding<network_precision_t> *>(m_encoding.get());
  if (hg_enc) {
    hg_enc->set_max_level_gpu(m_max_level_rand_training ? max_level : nullptr);
  }

  m_network->inference_mixed_precision(stream, coords_matrix, rgbsigma_matrix,
                                       false);
  ...
  linear_kernel(
      compute_loss_kernel_train_nerf_with_global_movement, 0, stream,
      n_rays_per_batch, m_aabb, n_rays_total, m_rng, target_batch_size,
      ray_counter, LOSS_SCALE, padded_output_width, m_envmap.envmap->params(),
      envmap_gradient, m_envmap.resolution, m_envmap.loss_type,
      m_background_color.head<3>(), m_color_space,
      m_nerf.training.random_bg_color, m_nerf.training.linear_colors,
      m_nerf.training.n_images_for_training,
      m_nerf.training.metadata_gpu.data(), mlp_out, compacted_counter,
      ray_indices, rays_unnormalized, numsteps,
      PitchedPtr<const NerfCoordinate>((NerfCoordinate *)coords, 1, 0,
                                       extra_stride),
      PitchedPtr<NerfCoordinate>((NerfCoordinate *)coords_compacted, 1, 0,
                                 extra_stride),
      dloss_dmlp_out, m_nerf.training.loss_type, loss, ek_loss, mask_loss,
      m_max_level_rand_training, max_level_compacted, m_nerf.rgb_activation,
      m_nerf.density_activation, m_nerf.training.snap_to_pixel_centers,
      ...
      m_nerf.training.sharpness_grid.data(), m_nerf.density_grid.data(),
      m_nerf.density_grid_mean.data(), m_nerf.training.cam_exposure_gpu.data(),
      m_nerf.training.optimize_exposure
          ? m_nerf.training.cam_exposure_gradient_gpu.data()
          : nullptr,
      m_nerf.training.depth_supervision_lambda, m_nerf.training.near_distance,
      m_nerf_network->variance(), m_training_step,
      m_predict_global_movement ? m_nerf_network->rotation()->params()
                                : nullptr,
      m_predict_global_movement ? m_nerf_network->transition()->params()
                                : nullptr,
      m_first_frame_offset, m_mask_loss_weight, m_ek_loss_weight,
  ...
  fill_rollover_and_rescale<network_precision_t>
      <<<n_blocks_linear(target_batch_size * padded_output_width),
         n_threads_linear, 0, stream>>>(target_batch_size, padded_output_width,
                                        compacted_counter, dloss_dmlp_out);
  fill_rollover<float><<<n_blocks_linear(target_batch_size * floats_per_coord),
                         n_threads_linear, 0, stream>>>(
      target_batch_size, floats_per_coord, compacted_counter,
      (float *)coords_compacted);
  fill_rollover<float>
  ...
  GPUMatrix<float> coords_gradient_matrix((float *)coords_gradient,
                                          floats_per_coord, target_batch_size);
  std::vector<uint32_t> compacted_counter_cpu(1);
  CUDA_CHECK_THROW(cudaMemcpyAsync(compacted_counter_cpu.data(),
                                   compacted_counter, sizeof(uint32_t) * 1,
                                   cudaMemcpyDeviceToHost, stream));
  std::vector<uint32_t> counter_cpu(1);
  CUDA_CHECK_THROW(cudaMemcpyAsync(counter_cpu.data(), counter,
  ...
  {
    auto ctx = m_network->forward(stream, compacted_coords_matrix,
                                  &compacted_rgbsigma_matrix, false,
                                  prepare_input_gradients);
    m_network->backward(
        stream, *ctx, compacted_coords_matrix, compacted_rgbsigma_matrix,
        gradient_matrix,
        prepare_input_gradients ? &coords_gradient_matrix : nullptr, false,
  ...
  if (train_extra_dims) {
    ...
    linear_kernel(compute_extra_dims_gradient_train_nerf, 0, stream,
                  n_rays_per_batch, n_rays_total, ray_counter,
                  m_nerf.training.extra_dims_gradient_gpu.data(),
                  m_nerf.training.dataset.n_extra_dims(),
                  m_nerf.training.n_images_for_training, ray_indices, numsteps,
                  PitchedPtr<NerfCoordinate>((NerfCoordinate *)coords_gradient,
                                             1, 0, extra_stride),
                  sample_image_proportional_to_error
                      ? m_nerf.training.error_map.cdf_img.data()
                      : nullptr);
  ...
  if (train_camera) {
    ...
    linear_kernel(compute_cam_gradient_train_nerf, 0, stream, n_rays_per_batch,
                  n_rays_total, m_rng, m_aabb, ray_counter,
                  m_nerf.training.transforms_gpu.data(),
                  ...
                  m_nerf.training.optimize_extrinsics
                      ...
                      : nullptr,
                  m_nerf.training.n_images_for_training,
                  m_nerf.training.metadata_gpu.data(), ray_indices,
                  rays_unnormalized, numsteps,
...
void Testbed::training_prep_nerf(uint32_t batch_size, cudaStream_t stream) {
  ...
  if (m_canonical_training_step < 256) {
    update_density_grid_nerf(
        alpha, NERF_GRIDSIZE() * NERF_GRIDSIZE() * NERF_GRIDSIZE() * n_cascades,
        0, stream);
  } else {
    update_density_grid_nerf(
        alpha,
...
void Testbed::optimise_mesh_step(uint32_t n_steps) {
  ...
  GPUMatrix<float> positions_matrix((float *)coords.data(), floats_per_coord,
                                    n_verts);
  GPUMatrix<network_precision_t, RM> density_matrix(
  ...
  for (uint32_t i = 0; i < n_steps; ++i) {
    linear_kernel(generate_nerf_network_inputs_from_positions, 0,
                  m_inference_stream, n_verts, m_aabb, m_mesh.verts.data(),
                  PitchedPtr<NerfCoordinate>((NerfCoordinate *)coords.data(), 1,
                                             0, extra_stride),
                  extra_dims_gpu);
    ...
    // ...as well as the input gradient w.r.t. density, which we will store in
    // the nerf coords.
    m_nerf_network->input_gradient(m_inference_stream, 3, positions_matrix,
                                   positions_matrix);
    // and the 1ring centroid for laplacian smoothing
    compute_mesh_1ring(m_mesh.verts, m_mesh.indices, m_mesh.verts_smoothed,
                       m_mesh.vert_normals);

    // With these, we can compute a gradient that points towards the
    // threshold-crossing of density...
    compute_mesh_opt_gradients(m_mesh.thresh, m_mesh.verts, m_mesh.vert_normals,
                               m_mesh.verts_smoothed, mlp_out.data(),
                               floats_per_coord, (const float *)coords.data(),
                               m_mesh.verts_gradient, m_mesh.smooth_amount,
                               m_mesh.density_amount, m_mesh.inflate_amount);

    // ...that we can pass to the optimizer.
    m_mesh.verts_optimizer->step(
        m_inference_stream, 1.0f, (float *)m_mesh.verts.data(),
        (float *)m_mesh.verts.data(), (float *)m_mesh.verts_gradient.data());
  }
}
...
void Testbed::compute_mesh_vertex_colors() {
  ...
  if (m_testbed_mode == ETestbedMode::Nerf) {
    ...
    linear_kernel(extract_srgb_with_activation, 0, m_inference_stream,
                  n_verts * 3, 3, mlp_out.data(),
                  (float *)m_mesh.vert_colors.data(), m_nerf.rgb_activation,
...
GPUMemory<float> Testbed::get_density_on_grid(Vector3i res3d,
  ...
  GPUMemoryArena::Allocation alloc;
  auto scratch =
      allocate_workspace_and_distribute<NerfPosition, network_precision_t>(
          m_inference_stream, &alloc, n_elements,
          batch_size * padded_output_width);
  ...
  generate_grid_samples_nerf_uniform<<<blocks, threads, 0,
                                       m_inference_stream>>>(
      res3d, m_nerf.density_grid_ema_step, aabb, nerf_mode ? m_aabb : unit_cube,
      positions);
  ...
  for (uint32_t offset = 0; offset < n_elements; offset += batch_size) {
    ...
    if (nerf_mode) {
      ...
      m_network->inference_mixed_precision(m_inference_stream, positions_matrix,
                                           density_matrix);
    }
    linear_kernel(grid_samples_half_to_float, 0, m_inference_stream,
                  local_batch_size, m_aabb,
                  density.data() + offset,  //+ axis_step * n_elements,
                  mlp_out, m_nerf.density_activation, positions + offset,
...
GPUMemory<Eigen::Array4f> Testbed::get_rgba_on_grid(Vector3i res3d,
  ...
  const dim3 blocks = {div_round_up((uint32_t)res3d.x(), threads.x),
                       div_round_up((uint32_t)res3d.y(), threads.y),
                       div_round_up((uint32_t)res3d.z(), threads.z)};
  generate_grid_samples_nerf_uniform_dir<<<blocks, threads, 0,
                                           m_inference_stream>>>(
      res3d, m_nerf.density_grid_ema_step, m_render_aabb, m_aabb, ray_dir,
      positions.data());
  ...
  for (uint32_t offset = 0; offset < n_elements; offset += batch_size) {
    ...
    GPUMatrix<float> positions_matrix((float *)(positions.data() + offset),
                                      ...
                                      local_batch_size);
    GPUMatrix<float> rgbsigma_matrix((float *)(rgba.data() + offset), 4,
                                     local_batch_size);
    m_network->inference(m_inference_stream, positions_matrix, rgbsigma_matrix);

    // convert network output to RGBA (in place)
    linear_kernel(compute_nerf_density, 0, m_inference_stream, local_batch_size,
                  rgba.data() + offset, m_nerf.rgb_activation,
                  m_nerf.density_activation);
  }
  return rgba;
}

int Testbed::marching_cubes(Vector3i res3d, const BoundingBox &aabb,
                            float thresh) {
...
#if rotation_reprensentation
  tcnn::linear_kernel(
      transform_mesh_with_6d<tcnn::network_precision_t>, 0, m_training_stream,
      n_verts, m_nerf_network->rotation()->params(),
      m_nerf_network->transition()->params(), (float *)m_mesh.verts.data());
#else
  tcnn::linear_kernel(
      transform_mesh_with_quaternion<tcnn::network_precision_t>, 0,
      m_training_stream, n_verts, m_nerf_network->rotation()->params(),
      m_nerf_network->transition()->params(), (float *)m_mesh.verts.data());
#endif

  m_mesh.verts_gradient.resize(n_verts);

  m_mesh.trainable_verts = std::make_shared<TrainableBuffer<3, 1, float>>(
      Matrix<int, 1, 1>{(int)n_verts});
  m_mesh.verts_gradient.copy_from_device(
      m_mesh.verts);  // Make sure the vertices don't get destroyed in the
                      // initialization

  pcg32 rnd{m_seed};
  m_mesh.trainable_verts->initialize_params(
      rnd, (float *)m_mesh.verts.data(), (float *)m_mesh.verts.data(),
      (float *)m_mesh.verts.data(), (float *)m_mesh.verts.data(),
...
int Testbed::find_best_training_view(int default_view) {
  ...
  for (int i = 0; i < m_nerf.training.n_images_for_training; ++i) {
    float score =
        (m_nerf.training.transforms[i].start.col(3) - m_camera.col(3)).norm();
    score +=
        0.25f *
        (m_nerf.training.transforms[i].start.col(2) - m_camera.col(2)).norm();
    if (score < bestscore) {
      bestscore = score;
      bestimage = i;
    }
