...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(
        log_dir=os.path.join('output', args.name, 'logs', time_name))
    ...
    if args.test:
        ...
        if args.save_mesh_path and args.save_mesh:
            ...
            print(
                f"Generating mesh via marching cubes and saving to {args.save_mesh_path}. Resolution=[{res},{res},{res}]"
            )
            testbed.compute_and_save_marching_cubes_mesh(
                args.save_mesh_path, [res, res, res])
            ...
            with tqdm(desc="Training", total=n_steps, unit="step") as t:
                while testbed.frame():
                    ...
                    if testbed.training_step % 20 == 0:
                        writer.add_scalar('loss/rgb_loss', testbed.loss,
                                          testbed.training_step)
                        writer.add_scalar('loss/ek_loss', testbed.ek_loss,
                                          testbed.training_step)
                        writer.add_scalar('loss/mask_loss', testbed.mask_loss,
                                          testbed.training_step)
        ...
        testbed.compute_and_save_marching_cubes_mesh(args.save_mesh_path,
                                                     [res, res, res])
            ...
            with tqdm(list(enumerate(test_transforms["frames"])),
                ...
                for i, frame in t:
                    ...
                    if testbed.color_space == ngp.ColorSpace.SRGB and ref_image.shape[
                        ...
                        ref_image += (1.0 - ref_image[..., 3:4]
                                     ) * testbed.background_color
                    ...
                    image = testbed.render(ref_image.shape[1],
                                           ref_image.shape[0], spp, True)
        ...
        if args.width:
            if ref_transforms:
                ...
                if not args.screenshot_frames:
                    args.screenshot_frames = range(len(
                ...
                for idx in args.screenshot_frames:
                    ...
                    image = testbed.render(
                        args.width or int(ref_transforms["w"]), args.height or
                        int(ref_transforms["h"]), args.screenshot_spp, True)
                ...
                image = testbed.render(args.width, args.height,
