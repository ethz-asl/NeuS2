...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "Save this snapshot after training every frame. recommended extension: .msgpack"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Number of steps to train for before quitting.")  # not used
...
if __name__ == "__main__":
    ...
    writer_all = SummaryWriter(
        log_dir=os.path.join('output', args.name, 'logs', time_name, 'all'))
    ...
    def file_backup(output_path, config_path):
        ...
        filepath = os.path.join('include', 'neural-graphics-primitives',
                                'nerf_network.h')
        if os.path.exists(filepath):
    ...
    if args.dynamic_test:
        ...
        while True:
            if os.path.isdir(args.scene):
                all_transform_path = sorted(
                    list(
                        filter(
                            lambda path: "downsample" not in path.split('/')[-1
                                                                            ],
                            glob.glob(args.scene + '/*.json'))))
            else:
                all_transform_path = [args.scene]
            if args.load_snapshot != "":
                load_snapshot_path = os.path.join(
                    args.load_snapshot, "checkpoints"
                ) + f"/frame_{testbed.current_training_time_frame}.msgpack"
            else:
                load_snapshot_path = os.path.join(
                    args.output_path, "checkpoints"
                ) + f"/frame_{testbed.current_training_time_frame}.msgpack"
            ...
            if args.dynamic_save_mesh:
                ...
                save_mesh_path = os.path.join(
                    args.output_path, "evaluation", "mesh",
                    f"frame_{testbed.current_training_time_frame:04}.obj")
                os.makedirs(os.path.join(args.output_path, "evaluation",
                                         "mesh"),
                            exist_ok=True)

                print(
                    f"Generating mesh via marching cubes and saving to {save_mesh_path}. Resolution=[{res},{res},{res}]"
                )
                aabb = ngp.BoundingBox(np.array([0, args.bbox_min, 0]),
                                       np.array([1.0, args.bbox_max, 1.0]))
                testbed.compute_and_save_marching_cubes_mesh(save_mesh_path,
                                                             [res, res, res],
                                                             aabb=aabb)
                if args.dynamic_save_mesh_only:
                    if not testbed.training_network_next_frame(
                    ) and testbed.current_training_time_frame >= testbed.all_training_time_frame - 1:
                        break
                    continue
            ...
            ref_images = load_ref_images(
                args, all_transform_path[testbed.current_training_time_frame])
            log_ptr, avg_psnr_this_frame = cal_psnr(
                testbed,
                ref_images,
                1,
                True,
                save_dir=os.path.join(
                    args.output_path, "evaluation", "psnr",
                    f"frame_{testbed.current_training_time_frame}"),
                spp=8,
                white_bkgd=args.white_bkgd)
            all_results[
                'frame_' +
                str(testbed.current_training_time_frame)] = avg_psnr_this_frame
            all_psnr += avg_psnr_this_frame
            all_frame += 1

            with open(
                    os.path.join(
                        args.output_path, "evaluation",
                        f"dynamic_{testbed.current_training_time_frame}_eval_log.json"
                    ), "w") as f:
        ...
        with open(
                os.path.join(args.output_path, "evaluation",
                             "dynamic_eval_log.json"), "w") as f:
        ...
        args.save_snapshot = os.path.join(args.output_path, "checkpoints",
                                          f"{n_steps}.msgpack")
        args.save_mesh_path = os.path.join(args.output_path, "mesh",
                                           f"{n_steps}.obj")
        ...
        if os.path.isdir(args.scene):
            all_transform_path = sorted(
                list(
                    filter(lambda path: "downsample" not in path.split('/')[-1],
                           glob.glob(args.scene + '/*.json'))))
        else:
            all_transform_path = [args.scene]
        all_pred_img = []
        ...
        writer_frame = SummaryWriter(log_dir=os.path.join(
            'output', args.name, 'logs', time_name, f'frame_{0}'))
        if n_steps > 0:
            now = time.monotonic()
            ...
            while testbed.frame():
                ...
                if total_steps % 20 == 0:
                    print(
                        f"training time_frame: {testbed.current_training_time_frame}, training step: {testbed.training_step}, training_time: {time.monotonic()-print_now}, trainig_rgb_loss:{testbed.loss}"
                    )
                    writer_all.add_scalar('loss/rgb_loss', testbed.loss,
                                          total_steps)
                    writer_all.add_scalar('loss/ek_loss', testbed.ek_loss,
                                          total_steps)
                    writer_all.add_scalar('loss/mask_loss', testbed.mask_loss,
                                          total_steps)

                    writer_frame.add_scalar('loss/rgb_loss', testbed.loss,
                                            testbed.training_step)
                    writer_frame.add_scalar('loss/ek_loss', testbed.ek_loss,
                                            testbed.training_step)
                    writer_frame.add_scalar('loss/mask_loss', testbed.mask_loss,
                                            testbed.training_step)

                if (testbed.current_training_time_frame == 0 and
                        testbed.training_step
                        == testbed.first_frame_max_training_step) or (
                            testbed.current_training_time_frame != 0 and
                            testbed.training_step
                            == testbed.next_frame_max_training_step):

                    print(
                        f"training frame {testbed.current_training_time_frame} for time :{time.monotonic() - now}\n",
                        file=training_log_ptr)

                    # change to next frame: do some validate
                    if args.save_snapshot_per_frame:
                        save_snapshot_path = os.path.join(
                            args.output_path, "checkpoints",
                            f"frame_{testbed.current_training_time_frame}.msgpack"
                        )
                    ...
                    os.makedirs(os.path.join(args.output_path,
                                             "pred_transform"),
                                exist_ok=True)
                    save_transform_path = os.path.join(
                        args.output_path, "pred_transform",
                        f"frame_{testbed.current_training_time_frame}.txt")
                    ...
                    if args.save_mesh:
                        ...
                        testbed.compute_and_save_marching_cubes_mesh(
                            save_mesh_path, [res, res, res])
                        args.save_mesh_path = save_mesh_path

                    training_log_ptr.flush()
                    pred_img, gt_img, pred_mesh = render_img_training_view(
                        args, testbed, eval_log_ptr,
                        all_transform_path[testbed.current_training_time_frame],
                    ...
                    if pred_mesh is not None:
                        all_pred_mesh.append(pred_mesh[:, :, [2, 1, 0]].astype(
                            np.uint8))

                    # update frame writer
                    if testbed.current_training_time_frame != testbed.all_training_time_frame - 1:
                        writer_frame = SummaryWriter(log_dir=os.path.join(
                            'output', args.name, 'logs', time_name,
                            f'frame_{testbed.current_training_time_frame + 1}'))
                        if args.test_psnr:
                            ref_images = load_ref_images(
                                args, all_transform_path[
                                    testbed.current_training_time_frame + 1])
                    now = time.monotonic()
                    print_now = time.monotonic()

                if (testbed.current_training_time_frame > 0 and
                        testbed.current_training_time_frame
                        == testbed.all_training_time_frame - 1 and
                        testbed.training_step
                        >= testbed.next_frame_max_training_step) or (
                            testbed.current_training_time_frame == 0 and
                            testbed.current_training_time_frame
                            == testbed.all_training_time_frame - 1 and
                            testbed.training_step
            ...
            video_path = os.path.join(args.output_path, 'video',
                                      f'{args.test_camera_view:04}')
            ...
            imageio.mimsave(os.path.join(video_path, 'pred_img.gif'),
                            all_pred_img,
                            fps=10)
            imageio.mimsave(os.path.join(video_path, 'gt_img.gif'),
                            all_gt_img,
                            fps=10)
            if len(all_pred_mesh) > 0:
                imageio.mimsave(os.path.join(video_path, 'pred_mesh.gif'),
                                all_pred_mesh,
                                fps=10)
