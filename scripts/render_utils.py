...
def cal_psnr(testbed,
    ...
    for camera_view, ref_image in tqdm(enumerate(ref_images)):
        ...
        if testbed.color_space == ngp.ColorSpace.SRGB and ref_image.shape[
            ...
            ref_image[..., :3] = np.divide(ref_image[..., :3],
                                           ref_image[..., 3:4],
                                           out=np.zeros_like(
                                               ref_image[..., :3]),
        ...
        image = testbed.render(ref_image.shape[1], ref_image.shape[0], spp,
                               True)
        ...
        if save_image:
            write_image(
                os.path.join(save_dir, "psnr_{:04d}.png".format(camera_view)),
                image)
            write_image(
                os.path.join(save_dir, "gt_{:04d}.png".format(camera_view)),
                ref_image)
        psnr_l.append(psnr)
...
def render_img_training_view(args,
                             testbed,
                             log_ptr,
                             image_dir,
                             frame_time_id=0,
    ...
    if training_step < 0:
        write_image(join(img_path, f"frame_{frame_time_id:06}_gt.png"),
                    ref_image)
    else:
        os.makedirs(os.path.join(img_path, f"frame_{frame_time_id:06}"),
                    exist_ok=True)
        write_image(
            join(img_path, f"frame_{frame_time_id:06}",
                 f"frame_{frame_time_id:06}_{training_step}_gt.png"), ref_image)

    H = ref_image.shape[0]
    W = ref_image.shape[1]  # original H W
    if args.render_img_HW is not None:
        # resize ref_image
        ref_image = cv2.resize(ref_image,
                               (args.render_img_HW, args.render_img_HW),
    ...
    if training_step < 0:
        ...
        write_image(
            join(img_path, f"frame_{frame_time_id:06}",
                 f"frame_{frame_time_id:06}_{training_step}_pred.png"), image)

    diffimg = np.absolute(image - ref_image)
    diffimg[..., 3:4] = 1.0

    if training_step < 0:
        write_image(join(img_path, f"frame_{frame_time_id:06}_diff.png"),
                    diffimg)
    else:
        write_image(
            join(img_path, f"frame_{frame_time_id:06}",
                 f"frame_{frame_time_id:06}_{training_step}_diff.png"), diffimg)
        return
    ...
    print(
        f"camera_view:{camera_view}, frame_time:{frame_time_id}: PSNR={psnr} SSIM={ssim}",
        file=log_ptr)
    ...
    if args.save_mesh:
        ...
        if 'intrinsic_matrix' in frame:
            ...
            if 'fl_x' in test_transforms or 'fl_y' in test_transforms:
                fl_x = (test_transforms['fl_x'] if 'fl_x' in test_transforms
                        else test_transforms['fl_y']) / downscale
                fl_y = (test_transforms['fl_y'] if 'fl_y' in test_transforms
                        else test_transforms['fl_x']) / downscale
            elif 'camera_angle_x' in test_transforms or 'camera_angle_y' in test_transforms:
                # blender, assert in radians. already downscaled since we use H/W
                fl_x = W / (2 * np.tan(test_transforms['camera_angle_x'] / 2)
                           ) if 'camera_angle_x' in test_transforms else None
                fl_y = H / (2 * np.tan(test_transforms['camera_angle_y'] / 2)
                           ) if 'camera_angle_y' in test_transforms else None
                if fl_x is None:
                    fl_x = fl_y
                if fl_y is None:
                    fl_y = fl_x
                ...
                raise RuntimeError(
                    'Failed to load focal length, please check the test_transformss.json!'
                )

            ixt = torch.tensor([[fl_x, 0, cx, 0], [0, fl_y, cy, 0],
                                [0, 0, 1, 0], [0, 0, 0, 1]])

        scale_ratio = torch.tensor(
            [renderer_resolution / W, renderer_resolution / H])
        ...
        renderer = Render(
            size=renderer_resolution)  ## to do:: support different resolution

        normal_img = render_mesh(renderer,
                                 vex.astype(np.float32),
                                 faces,
                                 ixt,
                                 ext,
                                 shaded=args.shaded_mesh)
        cv2.imwrite(join(img_path, f"frame_{frame_time_id:06}_mesh.png"),
                    normal_img)
