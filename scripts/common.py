...
NGP_DATA_FOLDER = os.environ.get("NGP_DATA_FOLDER") or os.path.join(
    ROOT_DIR, "data")
...
sys.path += [
    os.path.dirname(pyd)
    for pyd in glob.iglob(os.path.join(ROOT_DIR, "build*", "**/*.pyd"),
                          recursive=True)
]
sys.path += [
    os.path.dirname(pyd)
    for pyd in glob.iglob(os.path.join(ROOT_DIR, "build*", "**/*.so"),
                          recursive=True)
]
...
def diagonally_combine_two_images(img1,
                                  img2,
                                  x_threshold,
                                  angle,
                                  gap=0,
    ...
    if img2.shape != img1.shape:
        raise ValueError(
            f"img1 and img2 must have the same shape; {img1.shape} vs {img2.shape}"
        )
    ...
    if gap > 0:
        rr, cc, val = weighted_line(
            0, int(x_threshold * img1.shape[1] - (angle * img1.shape[0] / 2)),
            img1.shape[0] - 1,
            int(x_threshold * img1.shape[1] + (angle * img1.shape[0] / 2)), gap)
        result[rr, cc, :] = result[rr, cc, :] * (
            1 - val[..., np.newaxis]) + val[..., np.newaxis] * color
    return result
...
def diagonally_combine_images(images, x_thresholds, angle, gap=0, color=1):
    ...
    for img, thres in zip(images[1:], x_thresholds):
        result = diagonally_combine_two_images(result, img, thres, angle, gap,
                                               color)
    return result
...
def srgb_to_linear(img):
    limit = 0.04045
    return np.where(img > limit, np.power((img + 0.055) / 1.055, 2.4),
                    img / 12.92)


def linear_to_srgb(img):
    limit = 0.0031308
    return np.where(img > limit, 1.055 * (img**(1.0 / 2.4)) - 0.055,
                    12.92 * img)
...
def read_image(file):
    if os.path.splitext(file)[1] == ".bin":
        with open(file, "rb") as f:
            ...
            img = np.frombuffer(bytes,
                                dtype=np.float16,
                                count=h * w * 4,
                                offset=8).astype(np.float32).reshape([h, w, 4])
    else:
